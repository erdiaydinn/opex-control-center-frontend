...
class LocationCompletionContractTest {
    private fun input(lineCount: Int = 3) = LocationCompletionInput(
        activeShiftId = " SHIFT-20260818-001 ",
        attemptId = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa1",
        confirmedLineCount = lineCount,
        deviceSequence = 8,
    ...
    @Test
    fun `completion canonical payload matches backend v5 golden vector`() {
        val body = LocationCompletionCanonical.body(input())
        val expected =
    ...
    fun `completion factory binds shift attempt lease count location and auth session`() {
        ...
        assertTrue(event.canonicalPayload.contains("\"attempt_id\":\"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa1\""))
        assertTrue(event.canonicalPayload.contains("\"lease_id\":\"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbb1\""))
    ...
    fun `empty location completion keeps zero line count in signed payload`() {
        val body = LocationCompletionCanonical.body(input(lineCount = 0))
        assertTrue(body.contains("\"confirmed_line_count\":0"))
    }

    @Test
    fun `sync routing admits only count line or location completion contracts`() {
        val countLine = """{"active_shift_id":"SHIFT-1","attempt_id":"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa1","lease_id":"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbb1","barcode":"869"}"""
        val completion =
            """{"active_shift_id":"SHIFT-1","attempt_id":"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa1","lease_id":"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbb1","event_kind":"LOCATION_COMPLETE"}"""
        val unknown = """{"active_shift_id":"SHIFT-1","event_kind":"DELETE_ALL"}"""

        assertEquals("/api/inventory/v1/terminal/events", InventorySyncContract.endpointPath(countLine))
        assertEquals(
