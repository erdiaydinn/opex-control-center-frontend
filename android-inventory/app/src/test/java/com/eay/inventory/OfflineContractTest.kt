...
class OfflineContractTest {
    ...
    @Test fun queueCorruptionFailsClosed() {
        ...
        assertEquals(
            ...
            QueueIntegrity.failureReason(event.copy(canonicalPayload = canonical + "x"), "session-a"),
        )
    }
    ...
    @Test fun interactiveSessionChangeBlocksReplay() {
        ...
        assertEquals(SyncQuarantineReason.AUTH_BINDING_CHANGED, QueueIntegrity.failureReason(event, "session-b"))
    }
    ...
    @Test fun exactDuplicateIdentityIsIdempotentAcrossQueueStateChanges() {
        ...
        val durable = OfflineEvent(
            "e-5", 45, canonical, hash, authBindingId = "session-a",
            state = "QUARANTINED", attempts = 3,
    ...
    @Test fun duplicateEventIdWithDifferentImmutableIdentityFailsClosed() {
        ...
        assertTrue(!OfflineEventIdentity.sameImmutableIdentity(durable, durable.copy(deviceSequence = 47)))
        assertTrue(!OfflineEventIdentity.sameImmutableIdentity(durable, durable.copy(canonicalPayload = canonical + " ")))
        assertTrue(!OfflineEventIdentity.sameImmutableIdentity(durable, durable.copy(payloadHash = "0".repeat(64))))
        assertTrue(!OfflineEventIdentity.sameImmutableIdentity(durable, durable.copy(authBindingId = "session-b")))
    }
    ...
    @Test fun canonicalEventIsStableAndQuantityNormalized() {
        ...
        assertTrue(body.contains("\"attempt_id\":"))
        assertTrue(body.contains("\"lease_id\":"))
        assertEquals(64, TerminalEventCanonical.hash(body).length)
    }

    @Test fun canonicalEventMatchesBackendV5GoldenVectorByteForByte() {
        val body = TerminalEventCanonical.body(
            TerminalEventInput(
        ...
        assertEquals(
            "350270fce65667e06fdb6804fc5e047e126e62eb594f7cb6b0cfc9a95e3370ca",
            TerminalEventCanonical.hash(body),
        )
    }

    @Test fun confirmedBlindCountLineCreatesImmutableLeaseBoundQueueEvent() {
        val acceptedScan = AcceptedScan(
            sourceEventId = "scan-1", source = ScannerSource.HARDWARE_DATAWEDGE,
            symbology = BarcodeSymbology.EAN13, value = "8690000000001",
            payloadHash = "c".repeat(64), capturedAtEpochMs = 1_500L,
        )
        val evidence = BlindCountLineEvidence(
            missionId = "mission-1", itemPayloadHash = acceptedScan.payloadHash, quantity = 5,
        )
        val event = InventoryCountEventFactory.create(
            context = context(), acceptedScan = acceptedScan, evidence = evidence,
            deviceSequence = 7, eventId = "11111111-1111-4111-8111-111111111111",
            occurredAt = "2026-08-18T15:00:00Z", authBindingId = "session-a",
        )
        assertEquals("350270fce65667e06fdb6804fc5e047e126e62eb594f7cb6b0cfc9a95e3370ca", event.payloadHash)
        assertTrue(QueueIntegrity.valid(event, "session-a"))
    ...
    @Test fun confirmedBlindCountLineRejectsScanEvidenceSubstitution() {
        val acceptedScan = AcceptedScan(
            sourceEventId = "scan-2", source = ScannerSource.HARDWARE_DATAWEDGE,
            symbology = BarcodeSymbology.EAN13, value = "8690000000001",
            payloadHash = "c".repeat(64), capturedAtEpochMs = 1_500L,
        )
        assertThrows(IllegalArgumentException::class.java) {
            InventoryCountEventFactory.create(
                context = context(), acceptedScan = acceptedScan,
                evidence = BlindCountLineEvidence(
                    missionId = "mission-1", itemPayloadHash = "d".repeat(64), quantity = 5,
                ),
                deviceSequence = 7, eventId = "11111111-1111-4111-8111-111111111111",
                occurredAt = "2026-08-18T15:00:00Z", authBindingId = "session-a",
            )
    ...
    @Test fun serverClassificationSeparatesRetryConflictAndPolicy() {
        assertEquals(SyncServerOutcome.EXACT_REPLAY, InventorySyncClassifier.classify(200, true, true).outcome)
        assertEquals(SyncServerOutcome.AUTH_REJECTED, InventorySyncClassifier.classify(401, null, null).outcome)
        assertEquals(SyncServerOutcome.POLICY_REJECTED, InventorySyncClassifier.classify(403, null, null).outcome)
        assertEquals(SyncServerOutcome.BUSINESS_CONFLICT, InventorySyncClassifier.classify(409, null, null).outcome)
        assertEquals(SyncServerOutcome.RETRYABLE_FAILURE, InventorySyncClassifier.classify(503, null, null).outcome)
        assertEquals(SyncServerOutcome.PERMANENT_REJECTED, InventorySyncClassifier.classify(422, null, null).outcome)
    }
