...
export default function App() {
  ...
  async function generateOptimalPlan() {
    ...
    await runOperation({ mode: 'plan', title: 'Council Engine planogram üretiyor', subtitle: 'Satış, storage, fixture, facing, depth ve refill maliyeti birlikte hesaplanıyor.' }, async (signal) => {
      // v1.5: UI yerleşiminin tek doğrusu mevcut Store DNA / fixture state'i.
      // Backend çalışırsa ürünü zenginleştiririz; ama yerleştirmeyi her zaman lokal Store Plan allocator ile yaparız.
      let sourceProducts = products;
      try {
        const payload = { products: normalizeProductsForBackend(products), layout: null, mode: 'COUNCIL' };
        const result = await api.generatePlanogramCouncil(payload, signal);
        const backendProducts = productsFromPlanogram(result, products);
        if (backendProducts?.length >= Math.min(products.length * 0.35, 250)) sourceProducts = backendProducts;
      } catch (err) {
        if (isAbort(err)) throw err;
        // Backend kapalıysa sistemi düşürme. CSV/XLSX datası ile lokal allocator devam eder.
      }

      const assigned = buildStorePlan(sourceProducts, objects, { forceFrontBalance: true });
      const nextObjects = updateObjectsFromPlan(objects, assigned).map((o) => ({ ...o, utilization: o.utilization ? Math.min(96, o.utilization + 2) : o.utilization }));
