...
import { applyPlacementRulesBeforePlan, DEFAULT_OPTIMIZATION_WEIGHTS } from './utils/placementRuleAdapter.js';
import { enforceFoodSafetyHardIsolation } from './utils/foodSafetyGuard.js';
...
export default function App() {
  ...
  async function generateOptimalPlan() {
    ...
    await runOperation({ mode: 'plan', title: 'Council Engine planogram üretiyor', subtitle: 'Satış, storage, fixture, facing, depth ve refill maliyeti birlikte hesaplanıyor.' }, async (signal) => {
      ...
      const assigned = buildStorePlan(sourceProducts, planObjects, {
  ...
  useEffect(() => {
    ...
    const result = enforceFoodSafetyHardIsolation(products, unplacedProducts);

    if (result.changed) {
      setProducts(result.products);
      setUnplacedProducts(result.unplacedProducts);

      if (typeof notify === 'function') {
        notify(`G?da/g?da d??? hard izolasyon: ${result.removed} ?r?n kalite riski nedeniyle atanamayana al?nd?.`);
      }
    }
  }, [products]);

const common = { lang, objects, setObjects, products, setProducts, unplacedProducts, setUnplacedProducts, tasks, setTasks, store, notify, setActive, onGenerate: generateOptimalPlan, storeDna, setStoreDna, readiness, setReadiness, placementRules, setPlacementRules, optimizationWeights, setOptimizationWeights };
  const pages = {
