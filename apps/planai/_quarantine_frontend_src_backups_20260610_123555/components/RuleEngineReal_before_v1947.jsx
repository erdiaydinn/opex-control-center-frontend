...
export default function RuleEngineReal({
  ...
  // Düzeltme #2 (UX): Strateji her zaman bir varsayılana sahip olduğundan,
  // "Stratejiyle plan üret" düğmesini ancak kullanıcı bir strateji kartını açıkça
  // onayladıktan sonra aktif ederiz. Onay localStorage'da kalıcıdır.
  const [strategyTouched, setStrategyTouched] = useState(() => {
    try {
  ...
  function chooseStrategy(mode) {
    ...
    // Kullanıcı bilinçli bir strateji seçimi yaptı; düğmeyi aktif et ve onayı kalıcı yap.
  ...
  async function generate() {
    if (!strategyTouched) {
      // Güvenlik: düğme pasif olmasına rağmen tetiklenirse plan üretme.
      return;
    }
      ...
      <section className="rle-hero">
        ...
        <div className="rle-hero-actions">
          <button
            ...
            onClick={generate}
            disabled={!strategyTouched}
            title={strategyTouched ? 'Seçili stratejiyle plan üret' : 'Önce aşağıdan bir strateji seç'}
          >
            Stratejiyle plan üret
          </button>
          {!strategyTouched && (
