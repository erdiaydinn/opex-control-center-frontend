...
export function normalizeProduct(raw = {}, idx = 0) {
  ...
  const rawFacingDemand = Math.round(num(getAny(raw, ['facing', 'facing_count'], 0), 0) || (sales >= 250 ? 3 : sales >= 60 ? 2 : 1));
  const facing = Math.max(1, Math.min(maxFrontFacingFor({ ...raw, sales, width }), rawFacingDemand));
  const depth = Math.max(1, Math.min(12, Math.round(rawDepthUnits || demandDepthFor({ ...raw, sales }, facing))));
  return {
    ...raw,
    sku,
    name,
...
function maxFrontFacingFor(p = {}) {
  const sales = Number(p.sales || 0);
  const width = Number(p.width || p.width_cm || 8);
...
function demandDepthFor(p = {}, facing = 1) {
  const sales = Number(p.sales || 0);
...
function slotTargetSkuCount(slot = {}) {
  const width = Number(slot.width || 100);

  if (slot.zone === 'FROZEN') return Math.max(4, Math.min(7, Math.floor(width / 22)));
  if (slot.zone === 'CHILLED') return Math.max(5, Math.min(8, Math.floor(width / 18)));

  return Math.max(5, Math.min(9, Math.floor(width / 16)));
}

function slotMaxSkuCount(slot = {}) {
  return slotTargetSkuCount(slot) + 3;
}

function slotDensityScore(slot = {}) {
  const target = slotTargetSkuCount(slot);
  const count = Number(slot.count || 0);

  if (count > 0 && count < target) return 0;
  if (count === 0) return 1.5;
  return 3 + (count - target) / Math.max(target, 1);
}


function _domainText(obj = {}) {
  return [
    obj.sku,
    obj.SKU,
    obj.name,
...
function isExcludedFromAllocator(p = {}) {
  const raw = _domainText(p);
...
function isRealProduceProduct(p = {}) {
  const raw = _domainText(p);

  const categoryProduce =
  ...
  const nameProduce =
    ...
    raw.includes("MARUL") ||
    raw.includes("DILL") ||
    raw.includes("DEREOTU") ||
    raw.includes("PARSLEY") ||
    raw.includes("MAYDANOZ") ||
    raw.includes("MUSHROOM") ||
    raw.includes("MANTAR") ||
    raw.includes("PUMPKIN") ||
    raw.includes("KABAK") ||
    raw.includes("MINT") ||
    raw.includes("NANE") ||
    raw.includes("GINGER") ||
    raw.includes("ZENCEFIL") ||
    raw.includes("ZENCEF?L");
  ...
  return (categoryProduce || nameProduce) && !fakeProduce;
}

function productDomainClass(p = {}) {
  const raw = _domainText(p);

  if (isExcludedFromAllocator(p)) return "EXCLUDED";
  if (isRealProduceProduct(p)) return "PRODUCE";
...
function slotDomainClass(slot = {}) {
  const raw = _domainText(slot);
...
function canPlaceProductInSlotDomain(p = {}, slot = {}) {
  const pd = productDomainClass(p);
  const sd = slotDomainClass(slot);
...
export function buildStorePlan(inputProducts = [], objects = [], options = {}) {
  ...
  for (const p0 of sorted) {
    ...
    const candidates = slots
      .filter((s) => targets.includes(s.areaId) && canPlaceProductInSlotDomain(p, s) && s.count < slotMaxSkuCount(s))
      .sort((a, b) => {
        ...
        const densityA = slotDensityScore(a);
        const densityB = slotDensityScore(b);
        return (targetPriority.get(ai) ?? 99) - (targetPriority.get(bi) ?? 99)
          || shelfOrder.indexOf(a.shelf) - shelfOrder.indexOf(b.shelf)
          || (densityA + frontBoostA) - (densityB + frontBoostB)
          || a.used - b.used
          || a.module - b.module;
      });
    ...
    placed.push({
      ...
      facing: chosenFacing || p.facing,
      depth: Math.max(Number(p.depth || 1), demandDepthFor(p, chosenFacing || p.facing)),
      depth_strategy: 'DEPTH_FIRST_FRONT_FACE_CAPPED',
      product_domain: productDomainClass(p),
      slot_domain: slotDomainClass(slot),
      aisle: slot.areaId,
