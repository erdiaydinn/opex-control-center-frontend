...
export function isOdorNonFood(p = {}) {
  ...
  return (
    ...
    h.includes('ÇAMAŞIR') ||
    h.includes('CAMASIR') ||
    h.includes('YUMUŞATICI') ||
    h.includes('YUMUSATICI') ||
    h.includes('SHAMPOO') ||
    h.includes('ŞAMPUAN') ||
    h.includes('BLEACH') ||
    h.includes('CLEAN')
...
function targetAreas(p, objects) {
  ...
  if (isOdorNonFood(p)) return ['I', 'H', 'STEEL_RACK', 'G'].filter(has);
  ...
  for (const p of sorted) {
    ...
    let candidates = slots
      .filter((s) => {
        ...
        return inTarget && isCompatible(p, s) && s.count < maxSkuCount(s);
      });

    if (!candidates.length) {
      candidates = slots.filter((s) => isCompatible(p, s) && s.count < maxSkuCount(s));
    }
    ...
    placed.push({
      ...
      placement_reason: `${slot.domain} domain + physical front capacity + depth-first stock`,
    });
  }
