...
function foodFamilyV1919(p = {}) {
  if (chemicalProductV1921(p)) return "CHEMICAL";
  if (petProductV1919 && petProductV1919(p)) return "PET";
  if (paperProductV1921(p)) return "PAPER";
  if (bulkyProductV1921(p)) return "BULKY";
  return "FOOD";
}) {
...
function familyCompatibleV1919(a, b) {
  return familyCompatibleV1921(a, b);
}
...
export function productDomain(p = {}) {
  const h = productTextV1919(p);
  const explicit = explicitStorageV1919(p);
  ...
  if (petProductV1919(p)) return "AMBIENT";
  if (shelfStableCheeseV1919(p)) return "AMBIENT";

  if (explicit.includes("AMBIENT") && !strongChilledV1919(p)) return "AMBIENT";

  if (
    explicit.includes("CHILLED") ||
    explicit.includes("SO?UK") ||
    explicit.includes("SOGUK") ||
    strongChilledV1919(p)
  ) return "CHILLED";
...
function chemicalProductV1921(p = {}) {
  const h = productTextV1919 ? productTextV1919(p) : haystack(p);
...
function safeFrontWidthV1921(p = {}, rawWidth = 8) {
  const h = productTextV1919 ? productTextV1919(p) : haystack(p);
  let w = Number(rawWidth || 8);

  // Common data issue: mm or bad catalog values accidentally arrive as cm.
  if (w > 120) w = w / 10;
  if (w > 80 && !bulkyProductV1921(p)) w = w / 10;

  if (h.includes("CARBOY") || h.includes("DAMACANA") || h.includes("19 L") || h.includes("19L")) {
    return Math.max(32, Math.min(40, w));
  }

  if (h.includes("12 X") || h.includes("6 X") || h.includes("5 L") || h.includes("5L")) {
    return Math.max(22, Math.min(32, w));
  }

  if (paperProductV1921(p)) {
    return Math.max(20, Math.min(30, w));
  }

  // Normal SKU guard: yogurt, cheese, snack, beverage single packs etc.
  // If a non-bulky product says 40+ cm, treat it as bad dimension.
  if (!bulkyProductV1921(p) && w > 28) return 10;

  return Math.max(4, Math.min(28, w));
}

function familyCompatibleV1921(a, b) {
  if (!a || !b) return true;
  if (a === b) return true;
  ...
  // Paper and bulky can share only within own family, not with food.
...
export function normalizeProduct(raw = {}, idx = 0) {
  ...
  const width = safeFrontWidthV1921({ ...raw, name, category, subcategory, brand }, Math.max(baseWidth, inferredWidthV1919({ ...raw, name, category, subcategory, brand })));
...
export function buildStorePlan(inputProducts = [], objects = [], options = {}) {
  ...
  for (const p of sorted) {
    ...
    placed.push({
      ...
      placement_reason: affinityScoreForSlot(p, slot, options.affinityMap || {}) > 0
