...
export function productDomain(p = {}) {
  ...
  if (catalogStorage) return catalogStorage;

  const abcStorage = abcStorageFallback(p);
  if (abcStorage) return abcStorage;
...
function buildSlots(objects = []) {
  ...
  for (const rawObject of objects || []) {
    ...
    const isMartek = objectText(rawObject).includes("MARTEK");
    const domain = isMartek ? "CHILLED" : slotDomain(rawObject);
    const zone = domain === "PRODUCE" ? "AMBIENT" : domain;

    const rawModules = Number(rawObject.modules || 0);
    const modules = Math.max(
      1,
      Math.min(
        80,
        isMartek
          ? Math.max(2, rawModules || 2)
          : (rawModules || (domain === "CHILLED" || domain === "FROZEN" ? 1 : 0))
      )
    );

    const rawShelves = Number(rawObject.shelves || 0);
    const shelvesTotal = Math.max(
      1,
      Math.min(
        800,
        isMartek
          ? Math.max(10, rawShelves || 0, modules * 5)
          : (rawShelves || (domain === "CHILLED" || domain === "FROZEN" ? 5 : modules * 6))
      )
    );

    const shelvesPerModule = Math.max(1, Math.ceil(shelvesTotal / modules));
    const width = shelfWidthForObject(rawObject, domain);

    for (let m = 1; m <= modules; m += 1) {
      for (let s = 1; s <= shelvesPerModule; s += 1) {
        slots.push({
          areaId: String(rawObject.id),
          areaLabel: rawObject.label || rawObject.id,
          zone,
          domain,
          module: m,
          shelf: s,
          width,
          usableWidth: width * 0.96,
          used: 0,
          count: 0,
          maxCount: 999,
          foodFamily: null,
