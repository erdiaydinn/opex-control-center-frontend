...
def _connection() -> Iterator[sqlite3.Connection]:
    """Open one short-lived SQLite session and always release its file handle.

    ``sqlite3.Connection`` implements a transaction context manager, but its
    ``__exit__`` method does not close the connection.  That distinction is
    mostly invisible on Unix and causes ``WinError 32`` on Windows when a
    temporary audit database is removed immediately after a test or a
    development command.  Keeping the lifecycle here makes every caller
    commit/rollback and close consistently.
    """
    conn = _connect()
    try:
        yield conn
