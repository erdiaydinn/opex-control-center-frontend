...
# =====================================================
# USAGE LOG / ANALYTICS EXPORT
# =====================================================
USAGE_LOGS = []

@app.post("/save-usage-log")
def save_usage_log(payload: Dict[str, Any] = Body(...)):
    USAGE_LOGS.append(payload)
    if len(USAGE_LOGS) > 5000:
        del USAGE_LOGS[:-5000]
    return {"status": "success", "saved": len(USAGE_LOGS)}

@app.get("/usage-logs")
def usage_logs():
    return {"status": "success", "logs": USAGE_LOGS[-500:]}
