...
DATA_DIR = Path(os.getenv("PLANAI_DATA_DIR", str(BACKEND_ROOT / "data"))).expanduser()
# `master_products.csv` is the only authoritative physical master. The
# catalog fallback exists only for old local installations during migration.
MASTER_CSV = DATA_DIR / "master_products.csv"
LEGACY_MASTER_CSV = DATA_DIR / "catalog.csv"
# Backward-compatible name used by the original V3 test/install contract.
# It is only a fallback; MASTER_CSV always wins when both files exist.
CATALOG_CSV = LEGACY_MASTER_CSV
_DEFAULT_CATALOG_CSV = str(CATALOG_CSV)
MASTER_XLSX = DATA_DIR / "master_products.xlsx"
AFFINITY_CSV = DATA_DIR / "basket_affinity_top.csv"
AFFINITY_PARQUET = DATA_DIR / "basket_affinity_top.parquet"
...
def num(v: Any, d: float = 0) -> float:
    try:
        ...
        if isinstance(v, float) and math.isnan(v):
            return d
        text = str(v).replace("%", "").strip().replace(" ", "")
...
def get(p: Dict[str, Any], names: List[str], default: Any = "") -> Any:
    ...
    lower = {str(k).casefold(): k for k in p.keys()}
    for n in names:
        real = lower.get(str(n).casefold())
...
def product_dedup_key(p: Dict[str, Any]) -> str:
    ...
    name = norm(get(p, [
        "product_name", "Product Name", "product_name_local",
        "pim_product_name_local", "Ürün Adı",
    ], ""))
    b = norm(get(p, ["brand", "Brand", "brand_name", "Marka"], ""))
    weight_val = norm(get(p, [
        "product_contents_value", "product_weight_value", "weight_kg",
        "Ürün Brüt Ağırlık (Gr)",
    ], ""))
    weight_unit = norm(get(p, [
        "product_contents_unit", "product_weight_unit", "Ürün Ağırlık Birimi",
    ], ""))
    cat = norm(get(p, [
        "frontend_category_local", "Category L1", "category_l1", "category",
        "Ürün Segmenti",
    ], ""))
    sub = norm(get(p, [
        "frontend_subcategory_local", "Category L2", "category_l2", "subcategory",
    ], ""))
...
def _read_csv_table(path: Path) -> pd.DataFrame:
    ...
    for encoding in ("utf-8-sig", "utf-8", "cp1254"):
        try:
            ...
            if len(df.columns) == 1:
                auto = pd.read_csv(path, encoding=encoding, sep=None, engine="python", low_memory=False)
                if len(auto.columns) > 1:
                    df = auto
            return df
...
def _records_without_nan(df: pd.DataFrame) -> List[Dict[str, Any]]:
    """Convert tabular data to JSON-safe Python records.

    ``DataFrame.where(..., None)`` does not replace missing values in
    numeric columns because pandas keeps those columns as floating-point
    dtype.  The resulting ``float('nan')`` later makes FastAPI's JSON
    response fail with ``Out of range float values are not JSON compliant``.
    Converting to object dtype first preserves real nulls as ``None``.
    """
    safe_df = df.astype(object).where(pd.notna(df), None)
    return safe_df.to_dict(orient="records")


def read_master_rows() -> Tuple[List[Dict[str, Any]], Optional[Path]]:
    # Prefer the canonical master. The two legacy names are kept so older
    # local installers/tests can still point the fallback at catalog.csv.
    legacy_path = CATALOG_CSV if str(CATALOG_CSV) != _DEFAULT_CATALOG_CSV else LEGACY_MASTER_CSV
    for raw_path in (MASTER_CSV, MASTER_XLSX, legacy_path):
        path = Path(raw_path)
        if not path.exists():
            continue
        df = pd.read_excel(path) if path.suffix.lower() == ".xlsx" else _read_csv_table(path)
        return _records_without_nan(df), path
    return [], None


def _has_positive_dimensions(row: Dict[str, Any]) -> bool:
    return all([
        num(get(row, ["Ürün En (cm)", "product_width_in_cm", "width_cm", "width"], 0), 0) > 0,
        num(get(row, ["Ürün Boy (CM)", "product_length_in_cm", "depth_cm", "length_cm"], 0), 0) > 0,
        num(get(row, ["Ürün Yükseklik (cm)", "product_height_in_cm", "height_cm", "height"], 0), 0) > 0,
    ])
...
def load_master(force: bool = False) -> Dict[str, Any]:
    ...
    rows, source_path = read_master_rows()
    ...
    for r in rows:
        sku_v = norm(get(r, ["sku", "SKU", "Ürün Kodu (Global)", "global_sku"], ""))
        barcode_v = norm(first_barcode(get(r, [
            "product_barcodes", "barcode", "Barcode", "Barcodes",
            "Ürün Barkodu", "Dış Koli Barkodu", "İç Koli Barkodu (Var ise)",
        ], "")))
        catalog_v = norm(get(r, ["catalog_global_product_id", "Ürün Kodu (Global)"], ""))
    ...
    MASTER_CACHE.update({
        ...
        "dimension_rows": sum(1 for r in rows if _has_positive_dimensions(r)),
    })
...
def find_master_match(p: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    sku_v = norm(get(p, ["sku", "SKU", "Ürün Kodu (Global)", "global_sku"], ""))
    barcode_v = norm(first_barcode(get(p, [
        "product_barcodes", "barcode", "Barcode", "Barcodes",
        "Ürün Barkodu", "Dış Koli Barkodu", "İç Koli Barkodu (Var ise)",
    ], "")))
    catalog_v = norm(get(p, ["catalog_global_product_id", "Ürün Kodu (Global)"], ""))
...
def _find_column(columns: List[str], exact: List[str], contains: List[str]) -> Optional[str]:
    ...
    for column in columns:
        ...
        if any(token.casefold() in folded for token in contains):
            return column
    return None


def _read_affinity_table(path: Path) -> pd.DataFrame:
    if path.suffix.lower() == ".parquet":
        return pd.read_parquet(path)
    return _read_csv_table(path)


def load_affinity(force: bool = False) -> Dict[str, Any]:
    """Load basket affinity as an optional two-way soft placement signal."""
    if AFFINITY_CACHE["loaded"] and not force:
        return AFFINITY_CACHE

    source_path = None
    df = None
    for raw_path in (AFFINITY_PARQUET, AFFINITY_CSV):
        path = Path(raw_path)
        if not path.exists():
            continue
        try:
            df = _read_affinity_table(path)
            source_path = path
            break
        except Exception:
            continue
    ...
    if df is not None and not df.empty:
        ...
        source_col = _find_column(
            columns,
            ["source_sku", "sku", "sku_1", "product_a", "item_a", "base_sku"],
            ["source", "sku_1", "product_a", "item_a", "from"],
        )
        target_col = _find_column(
            columns,
            ["target_sku", "related_sku", "sku_2", "product_b", "item_b", "neighbor_sku"],
            ["target", "related", "sku_2", "product_b", "item_b", "to"],
        )
        score_col = _find_column(
            columns,
            ["affinity_score", "score", "lift", "confidence", "support", "co_purchase_count"],
            ["affinity", "score", "lift", "confidence", "support", "count"],
        )
        rank_col = _find_column(columns, ["rank", "affinity_rank"], ["rank"])

        if not source_col or not target_col:
            sku_like = [
                c for c in columns
                if any(token in c.casefold() for token in ["sku", "product", "item"])
            ]
            if len(sku_like) >= 2:
                source_col, target_col = sku_like[:2]

        if source_col and target_col:
            for row in df.to_dict(orient="records"):
                source = norm(row.get(source_col))
                target = norm(row.get(target_col))
                if not source or not target or source == target:
                    continue
                score = num(row.get(score_col), 0) if score_col else 0
                if score <= 0 and rank_col:
                    score = 1 / max(num(row.get(rank_col), 1), 1)
                # Affinity exports commonly use probability-like scores, but
                # lift is intentionally allowed to remain >1. Preserve the
                # metric's scale while capping pathological values.
                score = min(max(score or 1.0, 0.0), 100.0)
                by_sku.setdefault(source, {})[target] = max(score, by_sku.setdefault(source, {}).get(target, 0.0))
                by_sku.setdefault(target, {})[source] = max(score, by_sku.setdefault(target, {}).get(source, 0.0))

        used_columns = [x for x in [source_col, target_col, score_col, rank_col] if x]
    ...
    AFFINITY_CACHE.update({
        ...
        "pair_rows": int(len(df)) if df is not None else 0,
        "pairs": sum(len(v) for v in by_sku.values()),
        "link_count": sum(len(v) for v in by_sku.values()),
        "by_sku": by_sku,
        "links": by_sku,
...
def data_source_summary() -> Dict[str, Any]:
    ...
    return {
        "master": {
            "path": master.get("source_path"),
            "rows": len(master.get("rows", [])),
            "dimension_rows": master.get("dimension_rows", 0),
            "sku_index": len(master.get("by_sku", {})),
            "barcode_index": len(master.get("by_barcode", {})),
        },
        "basket_affinity": {
            "path": affinity.get("source_path"),
            "rows": affinity.get("rows", 0),
            "pairs": affinity.get("pairs", 0),
            "columns": affinity.get("columns", []),
        },
    }
...
def product_name(p: Dict[str, Any]) -> str:
    return clean_text(get(p, [
        "product_name", "Product Name", "product_name_local",
        "pim_product_name_local", "Ürün Adı", "name",
    ], ""))


def sku(p: Dict[str, Any]) -> str:
    return clean_text(get(p, ["sku", "SKU", "Ürün Kodu (Global)", "global_sku", "barcode"], ""))
...
def storage_type(p: Dict[str, Any]) -> str:
    explicit = get(p, [
        "storage_type", "Storage Type", "Storage", "storage_raw",
        "Saklama Koşulu (Raf/ +4/-18)",
    ], "")
...
def width(p: Dict[str, Any]) -> float:
    return max(0, num(get(p, [
        "width_cm", "Width", "en", "product_width_in_cm", "Ürün En (cm)",
    ], 0), 0))


def height(p: Dict[str, Any]) -> float:
    return max(0, num(get(p, [
        "height_cm", "Height", "yükseklik", "product_height_in_cm",
        "Ürün Yükseklik (cm)",
    ], 0), 0))
...
def depth(p: Dict[str, Any]) -> float:
    return max(0, num(get(p, [
        "depth_cm", "Depth", "derinlik", "product_length_in_cm", "Ürün Boy (CM)",
    ], 0), 0))


def weight(p: Dict[str, Any]) -> float:
    explicit_kg = get(p, ["weight_kg"], "")
    catalog_grams = get(p, ["Ürün Brüt Ağırlık (Gr)"], "")
    raw = explicit_kg if explicit_kg not in [None, ""] else get(p, [
        "Weight", "agirlik", "product_weight_value",
    ], catalog_grams if catalog_grams not in [None, ""] else 0.2)
    unit = norm(get(p, ["product_weight_unit", "weight_unit", "Ürün Ağırlık Birimi"], ""))
    value = num(raw, 0.2)
    if (explicit_kg in [None, ""] and catalog_grams not in [None, ""]) or unit in ["g", "gr", "gram", "grams"] or value > 20:
        value /= 1000
...
def case_pack(p: Dict[str, Any]) -> float:
    return max(1, num(get(p, [
        ...
        "units_in_pack_count",
        "Koli İçi Adedi",
        "Paket İçi Ürün Adedi(Varsa)(Yoksa Boşluk Bırak)",
    ], 12), 12))
...
def ai_estimate_dimensions(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "su" in words or "water" in words or "soda" in words:
...
def enrich_product(raw: Dict[str, Any], allow_ai_dimensions: bool = False) -> Dict[str, Any]:
    ...
    p_name = first_non_empty(
        get(original, ["product_name", "Product Name", "product_name_local", "Ürün Adı", "name"], ""),
        get(master, ["product_name", "product_name_local", "pim_product_name_local", "Ürün Adı"], ""),
    )

    b_name = first_non_empty(
        get(original, ["brand", "Brand", "brand_name", "Marka"], ""),
        get(master, ["brand_name", "Marka"], ""),
    ...
    cat2 = first_non_empty(
        get(original, ["category_l2", "Category L2", "subcategory", "frontend_subcategory_local", "Kategori"], ""),
        get(master, ["frontend_subcategory_local", "pim_cat_l2", "Kategori"], ""),
        "GENERAL",
    )
    ...
    file_w = get(original, ["width_cm", "Width", "en", "product_width_in_cm", "Ürün En (cm)"], "")
    file_h = get(original, ["height_cm", "Height", "yükseklik", "product_height_in_cm", "Ürün Yükseklik (cm)"], "")
    file_d = get(original, ["depth_cm", "Depth", "derinlik", "product_length_in_cm", "Ürün Boy (CM)"], "")

    master_w = get(master, ["product_width_in_cm", "width_cm", "Ürün En (cm)"], "")
    master_h = get(master, ["product_height_in_cm", "height_cm", "Ürün Yükseklik (cm)"], "")
    master_d = get(master, ["product_length_in_cm", "depth_cm", "Ürün Boy (CM)"], "")

    has_file_dim = num(file_w, 0) > 0 and num(file_h, 0) > 0 and num(file_d, 0) > 0
    has_master_dim = num(master_w, 0) > 0 and num(master_h, 0) > 0 and num(master_d, 0) > 0
    ...
    if has_file_dim:
        ...
        source_name = Path(str(MASTER_CACHE.get("source_path") or "")).name.casefold()
        if source_name == "catalog.csv":
            dim_source, dim_conf, dim_reason = "catalog", 1, "catalog_dimensions"
        else:
    ...
    weight_source = {**merged, **original}
    if not get(weight_source, [
        "weight_kg", "Weight", "product_weight_value", "Ürün Brüt Ağırlık (Gr)",
    ], ""):
        weight_source["weight_kg"] = estimate.get("weight_kg", 0.2)

    result = {
        **merged,
        "sku": first_non_empty(
            get(original, ["sku", "SKU", "Ürün Kodu (Global)"], ""),
            get(master, ["sku", "Ürün Kodu (Global)"], ""),
            get(original, ["barcode"], ""),
        ),
        "barcode": first_non_empty(
            get(original, ["barcode", "Barcode", "Barcodes", "product_barcodes", "Ürün Barkodu"], ""),
            get(master, ["product_barcodes", "Ürün Barkodu"], ""),
        ),
        "pim_product_id": first_non_empty(get(original, ["pim_product_id"], ""), get(master, ["pim_product_id"], "")),
        "catalog_global_product_id": first_non_empty(
            get(original, ["catalog_global_product_id", "Ürün Kodu (Global)"], ""),
            get(master, ["catalog_global_product_id", "Ürün Kodu (Global)"], ""),
        ),
        ...
        "category_l1": cat1,
        "category_l2": cat2,
        "product_segment": first_non_empty(
            get(original, ["product_segment", "Ürün Segmenti"], ""),
            get(master, ["product_segment", "Ürün Segmenti"], ""),
        ),
        ...
        "height_cm": max(0, h),
        "depth_cm": max(0, d),
        "weight_kg": weight(weight_source),
...
def basket_affinity_score(
    p: Dict[str, Any],
    aisle: Dict[str, Any],
    module: Dict[str, Any],
    shelf: Dict[str, Any],
) -> float:
    """Reward nearby products that are bought with the candidate.

    This is deliberately a soft score. It cannot override storage, fixture,
    dimensions, capacity, weight, or an explicit module/shelf rule.
    """
    related = load_affinity().get("by_sku", {}).get(norm(sku(p)), {})
    if not related:
        return 0

    same_shelf = shelf.get("products", [])
    same_module = [
        product
        for candidate_shelf in module.get("shelves", [])
        for product in candidate_shelf.get("products", [])
    ]
    same_aisle = [
        product
        for candidate_module in aisle.get("modules", [])
        for candidate_shelf in candidate_module.get("shelves", [])
        for product in candidate_shelf.get("products", [])
    ]

    def total(products: List[Dict[str, Any]]) -> float:
        return sum(related.get(norm(x.get("sku")), 0.0) for x in products)

    return total(same_shelf) * 220 + total(same_module) * 90 + total(same_aisle) * 20
...
def placement_score(
    ...
    score += basket_affinity_score(p, aisle, module, shelf) * cfg["affinity"]
...
def place_product_fast(
    ...
    # The old engine evaluated only a six-shelf heuristic window. That made
    # the result depend on list order and prevented affinity from choosing a
    # better compatible shelf. Keep the deterministic priority only for
    # evaluation order, but score every viable shelf.
    candidates = sorted(viable, key=candidate_priority)
...
def generate_planogram(
    ...
    # Do not silently discard catalog rows. If a caller needs a bounded
    # preview it must send a bounded product list explicitly.
    ...
    return {
        ...
        "insights": {
            ...
            "master_data": "Ürün ölçüleri kanonik master_products.csv/catalog.csv kaynağından okundu; eksik ölçüler sessizce tahmin edilmedi.",
            "basket_affinity": "Birlikte satın alma ilişkileri aynı raf/modül/koridor için soft yerleşim skoru olarak kullanıldı.",
        },
        "recommended_actions": [
            ...
            "dimension_missing kayıtları için doğrulanmış en-boy-yükseklik ölçüsünü master_products.csv içine ekle.",
        ],
