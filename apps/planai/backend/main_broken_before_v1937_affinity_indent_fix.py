...
def resolve_basket_affinity(payload: dict = Body(...)):
 ...
 norm(v):
        s = str(v or "").strip().lower()
        s = s.replace("?        s = s.replace("?", "s").replace("?", "g").replace("?", "u").replace("?", "o").replace("?", "c")
        s = s.replace("yemeksepeti market,", "").replace("yemeksepeti market", "")
        for ch in ["(", ")", ",", ".", "-", "_"]:
            s = s.replace(ch, " ")
        return " ".join(s.split())

    def resolve_vendor_id(raw):
        n = norm(raw)
        ...
        return hard.get(n, str(raw or "").strip())
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            if requested_skus and a not in requested_skus and b not in requested_skus:
                continue

            sku_hit_rows += 1

            if not requested_skus or a in requested_skus:
            ...
            if not requested_skus or b in requested_skus:
