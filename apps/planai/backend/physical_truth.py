...
def _product_text(product: Dict[str, Any]) -> str:
    return _norm(" ".join(_text(_first(product, field)) for field in (
        "product_name", "name", "Product Name", "brand", "brand_name",
        "category_l1", "category_l2", "frontend_category_local",
        "frontend_subcategory_local", "product_contents_value",
        "product_contents_unit", "volume", "volume_unit", "pack_type",
    )))
...
def normalize_temperature_zone(value: Any, default: str = "AMBIENT") -> str:
    ...
    if any(token in raw for token in ("+4", "chilled", "soguk", "dolap", "fridge", "cooler")):
        return "CHILLED"
    if any(token in raw for token in ("ambient", "raf", "shelf", "dry", "kuru", "pallet", "palet", "hdr")):
        return "AMBIENT"
    return default


def explicit_storage_value(product: Dict[str, Any]) -> str:
    return _text(_first(product, "catalog_storage_condition_raw", "catalog_storage_type", "storage_raw", "Storage Type", "storage_type", "storage"))
...
def parse_pack_metrics(product: Dict[str, Any]) -> Dict[str, Any]:
    ...
    source = "missing"
    match = re.search(
        r"(?:^|[^0-9])(?P<count>\d{1,3})\s*[x×]\s*(?P<value>\d+(?:[.,]\d+)?)\s*(?P<unit>ml|cl|l|lt|ltr|litre|liter)(?:[^a-z0-9]|$)",
        raw,
    )
    if match:
        ...
        unit_liters = value / 1000.0 if unit == "ml" else value / 100.0 if unit == "cl" else value
        source = "name_multipack"
    else:
        pack_count = int(max(1, _num(_first(product, "units_in_pack_count", "pack_count"), 1)))
        contents_value = _num(_first(product, "product_contents_value", "volume", "volume_value"), 0)
        contents_unit = _norm(_first(product, "product_contents_unit", "volume_unit"))
        if contents_value > 0 and contents_unit in {"ml", "cl", "l", "lt", "ltr", "litre", "liter"}:
            count = pack_count
            unit_liters = contents_value / 1000.0 if contents_unit == "ml" else contents_value / 100.0 if contents_unit == "cl" else contents_value
            source = "structured_content"
        else:
            single = re.search(r"(?:^|[^0-9])(?P<value>\d+(?:[.,]\d+)?)\s*(?P<unit>ml|cl|l|lt|ltr|litre|liter)(?:[^a-z0-9]|$)", raw)
            if single:
    ...
    return {"pack_count": count, "unit_liters": None if unit_liters is None else round(unit_liters, 4), "total_pack_liters": total_liters, "source": source}
...
def is_detergent_or_cleaning(product: Dict[str, Any]) -> bool:
    raw = _product_text(product)
    return any(token in raw for token in ("deterjan", "cleaning", "cleaner", "bleach", "camasir", "yumusatici", "domestos", "temizleyici", "bulasik"))


def is_explicit_bulky_pallet_product(product: Dict[str, Any]) -> bool:
    raw = _product_text(product)
    return any(token in raw for token in ("kedi kumu", "cat litter", "tuvalet kagidi", "toilet paper", "paper towel", "kagit havlu", "damacana"))


def product_weight_kg(product: Dict[str, Any]) -> float:
    return max(0.0, _num(_first(product, "weight_kg", "product_weight_kg", "Weight", "product_weight_value"), 0))
...
def requires_bottom_shelf(product: Dict[str, Any]) -> bool:
    if requires_pallet_fixture(product):
        return False
    return product_weight_kg(product) >= HEAVY_BOTTOM_THRESHOLD_KG or (is_detergent_or_cleaning(product) and product_weight_kg(product) >= 2.0)
...
def is_approved_dimension_product(product: Dict[str, Any]) -> bool:
    ...
    if source not in APPROVED_DIMENSION_SOURCES:
        return False
    return all(_num(product.get(field), 0) > 0 for field in ("width_cm", "height_cm", "depth_cm"))
...
def product_truth_report(products: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    estimated = sum(1 for product in rows if _norm(product.get("dimension_source")) in ESTIMATED_DIMENSION_SOURCES)
    missing = sum(1 for product in rows if _norm(product.get("dimension_source")) == "missing" or any(_num(product.get(field), 0) <= 0 for field in ("width_cm", "height_cm", "depth_cm")))
    images = sum(1 for product in rows if _text(_first(product, "image_url", "catalog_image_url", "pim_image_url", "Product Image URL")))
    master_linked = sum(1 for product in rows if _text(_first(product, "catalog_global_product_id", "pim_product_id")) or _norm(product.get("dimension_source")) in {"master", "approved_master"})
...
def _module_fixture_class(module: Dict[str, Any], shelf: Optional[Dict[str, Any]] = None) -> str:
    raw = _norm(" ".join(_text(value) for value in (module.get("fixture_class"), module.get("fixture_type"), module.get("module_type"), module.get("storage_type"), module.get("zone"), (shelf or {}).get("allowed_storage_type"))))
...
def layout_truth_report(layout: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    physical_shelves = sum(1 for shelf in shelves if all(_num(shelf.get(field), 0) > 0 for field in ("shelf_width_cm", "shelf_height_cm", "shelf_depth_cm", "max_weight_kg")) and _text(shelf.get("allowed_storage_type") or shelf.get("storage_type")))
    sided_modules = sum(1 for module in modules if _text(module.get("side")).upper() in VALID_SIDES)
    fixture_counts = Counter(_module_fixture_class(module, shelf) for aisle in aisles for module in (aisle.get("modules") or []) for shelf in (module.get("shelves") or [None]))
    pct = lambda value, denominator: round((value / denominator) * 100, 2) if denominator else 0.0
    blockers: List[str] = []
    if not shelves:
    ...
    return {"present": True, "aisle_count": len(aisles), "module_count": len(modules), "shelf_count": len(shelves), "physical_shelf_count": physical_shelves, "fixture_capacity_coverage_pct": pct(physical_shelves, len(shelves)), "sided_module_count": sided_modules, "module_side_coverage_pct": pct(sided_modules, len(modules)), "fixture_class_counts": dict(sorted(fixture_counts.items())), "blockers": blockers}


def _picker_width_m(store_dna: Dict[str, Any]) -> Optional[float]:
    for field in ("picker_aisle_width_m", "center_aisle_width_m", "walking_aisle_width_m", "aisle_clear_width_m"):
...
def store_dna_truth_report(store_dna: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    picker_valid = bool(picker_width is not None and PRODUCTION_PICKER_AISLE_MIN_M <= picker_width <= PRODUCTION_PICKER_AISLE_MAX_M)
    ...
    evidence_count = sum([bool(measured), aisle_count > 0, total_modules > 0, side_pct == 100, picker_valid])
    return {"present": True, "source": source or None, "measured": measured, "aisle_count": aisle_count, "module_count": total_modules, "side_module_coverage_pct": side_pct, "picker_aisle_width_m": None if picker_width is None else round(picker_width, 3), "picker_aisle_width_valid": picker_valid, "picker_aisle_expected_range_m": [PRODUCTION_PICKER_AISLE_MIN_M, PRODUCTION_PICKER_AISLE_MAX_M], "coverage_pct": round((evidence_count / 5) * 100, 2), "blockers": blockers}
...
def required_fixture_gap_report(products: Iterable[Dict[str, Any]], layout: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    missing = sorted(requirement for requirement, count in product_requirements.items() if count > 0 and available.get(requirement, 0) <= 0)
    return {"required_fixture_counts": dict(sorted(product_requirements.items())), "available_fixture_counts": dict(sorted(available.items())), "missing_fixture_classes": missing}
...
def production_acceptance_report(products: Iterable[Dict[str, Any]], layout: Optional[Dict[str, Any]], store_dna: Optional[Dict[str, Any]], *, require_images: bool = True) -> Dict[str, Any]:
    ...
    return {"production_ready": not blockers, "solver_optimizer_allowed": not blockers, "dataset": product_report, "layout": layout_report, "store_dna": dna_report, "fixture_requirements": fixture_report, "blockers": blockers}
...
def clone_with_physical_truth(product: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result["temperature_zone"] = temperature
    result["required_fixture_class"] = fixture
    ...
    # The legacy foundation engine marks any 5 L product as PALLET. Physical
    # truth recomputes that compatibility axis: large beverages/bulky products
    # may be PALLET, while a 5 L detergent remains ambient regular-shelf stock.
