...
def resolve_basket_affinity(payload: dict):
    import csv
    ...
    def norm(value):
        ...
        s = s.replace("?", "s").replace("?", "g").replace("?", "u").replace("?", "o").replace("?", "c")
        s = s.replace("yemeksepeti market,", "").replace("yemeksepeti market", "")
        for ch in ["(", ")", ",", ".", "-", "_"]:
            s = s.replace(ch, " ")
        return " ".join(s.split())

    def resolve_vendor_id(raw_value):
        n = norm(raw_value)
        hard_map = {
            "fulya": "j3a6",
            "fulya istanbul": "j3a6",
        ...
        return hard_map.get(n, str(raw_value or "").strip())
    ...
    resolved_vendor_id = resolve_vendor_id(raw_vendor or raw_store)

    max_partners_per_sku = int(payload.get("max_partners_per_sku") or 8)
    max_pairs = int(payload.get("max_pairs") or 5000)
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            if resolved_vendor_id and vendor and vendor != resolved_vendor_id:
                continue

            vendor_rows += 1

            sku_a = str(row.get("sku_a") or "").strip()
            sku_b = str(row.get("sku_b") or "").strip()

            if not sku_a or not sku_b or sku_a == sku_b:
                continue

            try:
                score = float(row.get("weighted_affinity_score") or row.get("affinity_score") or row.get("score") or 0)
            except Exception:
                score = 0.0

            try:
                together_count = int(float(row.get("together_count") or 0))
            except Exception:
                together_count = 0
            ...
            # SKU listesi geldiyse sadece o havuzdaki ili?kileri d?nd?r.
            # SKU listesi bo?sa debug i?in ilk max_pairs kadar vendor pair d?nd?r.
            if requested_skus:
                if sku_a not in requested_skus and sku_b not in requested_skus:
                    continue
            elif pair_count >= max_pairs:
                break

            sku_hit_rows += 1

            if not requested_skus or sku_a in requested_skus:
                affinity.setdefault(sku_a, []).append({
                    "sku": sku_b,
                    "score": score,
            ...
            if not requested_skus or sku_b in requested_skus:
                affinity.setdefault(sku_b, []).append({
                    "sku": sku_a,
                    "score": score,
                    "together_count": together_count,
                    "vendor_id": vendor,
    ...
    return {
        ...
        "summary": {
            ...
            "sku_hit_rows": sku_hit_rows,
            "raw_store": raw_store or None,
            "raw_vendor_id": raw_vendor or None,
            "resolved_vendor_id": resolved_vendor_id,
