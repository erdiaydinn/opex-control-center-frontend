...
def storage_type(p: Dict[str, Any]) -> str:
    """Return the HARD storage class used by the optimizer.

    Rule: an explicit catalog value wins. We only infer from name/category when
    storage_type is missing. This prevents AMBIENT products being reclassified
    because category text contains cold/frozen words, and keeps catalog truth as
    the single source of truth.
    """
    explicit = key(get(p, ["storage_type", "Storage Type", "Storage", "storage"], ""))
    explicit = explicit.replace(" ", "_")

    if explicit in ["AMBIENT", "CHILLED", "FROZEN", "PALLET"]:
        return explicit

    raw = key(
        f"{get(p, ['storage_raw', 'storageRaw', 'fixture_type', 'fixture_kind'], '')} "
        f"{product_name(p)} "
        f"{category_l1(p)} "
        f"{category_l2(p)}"
    )
    ...
    if any(x in raw for x in ["CHILLED", "COLD", "+4", "SÜT", "SUT", "DAIRY", "YOĞURT", "YOGURT", "FRIDGE", "DOLAP", "SOĞUK", "SOGUK"]):
        return "CHILLED"
...
def shelf_storage(shelf: Dict[str, Any]) -> str:
    value = key(get(shelf, ["allowed_storage_type", "storage_type", "storage"], "AMBIENT"))
    if value in ["AMBIENT", "CHILLED", "FROZEN", "PALLET"]:
        return value
    return "AMBIENT"


def fixture_kind(p: Dict[str, Any]) -> str:
    """Normalize catalog storage_raw/fixture type into a physical fixture class."""
    explicit = key(get(p, ["fixture_kind", "required_fixture_kind", "fixture_type"], ""))
    if explicit in ["SHELF", "FRIDGE", "FREEZER", "PALLET", "COLD_ROOM", "FROZEN_ROOM", "CHEST_FREEZER"]:
        return explicit

    raw = key(get(p, ["storage_raw", "storageRaw", "storage_mapping_source"], ""))
    st = storage_type(p)
    ...
    if any(x in raw for x in ["DOLAP", "+4", "CHILL", "CHILLED", "FRIDGE", "SOĞUK", "SOGUK"]):
        return "FRIDGE"
    if any(x in raw for x in ["RAF", "SHELF", "RAFLI", "GONDOLA"]):
        return "SHELF"
...
def module_fixture_kind(module: Dict[str, Any], shelf: Optional[Dict[str, Any]] = None) -> str:
    raw = key(
        f"{module.get('module_type', '')} {module.get('fixture_type', '')} "
        f"{module.get('temperature', '')} {module.get('source_layer', '')} "
        f"{module.get('label', '')}"
    )
    storage = shelf_storage(shelf or {})

    if any(x in raw for x in ["FREEZER", "FROZEN", "-18", "DONUK", "DONDUR", "ALGIDA"]):
        return "FREEZER"
    if any(x in raw for x in ["FRIDGE", "COOLER", "CHILL", "COLD", "+4", "DOLAP", "SOĞUK", "SOGUK"]):
        return "FRIDGE"
    if storage == "FROZEN":
        return "FREEZER"
    if storage == "CHILLED":
...
def enrich_product(raw: Dict[str, Any], allow_ai_dimensions: bool = True) -> Dict[str, Any]:
    ...
    result = {
        **merged,
        ...
        "storage_raw": first_non_empty(get(original, ["storage_raw", "storageRaw"], ""), get(master, ["storage_raw", "storageRaw"], "")),
        "storage_mapping_source": first_non_empty(get(original, ["storage_mapping_source"], ""), get(master, ["storage_mapping_source"], "")),
        "fixture_kind": fixture_kind({**base, **merged}),
        "required_fixture_kind": fixture_kind({**base, **merged}),
        "data_quality_flags": first_non_empty(get(original, ["data_quality_flags"], ""), get(master, ["data_quality_flags"], "")),
        "supplier_name": first_non_empty(get(original, ["supplier_name"], ""), get(master, ["supplier_name"], "")),
...
def merch_compatible(p: Dict[str, Any], aisle: Dict[str, Any], cached_groups: Optional[set] = None) -> bool:
    ...
    pg = p.get("_merch_group") or merch_group(p)

    # A koridoru hızlı gıda / yüksek frekans alanı olarak kalsın; gıda dışı buraya düşmesin.
    if aid == "A" and not is_food(p):
        return False

    # Eski motor sadece kokulu non-food'u ayırıyordu. Pratikte gıda + gıda dışı yan yana
    # görünmesinin ana sebebi buydu. Artık tüm non-food grupları gıda ile aynı koridora karışmaz.
    if str(pg).startswith("FOOD") and any(str(g).startswith("NON_FOOD") for g in groups):
        return False

    if str(pg).startswith("NON_FOOD") and any(str(g).startswith("FOOD") for g in groups):
        return False
...
def fixture_compatible(p: Dict[str, Any], module: Dict[str, Any], shelf: Dict[str, Any]) -> bool:
    required = fixture_kind(p)
    actual = module_fixture_kind(module, shelf)

    if required == "FREEZER":
        return actual == "FREEZER" and shelf_storage(shelf) == "FROZEN"
    if required == "FRIDGE":
        return actual == "FRIDGE" and shelf_storage(shelf) == "CHILLED"
    if required == "SHELF":
        return actual == "SHELF" and shelf_storage(shelf) == "AMBIENT"
    return True


def shelf_group_compatible(p: Dict[str, Any], shelf: Dict[str, Any]) -> bool:
    products = shelf.get("products", [])
    if not products:
        return True

    pg = p.get("_merch_group") or merch_group(p)
    p_cat1 = norm(category_l1(p))
    p_cat2 = norm(category_l2(p))

    for existing in products:
        eg = existing.get("merch_group") or existing.get("_merch_group") or merch_group(existing)
        if str(pg).startswith("FOOD") != str(eg).startswith("FOOD"):
            return False
        if str(pg).startswith("NON_FOOD") != str(eg).startswith("NON_FOOD"):
            return False

        # Aynı rafta kategori L1 karışmasın. L2 aynıysa sorun yok; marka zaten score ile gruplanır.
        e_cat1 = norm(category_l1(existing))
        e_cat2 = norm(category_l2(existing))
        if e_cat1 and p_cat1 and e_cat1 != p_cat1:
            return False
        if len(products) >= 2 and e_cat2 and p_cat2 and e_cat2 != p_cat2:
            return False
...
def can_place(
    ...
    if not fixture_compatible(p, module, shelf):
        return False, "fixture_not_fit"

    if not shelf_group_compatible(p, shelf):
        return False, "shelf_group_not_fit"
...
def place_product_fast(
    ...
    placed = {
        ...
        "storage_raw": p.get("storage_raw"),
        "fixture_kind": p.get("fixture_kind") or p.get("required_fixture_kind") or fixture_kind(p),
        "required_fixture_kind": p.get("required_fixture_kind") or fixture_kind(p),
...
def validate_planogram(plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for module in aisle.get("modules", []):
            ...
            for shelf in module.get("shelves", []):
                ...
                base = {
                    ...
                    "module_fixture_kind": module_fixture_kind(module, shelf),
                }
                ...
                for p in shelf.get("products", []):
                    shelf_groups.add(str(p.get("merch_group") or merch_group(p)))
                    if category_l1(p):
                        shelf_cat1.add(norm(category_l1(p)))
                    if category_l2(p):
                        shelf_cat2.add(norm(category_l2(p)))
                    ...
                    if not fixture_compatible(p, module, shelf):
                        violations.append({**base, "type": "fixture_violation", "sku": p.get("sku"), "product_name": p.get("product_name"), "required_fixture_kind": fixture_kind(p)})
                ...
                if shelf.get("products"):
                    has_food = any(g.startswith("FOOD") for g in shelf_groups)
                    has_non_food = any(g.startswith("NON_FOOD") for g in shelf_groups)
                    if has_food and has_non_food:
                        mixed_shelves.append({**base, "type": "food_nonfood_mixed", "groups": sorted(shelf_groups)})
                    if len(shelf_cat1) > 1:
                        mixed_shelves.append({**base, "type": "category_l1_mixed", "categories": sorted(shelf_cat1)})
                    if len(shelf.get("products", [])) >= 3 and len(shelf_cat2) > 1:
                        mixed_shelves.append({**base, "type": "category_l2_mixed", "categories": sorted(shelf_cat2)})

    violations.extend(mixed_shelves)
    ...
    return {
        ...
        "summary": {
            ...
            "mixed_shelf_count": len(mixed_shelves),
        },
    }
