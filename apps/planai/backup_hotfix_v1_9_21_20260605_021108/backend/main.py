...
def read_products_dataframe(content: bytes) -> pd.DataFrame:
    """Read comma/semicolon/tab CSV files without forcing the user to know delimiter details."""
    attempts = [
...
async def upload_products_csv(
    ...
    """Upload SKU catalog, enrich it, and make it visible in Product Library.

    Previous behavior only returned enriched rows to the caller. Product Library reads
    data/master_products.csv through /master-products, so the uploaded SKUs disappeared
    after upload. This endpoint now persists by default and reloads the master cache.
    """
    content = await file.read()
    df = read_products_dataframe(content)
    ...
    products = [enrich_product(r, allow_ai_dimensions=allow_ai_dimensions) for r in rows]
