...
def fitted_facing(p: Dict[str, Any], shelf: Dict[str, Any]) -> int:
    """Return the best feasible facing count, degrading gracefully to one.

    The previous engine rejected a SKU when its *preferred* facing did not fit,
    even when a single facing fitted perfectly. That was the main source of
    artificial empty shelves and avoidable unplaced products.
    """
    preferred = preferred_facing(p, shelf)
    effective_width = depth(p) if p.get("is_rotated") else width(p)
    remaining_width = max(
        0.0,
        num(shelf.get("shelf_width_cm"), 100)
        - num(shelf.get("used_width_cm", shelf.get("used", 0)), 0),
    )
    width_limit = int(remaining_width // max(effective_width * 1.1, 0.01))
    ...
    weight_limit = int(remaining_weight // max(weight(p), 0.01))
    return max(0, min(preferred, width_limit, weight_limit))
...
def used_width_for_facing(p: Dict[str, Any], facing: int) -> float:
    effective_width = depth(p) if p.get("is_rotated") else width(p)
    return effective_width * max(0, facing) * 1.1
...
def _rule_values(value: Any) -> List[str]:
    ...
    if not raw:
        return []
    return [norm(x) for x in re.split(r"[|;,]", raw) if norm(x)]


def category_rule_matches(p: Dict[str, Any], shelf: Dict[str, Any]) -> bool:
    """Apply the category fields that layouts already store but v1 ignored."""
    c1 = norm(category_l1(p))
    c2 = norm(category_l2(p))
    haystack = f"{c1} {c2}"

    allowed = _rule_values(shelf.get("allowed_categories"))
    blocked = _rule_values(shelf.get("blocked_categories"))

    if allowed and not any(x in haystack for x in allowed):
        return False
    if blocked and any(x in haystack for x in blocked):
        return False
    return True
...
def placement_score(
    ...
    # Consolidate before opening a new shelf. This turns the allocator into a
    # best-fit packer instead of spreading every next SKU over the emptiest row.
    existing = shelf.get("products", [])
    if existing:
        score += 260
        same_l2 = sum(norm(x.get("category_l2")) == norm(category_l2(p)) for x in existing)
        same_l1 = sum(norm(x.get("category_l1")) == norm(category_l1(p)) for x in existing)
        score += same_l2 * 150 + same_l1 * 70
    else:
        score -= 90
...
def weight_fit(p: Dict[str, Any], shelf: Dict[str, Any]) -> bool:
    return fitted_facing(p, shelf) >= 1
...
def can_place(
    ...
    if not category_rule_matches(p, shelf):
        return False, "category_rule_not_match"
...
def place_product_fast(
    ...
    # Never truncate to the 40 emptiest shelves. That old shortcut discarded
    # valid rule-bound candidates and systematically produced sparse layouts.
    candidates = sorted(
        candidates,
        key=lambda x: (
            0 if x["shelf"].get("products") else 1,
    ...
    f = fitted_facing(p, shelf)
    u = used_width_for_facing(p, f)
        ...
        for module in aisle.get("modules", []):
            ...
            for shelf in module.get("shelves", []):
                ...
                for p in shelf.get("products", []):
                    ...
                    if not category_rule_matches(p, shelf):
                        violations.append({**base, "type": "category_rule_violation", "sku": p.get("sku"), "product_name": p.get("product_name")})

                    if not dimension_fit(p, shelf):
                        violations.append({**base, "type": "dimension_violation", "sku": p.get("sku"), "product_name": p.get("product_name")})

                    if su > sw + 0.01:
                        violations.append({**base, "type": "capacity_violation", "sku": p.get("sku"), "product_name": p.get("product_name")})
