...
def search_products(q: str = "", storage: str = "", limit: int = Query(80, ge=1, le=500)):
    ...
    def ok(r):
        hay = f"{r.get('sku','')} {r.get('barcode','') or r.get('product_barcodes','')} {r.get('product_name','')} {r.get('brand','')} {r.get('category_l1','')} {r.get('category_l2','')} {r.get('storage_raw','')}".lower()
