    ...
    <div className="page">
      ...
      <div className="live-layout" style={{ marginTop: 20 }}>
        <div ref={stageRef} className={`live-stage-wrap ${isFull ? 'is-fullscreen' : ''}`}>
          <div className="stage three-stage">
            <div className="stage-toolbar">
              ...
              <div className="tabs">{['overview','top','chilled','frozen','dispatch'].map((c) => <button key={c} className={`tab ${camera===c?'active':''}`} onClick={() => chooseCamera(c)}>{cameraLabel(lang,c)}</button>)}</div>
        ...
        <aside className="side-panel">
          ...
          <div className="card pad"><div className="section-eyebrow">{tt(lang,'camera')}</div><div className="camera-grid" style={{ marginTop: 14 }}>{['overview','top','chilled','frozen','dispatch'].map((c) => <button key={c} className={`btn small ${camera===c?'active':''}`} onClick={() => chooseCamera(c)}>{cameraLabel(lang,c)}</button>)}</div></div>
