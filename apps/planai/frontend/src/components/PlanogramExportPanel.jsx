...
function rowsFromProducts(products = []) {
  return (products || []).map((p) => {
    ...
    return {
      ...
      family: p.food_family || p.merch_group || "-",
      facing: p.facing || p.facing_count || "-",
      depth: p.depth_units || p.depth || "-",
      reason: p.placement_reason || p.reason || "-",
      img: productImage(p),

      brand_block_id: p.brand_block_id || "",
      brand_block_name: p.brand_block_name || "",
      brand_block_sequence: p.brand_block_sequence ?? "",
      brand_block_split: p.brand_block_split === true ? "true" : "false",
      brand_block_warning: p.brand_block_warning || "",
      placed_units: p.placed_units ?? p.total_capacity_units ?? "",
      facing_count: p.facing_count ?? p.facing ?? "",
      depth_units: p.depth_units ?? "",
      capacity_compromised: p.capacity_compromised === true ? "true" : "false",
      refill_risk: p.refill_risk || "",
      capacity_warning: p.capacity_warning || "",
      case_pack_min_fit: p.case_pack_min_fit === true ? "true" : "",
      placement_confidence: p.placement_confidence ?? "",
      confidence_level: p.confidence_level || "",
      engine_source: p.placement_source || "",
      storage_violation: p.storage_violation === true ? "STORAGE_VIOLATION" : ""
    };
  });
}
...
function downloadCsv(rows = []) {
  ...
  const csv = [
    ...
    ...sorted.map((r) => [
      ...
      r.brand_block_id,
      r.brand_block_name,
      r.brand_block_sequence,
      r.brand_block_split,
      r.brand_block_warning,
      r.placed_units,
      r.facing_count,
      r.depth_units,
      r.capacity_compromised,
      r.refill_risk,
      r.capacity_warning,
      r.case_pack_min_fit,
      r.placement_confidence,
      r.confidence_level,
      r.engine_source,
      r.storage_violation
    ].map(csvEscape).join(","))
