...
export default function ProductPlacementStudio({ lang, objects, products, setProducts, unplacedProducts = [], notify, backendPlan = null }) {
  ...
  const shelfCount = Math.max(1, Math.ceil(Number(selectedArea?.shelves || moduleCount * 5) / moduleCount));
      ...
      <div className="grid cols-2" style={{ marginTop: 22 }}>
        <section className="card pad">
          ...
          {Array.from({ length: shelfCount }, (_, i) => i + 1).map((s) => {
            const found = findBackendShelf(backendPlan, aisle, moduleNo, s); const backendShelf = found?.shelf || null; const ps = (backendShelf?.products || []).slice().sort((a,b) => Number(a.position_order||a.position || 0) - Number(b.position_order||b.position || 0));
            return <button className="shelf" key={s} onClick={() => setModalShelf({ title: `Koridor ${selectedArea?.label || aisle} / Modül ${moduleNo} / Raf ${s}`, aisle, aisle_id: aisle, moduleNo, module_id: found?.module?.module_id ?? moduleNo, shelfNo: s, shelf_no: s, moduleId: `${aisle}.${moduleNo}`, shelf: backendShelf, shelf_width_cm: backendShelf?.shelf_width_cm ?? 100, shelf_depth_cm: backendShelf?.shelf_depth_cm ?? 50, products: ps })} style={{ width: '100%' }}>
