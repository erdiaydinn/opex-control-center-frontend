...
function modalChemicalProduct(p) {
  const raw = _productText(p);
  return (
    raw.includes('CIF') ||
    raw.includes('C?F') ||
    raw.includes('DOMESTOS') ||
    raw.includes('OMO') ||
    raw.includes('PRIL') ||
    raw.includes('DETERJAN') ||
    raw.includes('DETERGENT') ||
    raw.includes('BLEACH') ||
...
function modalPetProduct(p) {
  const raw = _productText(p);
  return (
    raw.includes('CAT TREAT') ||
    raw.includes('CAT FOOD') ||
    raw.includes('DOG FOOD') ||
    raw.includes('WANPY') ||
    raw.includes('DREAMIES') ||
    raw.includes('KEDI') ||
    raw.includes('KED?') ||
    raw.includes('MAMA')
  );
...
function modalBulkyProduct(p) {
  const raw = _productText(p);
  return (
    raw.includes('CARBOY') ||
    raw.includes('DAMACANA') ||
    raw.includes('19 L') ||
    raw.includes('19L') ||
    raw.includes('12 X') ||
    raw.includes('6 X') ||
    raw.includes('5 L') ||
    raw.includes('5L') ||
    raw.includes('TOILET PAPER') ||
    raw.includes('PAPER TOWEL') ||
    raw.includes('GARBAGE BAG') ||
    raw.includes('KOROPLAST')
  );
}

function modalPaperProduct(p) {
  const raw = _productText(p);
  return (
    raw.includes('TOILET PAPER') ||
    raw.includes('PAPER TOWEL') ||
...
function modalFoodFamily(p) {
  if (isProduceProduct(p)) return 'PRODUCE';
  if (modalChemicalProduct(p)) return 'CHEMICAL';
  if (modalPetProduct(p)) return 'PET';
  if (modalBulkyProduct(p)) return 'BULKY';
  if (modalPaperProduct(p)) return 'PAPER';
  return 'FOOD';
}

function modalFamiliesCompatible(a, b) {
  if (!a || !b) return true;
  if (a === b) return true;
...
function canAssignProductToShelf(product, shelf, existingProducts = []) {
  ...
  if (shelfDomain === 'PRODUCE') {
    if (!(produce && productStorage === 'AMBIENT')) return false;
  } else if (shelfDomain === 'CHILLED') {
    if (productStorage !== 'CHILLED') return false;
  } else if (shelfDomain === 'FROZEN') {
    if (productStorage !== 'FROZEN') return false;
  } else {
    if (produce) return false;
    if (productStorage !== 'AMBIENT') return false;
  }

  const family = modalFoodFamily(product);
  const activeFamilies = (existingProducts || [])
    .map((p) => modalFoodFamily(p))
    .filter(Boolean);

  const existingFamily = activeFamilies[0];

  if (existingFamily && !modalFamiliesCompatible(family, existingFamily)) return false;
