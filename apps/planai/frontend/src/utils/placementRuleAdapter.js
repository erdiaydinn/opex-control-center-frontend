export const V1947_STRATEGY_ADAPTER_VERSION = 'V1.9.47';
...
// Taraf değerini her zaman kanonik ASCII forma indir: "SAĞ"/"SAG"/"R"/"RIGHT" -> "SAG", "SOL"/"L"/"LEFT" -> "SOL".
// Bu, allocator'daki scorer ile kural yazımının garanti eşleşmesini sağlar (mojibake kaynaklı eşleşmeme hatasını önler).
export function canonicalSide(value) {
  const v = normalizeRuleText(value); // normalizeRuleText Türkçe karakterleri doğru ASCII'ye çevirir
  if (["R", "RIGHT", "SAG"].includes(v)) return "SAG";
  if (["L", "LEFT", "SOL"].includes(v)) return "SOL";
  return "";
}

export function loadStrategyProfile(fallback = DEFAULT_STRATEGY_PROFILE) {
  try {
...
function applyManualRules(product, activeRules = []) {
  ...
  for (const rule of activeRules) {
    ...
    if (targetSide) next.preferred_side = canonicalSide(targetSide) || targetSide;
  ...
  const hydrated = (products || []).map((product, originalIndex) => {
    ...
    if (hb) {
      ...
      if (strategyProfile.mode === STRATEGY_MODES.HYBRID_BRAND_SALES) {
        ...
        next.preferred_side = canonicalSide(hb.target.side) || hb.target.side;
      }
    }
