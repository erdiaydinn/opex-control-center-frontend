...
function isPetProduct(p = {}) {
  return petProductV1919(p);
}

function isShelfStableCheese(p = {}) {
  return shelfStableCheeseV1919(p);
}

function strongChilledSignal(p = {}) {
  return strongChilledV1919(p);
}
function explicitStorageSafe(p = {}) {
  const raw = String(
    p.storage_raw ??
    p.storageRaw ??
    p["Storage Raw"] ??
  ...
  if (!u || u === "NO LOCATION" || u === "NULL" || u === "N/A" || u === "-") return "";
...
function upperText(p = {}) {
  if (p == null) return "";
  if (typeof p === "string") {
    const rawString = p.toLocaleUpperCase("tr-TR");
    const foldedString = rawString
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/Ä°/g, "I")
      .replace(/IÌ‡/g, "I");
    return `${rawString} ${foldedString}`;
  }
...
// Catalog/storage answers "where can it physically live"; safety_family answers
// "which products can share the same shelf/module". Brand/category rules can never override this.
function safetyTextV1948(p = {}) {
  return productTextV1919(p)
    .replaceAll("İ", "I")
    .replaceAll("Ğ", "G")
    .replaceAll("Ü", "U")
    .replaceAll("Ş", "S")
...
function safetyFamilyV1948(p = {}) {
  const h = safetyTextV1948(p);
  ...
  const chemical =
    ...
    h.includes("OIL REMOVER") ||
    h.includes("DIRT") ||
    h.includes("TOILET BLOCK") ||
    h.includes("DEGREASER") ||
    h.includes("SOAP") ||
    h.includes("SABUN") ||
    h.includes("SHAMPOO") ||
    h.includes("MOUTHWASH") ||
    h.includes("LISTERINE") ||
    h.includes("TOOTHPASTE") ||
    h.includes("SENSODYNE");
  ...
  const nonFoodGeneral =
    ...
    h.includes("PED") ||
    h.includes("DIAPER") ||
    h.includes("BABY SHAMPOO") ||
    h.includes("SHOPPING BAG") ||
    h.includes("CABLE") ||
    h.includes("TTEC") ||
    h.includes("CONDOM");

  if (nonFoodGeneral) return "NONFOOD_GENERAL";

  return "FOOD";
}

function safetyGroupV1948(family = "") {
  const f = String(family || "");
  return f === "FOOD" || f === "PRODUCE" ? "FOOD" : "NONFOOD";
}

function familyModuleCompatibleV1948(existingFamily, nextFamily) {
  if (!existingFamily || !nextFamily) return true;
  if (existingFamily === nextFamily) return true;

  const eg = safetyGroupV1948(existingFamily);
  const ng = safetyGroupV1948(nextFamily);

  // Absolute rule: food and non-food never share the same module.
  if (eg !== ng) return false;

  // Non-food subfamilies are also kept clean to avoid odor / hygiene ambiguity.
  if (eg === "NONFOOD") return false;

  return true;
}

function familyAdjacentCompatibleV1948(existingFamily, nextFamily) {
  if (!existingFamily || !nextFamily) return true;

  // Food/non-food cannot be side-by-side modules.
  if (safetyGroupV1948(existingFamily) !== safetyGroupV1948(nextFamily)) return false;

  // Chemical should not touch pet/general hygiene either.
  if (existingFamily === "CHEMICAL" || nextFamily === "CHEMICAL") {
    return existingFamily === nextFamily;
  }

  return true;
}

function moduleKeyV1948(slot = {}) {
  return `${String(slot.areaId || "")}__${Number(slot.module || 0)}`;
}

function neighborModuleKeysV1948(slot = {}) {
  const area = String(slot.areaId || "");
  const moduleNo = Number(slot.module || 0);
  if (!area || !Number.isFinite(moduleNo)) return [];
  return [`${area}__${moduleNo - 1}`, `${area}__${moduleNo + 1}`];
}

function canShareShelfFoodFamily(product = {}, slot = {}) {
  const family = safetyFamilyV1948(product);
  if (!slot.safetyFamily) return true;
  return familyModuleCompatibleV1948(slot.safetyFamily, family);
}

function canPlaceSafetyFamilyV1948(product = {}, slot = {}, moduleFamilyLocks = new Map()) {
  const family = safetyFamilyV1948(product);
  const moduleKey = moduleKeyV1948(slot);
  const locked = moduleFamilyLocks.get(moduleKey);

  if (locked && !familyModuleCompatibleV1948(locked, family)) return false;

  for (const key of neighborModuleKeysV1948(slot)) {
    const neighborFamily = moduleFamilyLocks.get(key);
    if (neighborFamily && !familyAdjacentCompatibleV1948(neighborFamily, family)) {
      return false;
    }
  }
...
// Backward compatible alias used by older UI/export code.
function productFoodFamily(p = {}) {
  return safetyGroupV1948(safetyFamilyV1948(p));
}
...
export function productDomain(p = {}) {
  ...
  const h = upperText(p);
  const explicit = explicitStorageSafe(p);
...
function targetAreas(p, objects) {
  ...
  // V1.9.47: Manual brand/category rules are real placement intent.
  // Example: Ülker -> A / SAĞ must enter the candidate target list before ABC/sales heuristics.
  const manualPreferred = String(p.preferred_aisle || p.brand_block_target_aisle || '').trim();
  if (manualPreferred && has(manualPreferred)) {
  ...
  const safetyFamily = safetyFamilyV1948(p);
  if (safetyFamily === 'CHEMICAL') return ['I', 'H', 'STEEL_RACK', 'G', 'F'].filter(has);
  if (safetyFamily === 'PET') return ['H', 'I', 'STEEL_RACK', 'G', 'F'].filter(has);
  if (safetyGroupV1948(safetyFamily) === 'NONFOOD') return ['I', 'H', 'STEEL_RACK', 'G', 'F'].filter(has);
...
function normalizeHybridSide(value = "") {
  // Tüm olası girdileri (SAĞ / SAG / R / RIGHT / SOL / L / LEFT, büyük-küçük harf,
  // Türkçe karakterli veya ASCII) tek kanonik forma indir: "SAG" veya "SOL".
  // Türkçe harfleri açıkça eşleyerek mojibake/encoding kaynaklı eşleşmeme hatasını önler.
  const v = String(value || "")
    ...
    .replaceAll("Ç", "C")
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, ""); // kalan diyakritikleri at

  if (["R", "RIGHT", "SAG"].includes(v)) return "SAG";
...
function buildHybridSideByModule(slots = []) {
  ...
  for (const [aisle, set] of byAisle.entries()) {
    ...
    modules.forEach((moduleNo, idx) => {
      const side = idx < midpoint ? "SOL" : "SAG";
...
export function buildStorePlan(inputProducts = [], objects = [], options = {}) {
  ...
  const moduleFamilyLocks = new Map();
  const placed = [];
  const unplaced = [];

  const hasStrategyOrder = normalized.some((p) => p.strategy_mode || p.strategy_label);

  const sorted = [...normalized].sort((a, b) => {
    const domainOrder = { FROZEN: 0, CHILLED: 1, PRODUCE: 2, AMBIENT: 3 };
    const domainDiff = (domainOrder[productDomain(a)] ?? 9) - (domainOrder[productDomain(b)] ?? 9);
    if (domainDiff !== 0) return domainDiff;

    // V1.9.47: If products passed through placementRuleAdapter, their original_input_index is already
    // strategy-sorted (category->sales, ABC, hybrid brand, etc.). Do not destroy that order here.
    if (hasStrategyOrder) {
      return (
        Number(a.original_input_index ?? 999999) - Number(b.original_input_index ?? 999999) ||
  ...
  for (const p of sorted) {
    ...
    let candidates = slots
      .filter((s) => {
        ...
        return inTarget && isCompatible(p, s) && canShareShelfFoodFamily(p, s) && canPlaceSafetyFamilyV1948(p, s, moduleFamilyLocks) && s.count < maxSkuCount(s);
      });

    if (!candidates.length) {
      candidates = slots.filter((s) => isCompatible(p, s) && canShareShelfFoodFamily(p, s) && canPlaceSafetyFamilyV1948(p, s, moduleFamilyLocks) && s.count < maxSkuCount(s));
    }
    ...
    slot.used += required;
    slot.count += 1;
    slot.safetyFamily = slot.safetyFamily || safetyFamily;
    slot.foodFamily = slot.foodFamily || safetyGroupV1948(safetyFamily);
    moduleFamilyLocks.set(moduleKey, moduleFamilyLocks.get(moduleKey) || safetyFamily);
    ...
    placed.push({
      ...
      food_family: safetyGroupV1948(safetyFamily),
