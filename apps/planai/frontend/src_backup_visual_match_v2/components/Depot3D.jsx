...
function LayoutObject3D({ obj }) {
  ...
  const color = isWall ? "#020617" : isColumn ? "#475569" : isDispatch ? "#111827" : "#cbd5e1";
...
function Scene({ plan, selectedAisle, onShelfOpen, onModuleSelect, cameraPreset, routeMode }) {
  ...
  return (
    <>
      ...
      <hemisphereLight intensity={0.32} groundColor="#020617" />
      <directionalLight position={[15, 22, 10]} intensity={0.82} castShadow shadow-mapSize-width={2048} shadow-mapSize-height={2048} />
      <pointLight position={[-8, 4, 8]} color="#ff0f7b" intensity={0.52} distance={18} />
      <pointLight position={[8, 4, -4]} color="#22d3ee" intensity={0.42} distance={20} />

      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.035, 0]} receiveShadow>
        <planeGeometry args={[70, 70]} />
        <meshStandardMaterial color="#0b111d" roughness={0.28} metalness={0.74} />
      </mesh>

      <gridHelper args={[70, 70, "#164e63", "#1e293b"]} position={[0, 0.006, 0]} />

      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.009, 0]}>
        <planeGeometry args={[30, 18]} />
        <meshBasicMaterial color="#ff0f7b" transparent opacity={0.018} />
      </mesh>
