...
import PremiumLoading from "./components/loading/PremiumLoading";
import React, { useEffect, useMemo, useRef, useState } from "react";
import "./App.css";
import "./App.extra.css";
import "./App.premium.css";
import "./App.ultra.css";
import "./App.worldclass.css";
...
function handleLayoutChange(nextPositions = [], nextObjects = null) {
  ...
  return (
    <I18nProvider>
      <div className="plw-shell" data-view={view}>
        {(bootLoading || generating) && <PremiumLoading mode={generating ? "optimize" : "boot"} />}

        <aside className="plw-sidebar" aria-label="Plonagram navigation">
          <div className="plw-logo-lockup">
            <div className="plw-mark">P</div>
            <div className="plw-wordmark">P L O N A G R A M</div>
            <small>OS</small>
          </div>

          <nav className="plw-nav">
            <button className={view === "3D" ? "active" : ""} onClick={() => setView("3D")}><span>⌂</span>Komuta</button>
            <button className={view === "3D" ? "active" : ""} onClick={() => setView("3D")}><span>⬡</span>Canlı 3D</button>
            <button className={view === "2D" ? "active" : ""} onClick={() => setView("2D")}><span>▦</span>2D Plan</button>
            <button className={view === "ANALYTICS" ? "active" : ""} onClick={() => setView("ANALYTICS")}><span>⌁</span>Analytics</button>
            <button onClick={() => setStatus("Kurallar paneli aşağıda açık. Planogram motoru canlı.")}><span>✧</span>Kurallar</button>
            <button onClick={exportJSON}><span>☰</span>Raporlar</button>
          </nav>

          <div className="plw-user-card">
            <div className="plw-avatar-mini">EA</div>
            <div><b>{user?.username || "Erdi"}</b><small>{user?.role || "Admin"}</small></div>
          </div>
          <button className="plw-logout" onClick={logout}>Oturumu kapat</button>
        </aside>

        <header className="plw-topbar">
          <div className="plw-top-title">
            <span>PLONAGRAM OS · COMMAND LAYER</span>
            <strong>{view === "3D" ? "Canlı Dijital İkiz" : view === "2D" ? "2D Planogram" : "Operasyon Analitiği"}</strong>
            <small>{status}</small>
          </div>

          <div className="plw-top-actions">
            <div className="plw-system"><i /> Sistem Durumu <b>Çevrimiçi</b></div>
            <select value={storeCode} onChange={(e) => handleStoreChange(e.target.value)}>
              {(storeList.length ? storeList : [{ store_code: storeCode, store_name: storeCode }]).map((s) => (
                <option key={s.store_code} value={s.store_code}>{s.store_name || s.display_name || s.store_code}</option>
              ))}
            </select>
            <LanguageSelector compact embedded />
            <button onClick={() => productInputRef.current?.click()}>SKU yükle</button>
            <button onClick={() => layoutInputRef.current?.click()}>Layout yükle</button>
            <button className="plw-primary" onClick={generatePlanogram} disabled={generating}>✦ Optimum plan üret</button>
        ...
        <main className="plw-main">
          {view === "3D" && (
            <>
              <section className="plw-hero-panel">
                <div>
                  <span className="plw-kicker">CANLI 3D</span>
                  <h1>Canlı Dijital İkiz</h1>
                  <p>Depo operasyonlarınızın gerçek zamanlı, okunabilir ve müdahale edilebilir 3D görünümü.</p>
                  <div className="plw-hero-meta">
                    <span>✓ Gerçek zamanlı güncellemeler</span>
                    <span>✦ AI destekli içgörüler</span>
                    <span>⚠ Kritik alanlar anlık izleme</span>
                    <span>↗ Akıllı yönlendirme</span>
                  </div>
                </div>
                <div className="plw-view-tabs">
                  <button className="active" onClick={() => setView("3D")}>3D View</button>
                  <button onClick={() => setView("2D")}>2D Plan</button>
                  <button onClick={() => setView("ANALYTICS")}>Heatmap</button>
                </div>
              </section>

              <section className="plw-live-grid">
                <div className="plw-live-stage">
                  <Depot3D
                    plan={planogram}
              ...
              <section className="plw-kpi-row">
                <article><span>Planogram Skoru</span><b>92</b><small>↑ 8 vs last 7 days</small></article>
                <article><span>Doluluk</span><b>{Math.round(metrics?.capacity_utilization_pct || 76)}%</b><small>↑ 6% vs last 7 days</small></article>
                <article><span>SKU</span><b>{products.length.toLocaleString("tr-TR")}</b><small>Aktif SKU</small></article>
                <article><span>Refill Riski</span><b>Orta</b><small>23 SKU için stok yenileme</small></article>
                <article><span>Picker Rotası</span><b>1.35 dk</b><small>Ortalama sipariş</small></article>
                <article><span>Cold Chain</span><b>Sağlıklı</b><small>Tüm bölgeler normal</small></article>
              </section>
            </>
          )}

          {view === "2D" && (
            <section className="plw-plan-page">
              <div className="plw-page-head">
                <div><span className="plw-kicker">2D PLANOGRAM</span><h1>Raf raf operasyon planı</h1><p>Koridor, modül, raf ve ürün yerleşimini okunabilir biçimde yönet.</p></div>
                <div className="plw-view-tabs"><button onClick={() => setView("3D")}>3D View</button><button className="active">2D Plan</button><button onClick={() => setView("ANALYTICS")}>Heatmap</button></div>
              </div>
              <Planogram2D plan={planogram} onShelfOpen={setSelectedShelf} onAddModule={addModule} onAddShelf={addShelf} onModuleSize={openModuleSize} onShelfSize={openShelfSize} onRule={openRule} onPrintModule={printModule} onAddAisle={addAisle} onDeleteAisle={deleteAisle} onDeleteModule={deleteModule} onDeleteShelf={deleteShelf} />
            </section>
          )}

          {view === "ANALYTICS" && (
            <section className="plw-plan-page">
              <div className="plw-page-head"><div><span className="plw-kicker">ANALYTICS</span><h1>Operasyon analitiği</h1><p>Alan, hacim, doluluk, fixture ve cold chain metrikleri.</p></div></div>
              <AnalyticsPanel metrics={metrics} logs={logs} onExport={exportJSON} />
            </section>
          )}

          <details className="plw-rule-drawer">
            <summary>AI Planogram Motoru ve Kural Setleri</summary>
