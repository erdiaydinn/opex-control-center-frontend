...
from app.modules.dockos.identity import (
    authenticate_request as authenticate_dockos_request,
    reset_identity as reset_dockos_identity,
    set_identity as set_dockos_identity,
    verify_gateway as verify_dockos_gateway,
)
from app.modules.dockos.observability import (
    record_reservation,
    render_prometheus as render_dockos_prometheus,
    snapshot as dockos_snapshot,
)
from app.modules.dockos.persistence import (
    persistence_mode as dockos_persistence_mode,
    refresh_state as refresh_dockos_state,
)
from app.modules.dockos.production_runtime import (
    install_service_security as install_dockos_service_security,
    readiness_checks as dockos_readiness_checks,
    sync_bigquery_purchase_orders as sync_dockos_bigquery_purchase_orders,
)
...
if DOCKOS_PRODUCTION:
    # The service may inspect the legacy function arguments, but authority is the
    # request-scoped identity established only after HMAC gateway + OIDC checks.
    install_dockos_service_security(dockos_service)
    dockos_router_module.is_admin = dockos_service.is_admin

    def _production_live_pos(
        supplier_name=None,
        warehouse_name=None,
        user_email=None,
        user_role=None,
        ...
        return sync_dockos_bigquery_purchase_orders(
            dockos_service,
            supplier_name,
            warehouse_name,
        )
...
def _replace_verified_dockos_headers(request: Request, email: str, role: str) -> None:
    """Remove spoofable client identity and inject only verified server identity.
    ...
    headers.extend(
        [
            (b"x-opex-user", email.encode("utf-8")),
            (b"x-opex-role", role.encode("utf-8")),
            (b"x-dockos-gateway", os.environ["DOCKOS_GATEWAY_SECRET"].encode("utf-8")),
        ]
    )
...
app = FastAPI(
    title="EAY Platform API",
    version="26.6.0-converged",
    docs_url="/api/docs" if not DOCKOS_PRODUCTION else None,
    redoc_url=None,
    openapi_url="/api/openapi.json" if not DOCKOS_PRODUCTION else None,
    lifespan=lifespan,
)
...
async def dockos_production_boundary(request: Request, call_next):
            ...
            return JSONResponse(
                {
                    ...
                    "release": "EAY-converged-production-candidate",
                    "checks": checks,
                    "slo": slo,
                },
    ...
    try:
        if DOCKOS_PRODUCTION and protected:
            # Verify the original request before replacing any legacy route headers.
...
def dockos_metrics():
    return PlainTextResponse(
        render_dockos_prometheus(dockos_snapshot(dockos_service, _dockos_pool_stats())),
...
def root_health():
    return {
        "status": "ok",
        "service": "eay-platform-backend",
        "release": "convergence-v0.1",
        "dockos_persistence": dockos_persistence_mode(),
    }
