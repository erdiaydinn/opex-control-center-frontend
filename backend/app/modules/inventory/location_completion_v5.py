"""Inventory v5 location completion bound to the exact mission attempt and lease."""
...
from .mission_attempt import complete_mission_attempt, verify_event_lease
...
def location_completion_hash_input_v5(payload: dict[str, Any]) -> dict[str, Any]:
    return {
...
def record_location_completion_v5(
    ...
    active_shift_id = str(payload["active_shift_id"]).strip()
    confirmed_line_count = int(payload["confirmed_line_count"])
    actual_hash = canonical_payload_hash(location_completion_hash_input_v5(payload))
        ...
        try:
            ...
            if existing:
                if existing["payload_hash"] != actual_hash:
                    raise InventoryRuleError("Event ID farklı completion payload ile yeniden kullanılamaz.")
            ...
            try:
                ...
                raise RuntimeError("Workforce completion vardiya authority kullanılamıyor.") from error
            if shift is None:
                raise PermissionError("Completion aktif vardiya penceresi dışında üretildi.")
            ...
            if allowed["completed_event_id"] is not None:
            ...
            committed = db.execute(
                """SELECT count(*)::integer AS committed_line_count
                   ...
                   WHERE tenant_id=%s AND document_id=%s AND location_id=%s
                     ...
                     AND occurred_at<=%s""",
                (principal.tenant_id, document_id, location, attempt_id, occurred_at),
            ).fetchone()
            committed_line_count = int(committed["committed_line_count"])
            if committed_line_count != confirmed_line_count:
                raise InventoryRuleError(
                    "Completion satır sayısı exact mission-attempt committed kanıtıyla eşleşmiyor."
                )
            ...
            _audit(db, principal, "LOCATION_COUNT_COMPLETED", document_id, document["warehouse_id"], response)
            db.execute(
