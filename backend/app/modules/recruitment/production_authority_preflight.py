...
def run_live_preflight() -> dict:
    ...
    database_ok = bool(persistence.ENABLED and (schema or 0) >= 43)
    checks.append(
        {
            "key": "postgres_v43",
            "required": True,
    ...
    if not database_ok:
        raise ProductionAuthorityPreflightError("PostgreSQL V43 authority hazır değil.")

    try:
        storage = S3KmsEnvelopeEvidenceStore.from_environment()
        storage_result = storage.preflight()
        checks.append({"key": "aws_evidence_storage", "required": True, "ok": True, "detail": storage_result})
    except EvidenceStorageError as error:
        checks.append({"key": "aws_evidence_storage", "required": True, "ok": False, "detail": str(error)})
    ...
    try:
        ...
        checks.append({"key": "aws_scanner_hmac", "required": True, "ok": True, "detail": scanner_result})
    except ScannerKeyAuthorityError as error:
        checks.append({"key": "aws_scanner_hmac", "required": True, "ok": False, "detail": str(error)})
        raise ProductionAuthorityPreflightError(str(error)) from error

    try:
        scanner_db_result = scanner_db_preflight()
        checks.append({"key": "scanner_postgres_role", "required": True, "ok": True, "detail": scanner_db_result})
    except ScannerDatabaseAuthorityError as error:
        checks.append({"key": "scanner_postgres_role", "required": True, "ok": False, "detail": str(error)})
        raise ProductionAuthorityPreflightError(str(error)) from error
    ...
    if m2m_state == "EXTERNAL_AGREEMENT_PENDING":
        ...
        raise ProductionAuthorityPreflightError(
            "Yetkili e-Devlet M2M kısmi yapılandırması reddedildi; anlaşma sonrası tam credential seti gerekir."
        )
    else:
        try:
            ...
            checks.append(
                {
                    ...
                    "ok": True,
                    "detail": {
                        "state": "CONFIGURED_TRANSPORT_PROVEN",
            ...
            checks.append({"key": "official_m2m_transport", "required": False, "ok": False, "detail": str(error)})
            raise ProductionAuthorityPreflightError(str(error)) from error
    ...
    return {
        ...
        "external_pending": [
            check["key"]
            for check in checks
            if check.get("required") is False and check.get("ok") is None
        ],
