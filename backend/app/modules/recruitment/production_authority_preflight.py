...
def run_live_preflight() -> dict:
    ...
    database_ok = bool(persistence.ENABLED and (schema or 0) >= 46)
    checks.append({"key": "postgres_v46", "required": True, "ok": database_ok, "detail": f"schema={schema or 0}"})
    if not database_ok:
        raise ProductionAuthorityPreflightError("PostgreSQL V46 authority hazır değil.")
