...
def record_verified_request_scan(
    ...
    *,
    ...
    required = persistence.SCHEMA_VERSION if os.getenv("DOCKOS_ENV", "development").strip().lower() == "production" else 41
    if not persistence.ENABLED or (persistence.schema_version() or 0) < required:
        raise RequestEvidenceScanAuthorityError(
            "PostgreSQL request scanner receipt otoritesi V41 olmadan hazır değil."
        )
    try:
    ...
    with persistence.connection() as database, database.cursor() as cursor:
        ...
        try:
            receipt = verify_scanner_receipt(
                ...
                # The generic scanner receipt subject slot binds the request ID.
