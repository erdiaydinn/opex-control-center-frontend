"""Governed employee shift trading on top of canonical Workforce shift authority.

The trade workflow never creates a second schedule. Employees may propose or
accept a transfer/swap, but canonical shift ownership changes only after manager
approval and a fresh revalidation of Employee Master, leave, availability,
capabilities, warehouse scope, daily maximum and between-shift rest rules.
"""
...
def _shift(shift_id: str) -> dict | None:
    return next((row for row in service._SHIFTS if str(row.get("id")) == str(shift_id)), None)
...
def _active_trade_for_shift(rows: list[dict], shift_id: str) -> dict | None:
    return next(
        (
            ...
            and str(row.get("shift_id")) == str(shift_id)
        ),
        None,
    )
...
def _assert_shift_tradeable(shift: dict, owner_id: str) -> None:
    if str(shift.get("person_id")) != str(owner_id):
        raise service.WorkforceRuleError("Yalnızca kendi vardiyanız için takas/transfer talebi oluşturabilirsiniz.")
    if str(shift.get("status")) != "Atandı":
        raise service.WorkforceRuleError("Yalnızca henüz başlamamış atanmış vardiya takas/transfer edilebilir.")
    if any(str(row.get("shift_id")) == str(shift.get("id")) for row in service._ATTENDANCE):
        raise service.WorkforceRuleError("Check-in veya puantaj kaydı oluşmuş vardiya takas/transfer edilemez.")
    ...
    if notice < int(_POLICY["minimum_notice_minutes"]):
        raise service.WorkforceRuleError("Vardiya takası için minimum bildirim süresi doldu.")
...
def _evaluate_assignment(
    ...
    *,
    ...
    """Revalidate one proposed canonical assignment without mutating schedule."""
    ...
    if required_skills - _capability_keys(person, "skill_keys"):
        reasons.append("SKILL_REQUIREMENT")
    if required_certifications - _capability_keys(person, "certification_keys"):
        reasons.append("CERTIFICATION_REQUIREMENT")
    if required_equipment - _capability_keys(person, "equipment_keys"):
        reasons.append("EQUIPMENT_REQUIREMENT")

    availability = _availability_for(_load_availability(), str(person_id), str(shift["date"]))
    ...
    if service._day_context(str(person_id), str(shift["date"])).get("on_approved_leave"):
        reasons.append("APPROVED_LEAVE")

    existing = [
        row for row in service.list_shifts(str(person_id))
        if row.get("status") != "İptal" and str(row.get("id")) not in ignored
    ]
    ...
    for current in existing:
        ...
        if candidate_start >= current_end:
            ...
            gap = -1
        if gap < service._rule_value("betweenShifts", str(shift["date"]), 660):
...
def _revalidate_trade(trade: dict) -> dict:
    ...
    if not target_person_id:
        raise service.WorkforceRuleError("Takas/transfer için kabul eden personel bulunamadı.")

    if trade["mode"] == "TRANSFER":
        result = _evaluate_assignment(source, target_person_id)
        if not result["eligible"]:
            raise service.WorkforceRuleError(
                "Transfer, hedef personelin güncel uygunluk/çalışma/izin/yetkinlik/dinlenme kurallarıyla eşleşmiyor."
            )
    ...
    if str(source.get("warehouse_id")) != str(target.get("warehouse_id")):
        raise service.WorkforceRuleError("V1 vardiya takası farklı çalışma konumları arasında yapılamaz.")
    target_eval = _evaluate_assignment(source, target_person_id, ignored_shift_ids={str(target["id"])})
    ...
    if not target_eval["eligible"] or not requester_eval["eligible"]:
        raise service.WorkforceRuleError(
            "Takas, iki çalışanın güncel uygunluk/çalışma/izin/yetkinlik/dinlenme kurallarıyla eşleşmiyor."
        )
    return {
        "source": source,
        "target": target,
...
def create_shift_trade(payload: dict, actor: str) -> dict:
    ...
    with _LOCK:
        ...
        if _active_trade_for_shift(rows, str(source["id"])):
            raise service.WorkforceRuleError("Bu vardiya için zaten açık bir takas/transfer talebi var.")
        ...
        if mode == "SWAP":
            ...
            if not target_person_id or target_person_id == requester:
                raise service.WorkforceRuleError("Takas için farklı bir çalışan vardiyası gerekir.")
            _assert_shift_tradeable(target, target_person_id)
            if _active_trade_for_shift(rows, target_shift_id):
                raise service.WorkforceRuleError("Karşı vardiya başka bir açık takas/transfer talebinde kullanılıyor.")
            if str(source.get("warehouse_id")) != str(target.get("warehouse_id")):
                raise service.WorkforceRuleError("V1 vardiya takası farklı çalışma konumları arasında yapılamaz.")
        ...
        if target_person_id:
            person = service.resolve_person_identity(target_person_id, "EMPLOYEE_ID")
            if person is None:
                raise service.WorkforceRuleError("Hedef Employee Master kaydı bulunamadı.")
        ...
        record = {
            ...
            "status": "PENDING_EMPLOYEE_ACCEPTANCE" if target_person_id else "OPEN_FOR_ACCEPTANCE",
...
def list_shift_trades_for_person(person_id: str) -> list[dict]:
    ...
    for trade in _load_trades():
        if trade.get("status") in _FINAL_STATES:
            if str(trade.get("requester_person_id")) == str(person_id) or str(trade.get("target_person_id")) == str(person_id):
                rows.append(trade)
            continue
        if str(trade.get("requester_person_id")) == str(person_id) or str(trade.get("target_person_id")) == str(person_id):
            rows.append(trade)
            continue
        if trade.get("status") == "OPEN_FOR_ACCEPTANCE" and trade.get("mode") == "TRANSFER":
            source = _shift(str(trade["shift_id"]))
            if source and _evaluate_assignment(source, str(person_id))["eligible"]:
                rows.append(trade)
    return deepcopy(sorted(rows, key=lambda row: (str(row.get("date")), str(row.get("created_at")))))
...
def accept_shift_trade(trade_id: str, person_id: str, actor: str) -> dict:
    with _LOCK:
        if persistence.ENABLED:
            service._hydrate_snapshot(persistence.load_snapshot(service._snapshot_kinds()))
        rows = _load_trades()
        trade = next((row for row in rows if str(row.get("id")) == str(trade_id)), None)
        if trade is None or trade.get("status") not in {"OPEN_FOR_ACCEPTANCE", "PENDING_EMPLOYEE_ACCEPTANCE"}:
            raise service.WorkforceRuleError("Takas/transfer talebi bulunamadı veya artık kabul edilemez.")
        if str(trade.get("requester_person_id")) == str(person_id):
            raise service.WorkforceRuleError("Talebi oluşturan çalışan kendi talebini kabul edemez.")
        bound_target = str(trade.get("target_person_id") or "")
        if bound_target and bound_target != str(person_id):
            raise service.WorkforceRuleError("Bu takas/transfer talebi başka bir çalışan için oluşturuldu.")
        if not bound_target:
...
def _replace_assignment(shift: dict, person_id: str, trade: dict, actor: str) -> None:
...
def _reschedule_shift_notifications(shifts: list[dict], actor: str, trade_id: str) -> list[str]:
    ...
    service._NOTIFICATIONS[:] = [
        row for row in service._NOTIFICATIONS if str(row.get("shift_id")) not in shift_ids
    ]
...
def approve_shift_trade(trade_id: str, actor: str, note: str = "") -> dict:
    """Apply an accepted trade atomically with canonical shifts and notifications."""
    with _LOCK:
        if persistence.ENABLED:
            service._hydrate_snapshot(persistence.load_snapshot(service._snapshot_kinds()))
        rows = _load_trades()
        ...
        if trade is None or trade.get("status") != "PENDING_MANAGER_APPROVAL":
            raise service.WorkforceRuleError("Yönetici onayı bekleyen takas/transfer talebi bulunamadı.")
        ...
        try:
            ...
            cancelled_notification_ids = _reschedule_shift_notifications(affected, actor, trade_id)
            now = datetime.now(ZoneInfo("UTC")).isoformat()
            trade.update(
                {
                    "status": "APPROVED",
            ...
            if persistence.ENABLED:
                service._hydrate_snapshot(persistence.load_snapshot(service._snapshot_kinds()))
            raise service.WorkforceRuleError(
                "Takas onayı sırasında Workforce verisi değişti; işlem uygulanmadı, tekrar değerlendirin."
            ) from error
        except Exception:
            service._hydrate_snapshot(before)
            raise
...
def cancel_shift_trade(trade_id: str, person_id: str, actor: str) -> dict:
    with _LOCK:
        ...
        if trade is None or trade.get("status") in _FINAL_STATES:
            raise service.WorkforceRuleError("Aktif takas/transfer talebi bulunamadı.")
        if str(trade.get("requester_person_id")) != str(person_id):
            raise service.WorkforceRuleError("Yalnızca talebi oluşturan çalışan iptal edebilir.")
        trade.update(
            {
                "status": "CANCELLED",
