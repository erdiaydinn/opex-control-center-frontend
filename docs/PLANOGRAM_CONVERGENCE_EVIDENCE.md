...
The Planogram child is not eligible to compose back into the parent until the same exact head passes the permanent read-only `EAY Planogram Physical Truth Convergence` gate, including canonical physical-truth tests, compile, raw-master fail-closed smoke, platform-authority boundary guard and the existing EAY shell production build.
