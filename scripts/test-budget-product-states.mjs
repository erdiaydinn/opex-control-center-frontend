...
const required = [
  ...
  ['aria-busy={loading?"true":"false"}', "busy semantics"],
...
if (!source.includes("setData(EMPTY_DATA)") || !source.includes("Erişim veya servis durumu doğrulanmadan finansal değer gösterilmiyor")) {
  console.error(`${path}: failed Budget reads must clear stale decision data and preserve the truth boundary.`);
  process.exit(1);
}
