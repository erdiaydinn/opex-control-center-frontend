...
assert.match(mobile, /private_evidence_transport_unavailable/);
...
assert.match(offline, /managed-device policy requires authoritative device attestation/);
assert.match(offline, /camera-only policy requires authoritative capture attestation/);
...
console.log("Field Intelligence mobile/offline contract passed");
