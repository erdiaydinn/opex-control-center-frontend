"""Seed baseline system-role application permissions.

This migration is an immutable snapshot of the permission catalog.
Future permission changes require a new migration.

Revision ID: 0005_system_role_permissions
Revises: 0004_ai_reader_role
...
def downgrade() -> None:
    op.execute(
        ...
        USING roles AS r, desired
        WHERE rp.tenant_id = r.tenant_id
          AND rp.role_id = r.id
          AND r.is_system = true
          AND r.key = desired.role_key
          AND rp.permission_key = desired.permission_key
        """
    )
