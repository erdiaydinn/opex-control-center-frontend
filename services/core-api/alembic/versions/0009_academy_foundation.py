...
Revision ID: 0009_academy_foundation
...
revision: str = "0009_academy_foundation"
...
def upgrade() -> None:
    op.execute(
        ...
        CREATE TABLE academy_media_assets (
            ...
            CONSTRAINT uq_academy_media_delivery_key
        ...
        CREATE TABLE academy_learning_paths (
            ...
            created_by varchar(255) NOT NULL,
            created_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
            CONSTRAINT ck_academy_path_status
                CHECK (status IN ('draft', 'published', 'retired')),
        ...
        CREATE TABLE academy_path_role_assignments (
            ...
            CONSTRAINT fk_academy_path_role_path
                ...
                REFERENCES academy_learning_paths(tenant_id, id)
                ON DELETE CASCADE,
            CONSTRAINT ck_academy_role_due_days
        ...
        CREATE TABLE academy_enrollments (
            ...
            CONSTRAINT fk_academy_enrollment_path
                ...
                REFERENCES academy_learning_paths(tenant_id, id)
                ON DELETE RESTRICT,
            CONSTRAINT ck_academy_enrollment_source
                CHECK (source IN ('role', 'manual')),
            CONSTRAINT ck_academy_enrollment_status
                CHECK (status IN ('assigned', 'in_progress', 'completed', 'revoked')),
        ...
        CREATE TABLE academy_progress (
            ...
            CONSTRAINT fk_academy_progress_version
                ...
                REFERENCES academy_content_versions(tenant_id, id)
                ON DELETE RESTRICT,
            CONSTRAINT ck_academy_progress_status
                CHECK (status IN ('not_started', 'in_progress', 'completed')),
            CONSTRAINT ck_academy_progress_percent
                CHECK (progress_percent BETWEEN 0 AND 100),
            CONSTRAINT ck_academy_progress_positions
                CHECK (last_position_ms >= 0 AND max_position_ms >= 0 AND watched_ms >= 0),
        ...
        CREATE TABLE academy_quizzes (
            ...
            CONSTRAINT fk_academy_quiz_supersedes
                ...
                ON DELETE RESTRICT,
            CONSTRAINT ck_academy_quiz_kind
                CHECK (kind IN ('checkpoint', 'completion')),
            CONSTRAINT ck_academy_quiz_status
                CHECK (status IN ('draft', 'published', 'retired')),
        ...
        CREATE TABLE academy_questions (
            ...
            CONSTRAINT fk_academy_question_quiz
                ...
                REFERENCES academy_quizzes(tenant_id, id)
                ON DELETE CASCADE,
            CONSTRAINT ck_academy_question_type
        ...
        CREATE TABLE academy_quiz_attempts (
            ...
            CONSTRAINT uq_academy_quiz_attempt_number
                UNIQUE (tenant_id, enrollment_id, quiz_id, subject, attempt_number),
        ...
        CREATE TABLE academy_certificates (
            ...
            CONSTRAINT fk_academy_certificate_path
                ...
                REFERENCES academy_learning_paths(tenant_id, id)
                ON DELETE RESTRICT,
            CONSTRAINT ck_academy_completion_fingerprint
                CHECK (completion_fingerprint ~ '^[0-9a-f]{64}$'),
        ...
        CREATE TABLE academy_entitlements (
            ...
            created_by varchar(255) NOT NULL,
            created_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
            CONSTRAINT ck_academy_entitlement_resource
                CHECK (resource_type IN ('content', 'path')),
            CONSTRAINT ck_academy_entitlement_principal
                CHECK (principal_type IN ('role', 'subject')),
            CONSTRAINT ck_academy_entitlement_permission
                CHECK (permission IN ('view', 'learn', 'manage')),
            CONSTRAINT ck_academy_entitlement_window
                CHECK (ends_at IS NULL OR starts_at IS NULL OR ends_at > starts_at),
            CONSTRAINT uq_academy_entitlement
                UNIQUE (tenant_id, resource_type, resource_id, principal_type, principal_key, permission),
        ...
        CREATE TABLE academy_learning_events (
            ...
            created_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
            CONSTRAINT uq_academy_learning_event_idempotency
        ...
        CREATE TABLE academy_idempotency_keys (
            ...
            created_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
            CONSTRAINT ck_academy_request_fingerprint
                CHECK (request_fingerprint ~ '^[0-9a-f]{64}$'),
            CONSTRAINT uq_academy_idempotency
                UNIQUE (tenant_id, subject, operation, idempotency_key),
        ...
        CREATE INDEX ix_academy_content_tenant_status
            ON academy_content_items (tenant_id, status, content_type);
        CREATE INDEX ix_academy_content_version_tenant_status
            ON academy_content_versions (tenant_id, status, content_id);
        CREATE INDEX ix_academy_media_content_version
            ON academy_media_assets (tenant_id, content_version_id, transcode_status);
        CREATE INDEX ix_academy_enrollment_subject_status
            ON academy_enrollments (tenant_id, subject, status);
        CREATE INDEX ix_academy_progress_subject
            ON academy_progress (tenant_id, subject, updated_at DESC);
        CREATE INDEX ix_academy_quiz_content_version
            ON academy_quizzes (tenant_id, content_version_id, status, checkpoint_at_ms);
        CREATE INDEX ix_academy_learning_events_subject_created
            ON academy_learning_events (tenant_id, subject, created_at DESC);
        CREATE INDEX ix_academy_document_chunks_search
            ON academy_document_chunks USING gin (search_vector);
        """
    )
    ...
    op.execute(
        f"GRANT SELECT, INSERT ON TABLE academy_learning_events TO {RUNTIME_ROLE}"
    )
