...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "source_system IN ('MANUAL','ARIBA','SAP','BIGQUERY')", name="ck_budget_invoice_source"
        ),
        sa.CheckConstraint(
            "status IN ('POSTED','HOLD','REJECTED')", name="ck_budget_invoice_status"
        ),
        sa.CheckConstraint("base_amount > 0", name="ck_budget_invoice_amount"),
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(
            ["tenant_id", "purchase_order_id"],
            ["purchase_order.tenant_id", "purchase_order.id"],
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
        ...
        sa.UniqueConstraint(
            "tenant_id", "supplier_id", "invoice_number", name="uq_budget_invoice_identity"
        ),
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('OPEN','CLOSED','CANCELED')", name="ck_budget_commitment_status"
        ),
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(
            ["tenant_id", "purchase_order_id"],
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(
            ["tenant_id", "invoice_id"], ["invoice.tenant_id", "invoice.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
            ["tenant_id", "budget_line_id"],
            ["budget_line.tenant_id", "budget_line.id"],
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "entity_type IN ('PURCHASE_ORDER','INVOICE')", name="ck_budget_reconciliation_entity"
        ),
    ...
    op.create_index(
        "ix_budget_reconciliation_open",
        "reconciliation_issue",
        ["tenant_id", "cost_center_id", "status"],
    )
