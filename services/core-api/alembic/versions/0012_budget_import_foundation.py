...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "source_system IN ('ARIBA','SAP','BIGQUERY','MANUAL')", name="ck_budget_import_source"
        ),
        sa.CheckConstraint(
            "status IN ('STAGED','MATERIALIZED','REJECTED')", name="ck_budget_import_status"
        ),
        ...
        sa.UniqueConstraint(
            ...
            "entity_type",
            "content_hash",
            name="uq_budget_import_batch_hash",
        ),
    )
    op.create_table(
        "import_row",
        ...
        sa.CheckConstraint(
            "status IN ('READY','MATERIALIZED','REJECTED')", name="ck_budget_import_row_status"
        ),
        ...
        sa.UniqueConstraint(
            ...
            "entity_type",
            "row_hash",
            name="uq_budget_import_row_hash",
        ),
    )
