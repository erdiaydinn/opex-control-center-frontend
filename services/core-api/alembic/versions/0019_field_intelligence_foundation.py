...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("groups", postgresql.ARRAY(sa.String(length=120)), nullable=False, server_default="{}"),
    ...
    op.create_index("ix_field_locations_geo", "field_locations", ["tenant_id", "country", "region", "city", "district"])
    ...
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('draft','active','retired')", name="ck_field_template_status"),
    ...
    op.create_table(
        ...
        sa.Column("instructions_i18n", JSONB, nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("status", sa.String(length=20), nullable=False, server_default="draft"),
        ...
        sa.CheckConstraint("status IN ('draft','active','closed','cancelled')", name="ck_field_mission_status"),
        sa.CheckConstraint("priority IN ('normal','high','critical')", name="ck_field_mission_priority"),
    ...
    op.create_index("ix_field_missions_status_deadline", "field_missions", ["tenant_id", "status", "deadline_at"])
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("CURRENT_TIMESTAMP")),
        sa.ForeignKeyConstraint(["tenant_id", "mission_id"], ["field_missions.tenant_id", "field_missions.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["tenant_id", "location_id"], ["field_locations.tenant_id", "field_locations.location_id"], ondelete="RESTRICT"),
        sa.CheckConstraint("status IN ('unseen','seen','started','partial','submitted','rework','verified','overdue','exempt')", name="ck_field_target_status"),
        sa.PrimaryKeyConstraint("tenant_id", "mission_id", "location_id", name="pk_field_mission_targets"),
    )
    op.create_index("ix_field_target_status", "field_mission_targets", ["tenant_id", "mission_id", "status"])
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["tenant_id", "mission_id", "location_id"], ["field_mission_targets.tenant_id", "field_mission_targets.mission_id", "field_mission_targets.location_id"], ondelete="RESTRICT"),
        sa.PrimaryKeyConstraint("tenant_id", "id", name="pk_field_evidence"),
        sa.UniqueConstraint("tenant_id", "mission_id", "location_id", "fingerprint", name="uq_field_evidence_fingerprint"),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["tenant_id", "evidence_id"], ["field_evidence.tenant_id", "field_evidence.id"], ondelete="RESTRICT"),
        sa.CheckConstraint("decision IN ('accept','rework','reject')", name="ck_field_review_decision"),
        sa.CheckConstraint("decision = 'accept' OR length(trim(coalesce(reason,''))) > 0", name="ck_field_review_reason"),
        sa.PrimaryKeyConstraint("tenant_id", "id", name="pk_field_reviews"),
    )

    for table_name in ("field_locations", "field_templates", "field_missions", "field_mission_targets", "field_evidence", "field_reviews"):
        _tenant_policy(table_name)

    op.execute("CREATE FUNCTION prevent_field_evidence_mutation() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'field evidence is append-only'; END; $$")
...
def downgrade() -> None:
    ...
    op.execute(f"DELETE FROM role_permissions rp USING roles r WHERE rp.tenant_id=r.tenant_id AND rp.role_id=r.id AND r.key='super_admin' AND r.is_system IS TRUE AND rp.permission_key=ANY({_sql_array(created_keys)})")
    for role_key in ROLE_POLICIES:
        escaped = role_key.replace("'", "''")
        op.execute(f"DELETE FROM role_permissions rp USING roles r WHERE rp.tenant_id=r.tenant_id AND rp.role_id=r.id AND r.key='{escaped}' AND r.is_system IS TRUE")
        op.execute(f"DELETE FROM roles r WHERE r.key='{escaped}' AND r.is_system IS TRUE AND NOT EXISTS (SELECT 1 FROM membership_roles mr WHERE mr.tenant_id=r.tenant_id AND mr.role_id=r.id)")
    ...
    for table_name in ("field_reviews", "field_evidence", "field_mission_targets", "field_missions", "field_templates", "field_locations"):
