...
def upgrade()->None:
    ...
    op.execute("ALTER TABLE budget_ai_recommendation ENABLE ROW LEVEL SECURITY"); op.execute("ALTER TABLE budget_ai_recommendation FORCE ROW LEVEL SECURITY")
...
def downgrade()->None:
    op.execute('DROP TRIGGER IF EXISTS trg_budget_ai_recommendation_guard ON budget_ai_recommendation'); op.execute('DROP FUNCTION IF EXISTS budget_ai_recommendation_guard()'); op.execute('DROP TABLE IF EXISTS budget_ai_recommendation')
