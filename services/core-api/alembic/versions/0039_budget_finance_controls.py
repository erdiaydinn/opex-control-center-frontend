...
def downgrade()->None:
    op.execute('DROP TRIGGER IF EXISTS trg_budget_export_guard ON budget_export_request'); op.execute('DROP FUNCTION IF EXISTS budget_export_guard()')
    for t in ('budget_finance_uat_attestation','budget_import_version','budget_export_request'): op.execute(f'DROP TABLE IF EXISTS {t}')
