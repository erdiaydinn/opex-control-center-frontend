...
def _ai_tool_admission_limited() -> HTTPException:
    return HTTPException(
...
async def _release_admission_safely(
    *,
    ...
    try:
        ...
        # The lease has a bounded TTL. A release failure must never turn into
        # an execution retry or erase the durable idempotency decision.
        logger.warning(
...
async def execute_ai_tool(
    ...
    """Broker one governed invocation with idempotency and admission control."""
    ...
    try:
        ...
        raise _ai_tool_admission_limited() from exc
    ...
    try:
        ...
        try:
            ...
            # The grant token never leaves this process on this path. It may
            # remain in Redis until expiry, but AI Core never receives it.
            raise _ai_tool_runtime_unavailable(
        ...
        await _release_admission_safely(
            capability=capability,
