...
def _extract_single_assertion_header(request: Request) -> str:
...
async def require_jarvis_service(
    ...
    try:
        verified = verify_jarvis_service_assertion(token, settings)
...
async def require_fresh_jarvis_service(
    ...
    verified = await require_jarvis_service(request, settings)
    ttl_seconds = (
