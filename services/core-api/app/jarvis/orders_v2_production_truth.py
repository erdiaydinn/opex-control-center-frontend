...
def orders_v2_production_receipt(
    ...
    blockers: list[str] = []
    tenants = {record.tenant_id.strip() for record in records if record.tenant_id.strip()}
    if len(tenants) != 1:
    ...
    for key in REQUIRED_EVIDENCE_KEYS:
        ...
        if not record.passed:
            ...
            continue
        if record.evidence_class != "REAL_PRODUCTION_READONLY":
            blockers.append(f"{key}:not_live_production_evidence")
            continue
