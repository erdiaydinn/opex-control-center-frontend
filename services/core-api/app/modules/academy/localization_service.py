...
def _latest_default_sources(
    ...
    for item in content_versions:
        ...
        candidate_version = int(candidate.get("version_number") or 0) if candidate else -1
        item_version = int(item.get("version_number") or 0)
        if candidate is None or item_version > candidate_version:
...
async def localization_governance_telemetry(
    ...
    return {
        ...
        "summary": {
            ...
            "required_coverage_percent": _coverage_percent(
            ...
            "pending_review_count": sum(
                int(row["pending_review_count"]) for row in locale_rows
            ),
