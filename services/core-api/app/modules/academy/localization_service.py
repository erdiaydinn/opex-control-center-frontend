...
def _latest_default_sources(
    ...
    for item in content_versions:
        if item.get("locale") != default_locale:
            continue
        if (
            item.get("version_status") != "published"
            or item.get("content_status") != "published"
        ):
            continue
...
async def localization_governance_telemetry(
    ...
    required_authoritative = sum(
        int(row["authoritative_content_count"]) for row in required_rows
    )
