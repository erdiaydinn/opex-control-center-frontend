...
def issue_playback_token(
    ...
    *,
    ...
    signature = _b64url(hmac.new(config.signing_secret, encoded.encode("ascii"), hashlib.sha256).digest())
...
def build_playback_url(
    ...
    *,
    ...
    return f"{config.cdn_base_url}/{safe_delivery_key}/{safe_manifest}?eay_token={quote(token, safe='') }"
