...
async def get_completion_snapshot(
    ...
    enrollment = (
        await session.execute(
            ...
            {"tenant_id": principal.tenant_id, "enrollment_id": enrollment_id, "subject": principal.subject},
        )
    ).mappings().one_or_none()
    if enrollment is None:
        return None

    required_result = await session.execute(text("""
    ...
    incomplete_result = await session.execute(text("""
        SELECT lpi.content_version_id
        FROM academy_learning_path_items AS lpi
        ...
        ORDER BY lpi.ordinal
    """), {"tenant_id": principal.tenant_id, "path_id": enrollment["path_id"], "enrollment_id": enrollment_id, "subject": principal.subject})
    ...
    certificate = (await session.execute(text("""
    ...
    """), {"tenant_id": principal.tenant_id, "enrollment_id": enrollment_id})).mappings().one_or_none()
...
async def mark_enrollment_completed(
    ...
    *,
    ...
    """), {
        "tenant_id": principal.tenant_id, "enrollment_id": enrollment_id, "path_id": path_id,
        "subject": principal.subject, "certificate_code": certificate_code, "completion_fingerprint": completion_fingerprint,
    })
    certificate = (await session.execute(text("""
        SELECT certificate_code, contract_version, completion_fingerprint
        FROM academy_certificates
        WHERE tenant_id=:tenant_id AND enrollment_id=:enrollment_id
    """), {"tenant_id": principal.tenant_id, "enrollment_id": enrollment_id})).mappings().one()
