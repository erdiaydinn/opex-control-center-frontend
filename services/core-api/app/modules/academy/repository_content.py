...
async def _insert_version(session: AsyncSession, principal: Principal, content_id: UUID, payload: Any, version_number: int) -> dict[str, Any]:
    ...
    """), {
        ...
        "version_label": payload.version_label, "version_number": version_number,
        "locale": payload.locale, "mime_type": payload.mime_type,
        "source_sha256": payload.source_sha256.lower() if payload.source_sha256 else None,
        "storage_key": payload.storage_key, "delivery_key": payload.delivery_key,
        "size_bytes": payload.size_bytes, "duration_ms": payload.duration_ms,
        "accessibility_metadata": json_text(payload.accessibility_metadata),
        "status": payload.status, "created_by": principal.subject,
    })).mappings().one()
    return dict(row)


async def create_content(session: AsyncSession, principal: Principal, payload: Any) -> dict[str, Any]:
    ...
    """), {
        "tenant_id": principal.tenant_id, "content_type": payload.content_type,
        "slug": payload.slug, "title_i18n": json_text(payload.title_i18n),
...
async def create_content_version(session: AsyncSession, principal: Principal, content_id: UUID, payload: Any) -> dict[str, Any] | None:
    version_number = await session.scalar(text("""
...
async def create_media_asset(session: AsyncSession, principal: Principal, payload: Any) -> dict[str, Any] | None:
    row = (await session.execute(text("""
    ...
    """), {
        "tenant_id": principal.tenant_id, "content_version_id": payload.content_version_id,
        "asset_kind": payload.asset_kind, "storage_provider": payload.storage_provider,
        "storage_bucket": payload.storage_bucket, "storage_key": payload.storage_key,
        "delivery_key": payload.delivery_key, "manifest_path": payload.manifest_path,
        "checksum_sha256": payload.checksum_sha256.lower(), "size_bytes": payload.size_bytes,
        "duration_ms": payload.duration_ms, "transcode_status": payload.transcode_status,
        "delivery_mode": payload.delivery_mode, "encryption_mode": payload.encryption_mode,
        "segment_duration_seconds": payload.segment_duration_seconds, "created_by": principal.subject,
