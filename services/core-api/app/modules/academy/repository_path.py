...
async def create_learning_path(session: AsyncSession, principal: Principal, payload: Any) -> dict[str, Any]:
    path = (await session.execute(text("""
    ...
    """), {
        "tenant_id": principal.tenant_id, "key": payload.key,
    ...
    for ordinal, item in enumerate(payload.items, 1):
        ...
        """), {
            "tenant_id": principal.tenant_id, "path_id": path["id"],
            "content_version_id": item.content_version_id, "ordinal": ordinal,
            "required": item.required, "completion_policy": json_text(item.completion_policy),
        })
        if inserted is None:
    ...
    for assignment in payload.role_assignments:
        ...
        """), {
            ...
            "required": assignment.required, "due_days": assignment.due_days,
        })
    return dict(path)


async def grant_entitlement(session: AsyncSession, principal: Principal, payload: Any) -> dict[str, Any]:
    ...
    """), {
        "tenant_id": principal.tenant_id, "resource_type": payload.resource_type,
        "resource_id": payload.resource_id, "principal_type": payload.principal_type,
        "principal_key": principal_key, "permission": payload.permission,
