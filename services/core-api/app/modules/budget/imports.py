...
async def stage_import(uow: BudgetUnitOfWork, body: ImportStage) -> dict[str, object]:
    ...
    if errors:
        raise HTTPException(
            status_code=422, detail={"message": "Import validation failed", "rows": errors[:100]}
        )
    content_hash = batch_hash(normalized_rows, namespace=namespace)
    result = await uow.session.execute(
        text(
            """INSERT INTO import_batch(tenant_id,source_system,entity_type,content_hash,created_by) VALUES (:tenant,:source,:entity,:hash,:actor) ON CONFLICT (tenant_id,source_system,entity_type,content_hash) DO NOTHING RETURNING id"""
        ),
        {
    ...
    if created is None:
        existing = await uow.session.execute(
            text(
                """SELECT id FROM import_batch WHERE tenant_id=:tenant AND source_system=:source AND entity_type=:entity AND content_hash=:hash"""
            ),
            {
                "tenant": uow.tenant_id,
                "source": body.source_system,
    ...
    for row in normalized_rows:
        ...
        row_result = await uow.session.execute(
            text(
                """INSERT INTO import_row(tenant_id,batch_id,source_system,entity_type,row_hash,payload) VALUES (:tenant,:batch,:source,:entity,:hash,CAST(:payload AS jsonb)) ON CONFLICT (tenant_id,source_system,entity_type,row_hash) DO NOTHING RETURNING id"""
            ),
            {
                "tenant": uow.tenant_id,
                "batch": batch_id,
    ...
    await emit_financial_event(
        ...
        payload={**response, "source_system": body.source_system, "entity_type": body.entity_type},
    )
    return response
