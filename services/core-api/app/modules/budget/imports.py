...
async def stage_import(uow: BudgetUnitOfWork, body: ImportStage) -> dict[str, object]:
    ...
    for index, raw in enumerate(body.rows, start=1):
        ...
        error = validate_external_identity(
            body.source_system,
            body.entity_type,
            normalized,
        )
    ...
    if errors:
        raise HTTPException(
            status_code=422,
            detail={"message": "Import validation failed", "rows": errors[:100]},
        )
    ...
    result = await uow.session.execute(
        text(
            ...
            INSERT INTO import_batch(
              tenant_id,source_system,entity_type,content_hash,created_by
            ) VALUES (:tenant,:source,:entity,:hash,:actor)
            ON CONFLICT (tenant_id,source_system,entity_type,content_hash) DO NOTHING
            RETURNING id
            """
        ),
        {
            ...
            text(
                ...
                SELECT id FROM import_batch
                 WHERE tenant_id=:tenant AND source_system=:source
                   AND entity_type=:entity AND content_hash=:hash
                """
            ),
            {
                "tenant": uow.tenant_id,
        ...
        row_result = await uow.session.execute(
            text(
                ...
                INSERT INTO import_row(
                  tenant_id,batch_id,source_system,entity_type,row_hash,payload
                ) VALUES (:tenant,:batch,:source,:entity,:hash,CAST(:payload AS jsonb))
                ON CONFLICT (tenant_id,source_system,entity_type,row_hash) DO NOTHING
                RETURNING id
                """
            ),
            {
