...
async def create_plan(uow: BudgetUnitOfWork, body: PlanCreate) -> dict[str, object]:
    result = await uow.session.execute(
        text(
            """INSERT INTO budget_plan(tenant_id,name,fiscal_year,base_currency,created_by) VALUES (:tenant,:name,:year,:currency,:actor) RETURNING *"""
        ),
        {
    ...
    await emit_financial_event(
        uow,
        event_type="BUDGET_PLAN_CREATED",
...
async def activate_plan(uow: BudgetUnitOfWork, plan_id: UUID) -> dict[str, object]:
    plan_result = await uow.session.execute(
        text("SELECT * FROM budget_plan WHERE tenant_id=:tenant AND id=:id FOR UPDATE"),
    ...
    if plan.created_by == uow.actor:
        raise HTTPException(
            status_code=409, detail="Budget Plan activation requires a second actor"
        )
    counts = await uow.session.execute(
        text(
            """SELECT (SELECT COUNT(*) FROM fiscal_period WHERE tenant_id=:tenant AND plan_id=:id) periods, (SELECT COUNT(*) FROM budget_line WHERE tenant_id=:tenant AND plan_id=:id) lines"""
        ),
        {"tenant": uow.tenant_id, "id": plan_id},
    )
    current = counts.one()
    ...
    result = await uow.session.execute(
        text(
            """UPDATE budget_plan SET status='ACTIVE',activated_by=:actor,activated_at=now() WHERE tenant_id=:tenant AND id=:id AND status='DRAFT' RETURNING *"""
        ),
    ...
    await emit_financial_event(
        uow,
        event_type="BUDGET_PLAN_ACTIVATED",
        aggregate_type="budget_plan",
        aggregate_id=item["id"],
        payload=item,
    )
...
async def create_period(uow: BudgetUnitOfWork, body: PeriodCreate) -> dict[str, object]:
    await uow.session.execute(
        ...
        {"key": f"budget-period:{uow.tenant_id}:{body.plan_id}"},
    )
    ...
    overlap = await uow.session.execute(
        text(
            """SELECT EXISTS(SELECT 1 FROM fiscal_period WHERE tenant_id=:tenant AND plan_id=:plan AND daterange(starts_on,ends_on,'[]') && daterange(:starts,:ends,'[]'))"""
        ),
        {
            "tenant": uow.tenant_id,
            "plan": body.plan_id,
            "starts": body.starts_on,
            "ends": body.ends_on,
        },
    )
    ...
    result = await uow.session.execute(
        text(
            """INSERT INTO fiscal_period(tenant_id,plan_id,code,starts_on,ends_on) VALUES (:tenant,:plan,:code,:starts,:ends) RETURNING *"""
        ),
        {
            "tenant": uow.tenant_id,
            "plan": body.plan_id,
    ...
    await emit_financial_event(
        uow,
        event_type="FISCAL_PERIOD_CREATED",
        aggregate_type="fiscal_period",
        aggregate_id=item["id"],
        payload=item,
    )
    return item
...
async def create_cost_center(uow: BudgetUnitOfWork, body: CostCenterCreate) -> dict[str, object]:
    result = await uow.session.execute(
        text(
            """INSERT INTO cost_center(tenant_id,code,name,store_code,created_by) VALUES (:tenant,:code,:name,:store,:actor) RETURNING *"""
        ),
        {
            "tenant": uow.tenant_id,
...
async def create_line(uow: BudgetUnitOfWork, body: BudgetLineCreate) -> dict[str, object]:
    ...
    period = await uow.session.execute(
        text(
            "SELECT 1 FROM fiscal_period WHERE tenant_id=:tenant AND id=:period AND plan_id=:plan"
        ),
        {"tenant": uow.tenant_id, "period": body.fiscal_period_id, "plan": body.plan_id},
    )
    if period.first() is None:
        raise HTTPException(status_code=409, detail="Budget Line period does not belong to plan")
    result = await uow.session.execute(
        text(
            """INSERT INTO budget_line(tenant_id,plan_id,fiscal_period_id,cost_center_id,category,supplier_id,supplier_name,store_code,budget_base_amount,created_by) VALUES (:tenant,:plan,:period,:center,:category,:supplier_id,:supplier_name,:store,:amount,:actor) RETURNING *"""
        ),
        {
            "tenant": uow.tenant_id,
            "plan": body.plan_id,
            ...
            "store": body.store_code,
            "amount": body.budget_base_amount,
            "actor": uow.actor,
        },
    )
    item = _row(result.one())
    await emit_financial_event(
        uow,
        event_type="BUDGET_LINE_CREATED",
...
async def create_forecast(uow: BudgetUnitOfWork, body: ForecastCreate) -> dict[str, object]:
    result = await uow.session.execute(
        text(
            """INSERT INTO forecast(tenant_id,budget_line_id,fiscal_period_id,cost_center_id,forecast_base_amount,as_of,created_by) VALUES (:tenant,:line,:period,:center,:amount,:as_of,:actor) RETURNING *"""
        ),
        {
            "tenant": uow.tenant_id,
            ...
            "period": body.fiscal_period_id,
            "center": body.cost_center_id,
            "amount": body.forecast_base_amount,
            "as_of": body.as_of,
            "actor": uow.actor,
        },
    )
...
async def close_period(uow: BudgetUnitOfWork, period_id: UUID) -> dict[str, object]:
    ...
    result = await uow.session.execute(
        text(
            """UPDATE fiscal_period SET status='CLOSED',closed_by=:actor,closed_at=now() WHERE tenant_id=:tenant AND id=:id AND status='OPEN' RETURNING *"""
        ),
