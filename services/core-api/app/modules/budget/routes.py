...
PlanCreateSession = Annotated[
    BudgetUnitOfWork, Depends(require_budget(BUDGET_CREATE_PLAN, all_cost_centers=True))
]
PlanActivateSession = Annotated[
    BudgetUnitOfWork, Depends(require_budget(BUDGET_ACTIVATE_PLAN, all_cost_centers=True))
]
PeriodSession = Annotated[
    BudgetUnitOfWork, Depends(require_budget(BUDGET_MANAGE_PERIODS, all_cost_centers=True))
]
CostCenterSession = Annotated[
    BudgetUnitOfWork, Depends(require_budget(BUDGET_MANAGE_COST_CENTERS, all_cost_centers=True))
]
LineSession = Annotated[
    BudgetUnitOfWork, Depends(require_budget(BUDGET_MANAGE_LINES, all_cost_centers=True))
]
...
ImportSession = Annotated[
    BudgetUnitOfWork, Depends(require_budget(BUDGET_IMPORT, all_cost_centers=True))
]
ReconciliationSession = Annotated[BudgetUnitOfWork, Depends(require_budget(BUDGET_RECONCILE))]
ClosePeriodSession = Annotated[
    BudgetUnitOfWork, Depends(require_budget(BUDGET_CLOSE_PERIOD, all_cost_centers=True))
]
AuditSession = Annotated[BudgetUnitOfWork, Depends(require_budget(BUDGET_VIEW_AUDIT))]
ExportSession = Annotated[
    BudgetUnitOfWork, Depends(require_budget(BUDGET_EXPORT, all_cost_centers=True))
]
...
async def post_plan(body: PlanCreate, key: IdempotencyKey, uow: PlanCreateSession):
    return await run_command(
        uow,
        key=key,
        operation="budget.plan.create",
        payload=body,
        perform=lambda: create_plan(uow, body),
    )
...
async def post_plan_activation(plan_id: UUID, key: IdempotencyKey, uow: PlanActivateSession):
    return await run_command(
        uow,
        key=key,
        operation="budget.plan.activate",
        payload={"plan_id": plan_id},
        perform=lambda: activate_plan(uow, plan_id),
    )
...
async def post_period(body: PeriodCreate, key: IdempotencyKey, uow: PeriodSession):
    return await run_command(
        uow,
        key=key,
        operation="budget.period.create",
        payload=body,
        perform=lambda: create_period(uow, body),
    )
...
async def post_cost_center(body: CostCenterCreate, key: IdempotencyKey, uow: CostCenterSession):
    return await run_command(
        uow,
        key=key,
        operation="budget.cost_center.create",
        payload=body,
        perform=lambda: create_cost_center(uow, body),
    )
...
async def post_line(body: BudgetLineCreate, key: IdempotencyKey, uow: LineSession):
    return await run_command(
        uow,
        key=key,
        operation="budget.line.create",
        payload=body,
        perform=lambda: create_line(uow, body),
    )
...
async def post_request(body: PurchaseRequestCreate, key: IdempotencyKey, uow: RequestSession):
    return await run_command(
        uow,
        key=key,
        operation="budget.request.create",
        payload=body,
        perform=lambda: create_request(uow, body),
    )


@router.post("/requests/{request_id}/decision")
async def post_request_decision(
    request_id: UUID, body: ApprovalDecision, key: IdempotencyKey, uow: ApprovalSession
):
    return await run_command(
        uow,
        key=key,
        operation="budget.request.decision",
        payload={"request_id": request_id, "body": body},
        perform=lambda: decide_request(uow, request_id, body),
    )
...
async def post_purchase_order(body: PurchaseOrderCreate, key: IdempotencyKey, uow: POSession):
    return await run_command(
        uow,
        key=key,
        operation="budget.po.create",
        payload=body,
        perform=lambda: create_po(uow, body),
    )
...
async def post_invoice_route(body: InvoiceCreate, key: IdempotencyKey, uow: InvoiceSession):
    return await run_command(
        uow,
        key=key,
        operation="budget.invoice.post",
        payload=body,
        perform=lambda: post_invoice(uow, body),
    )
...
async def post_forecast(body: ForecastCreate, key: IdempotencyKey, uow: ForecastSession):
    return await run_command(
        uow,
        key=key,
        operation="budget.forecast.create",
        payload=body,
        perform=lambda: create_forecast(uow, body),
    )
...
async def post_import(body: ImportStage, key: IdempotencyKey, uow: ImportSession):
    return await run_command(
        uow,
        key=key,
        operation="budget.import.stage",
        payload=body,
        perform=lambda: stage_import(uow, body),
    )


@router.post("/reconciliation/{issue_id}/resolve")
async def post_reconciliation(
    issue_id: UUID, body: ReconciliationResolve, key: IdempotencyKey, uow: ReconciliationSession
):
    return await run_command(
        uow,
        key=key,
        operation="budget.reconciliation.resolve",
        payload={"issue_id": issue_id, "body": body},
        perform=lambda: resolve_reconciliation(uow, issue_id, body),
    )
...
async def post_period_close(period_id: UUID, key: IdempotencyKey, uow: ClosePeriodSession):
    return await run_command(
        uow,
        key=key,
        operation="budget.period.close",
        payload={"period_id": period_id},
        perform=lambda: close_period(uow, period_id),
    )
...
async def export_variance(uow: ExportSession):
    ...
    fields = [
        ...
        "budget_base_amount",
        "actual_base_amount",
        "committed_base_amount",
        "forecast_base_amount",
        "variance_base_amount",
    ]
    output = io.StringIO()
    ...
    await emit_financial_event(
        uow,
        event_type="VARIANCE_EXPORTED",
        aggregate_type="variance_export",
        aggregate_id=uuid4(),
        payload={"row_count": report["count"], "format": "csv"},
    )
