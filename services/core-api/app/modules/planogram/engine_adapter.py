...
def _resolve_engine_root() -> Path:
    required = ("engine.py", "physical_truth.py", "physical_engine.py", "architecture_truth.py")
...
def _module_from_root(name: str, root: Path) -> ModuleType:
    ...
    if existing is not None:
        ...
        if source is None or Path(source).resolve().parent != root:
            raise PlanogramEngineUnavailable(f"Unsafe Python module collision while loading Planogram dependency: {name}")
        return existing
    root_text = str(root)
    ...
    if source is None or Path(source).resolve().parent != root:
        raise PlanogramEngineUnavailable(f"Planogram dependency resolved outside canonical engine root: {name}")
    return module
...
def _load_optimizer() -> ModuleType:
    ...
    if not (root / "physical_optimizer_v3.py").is_file():
...
def _load_market_search_optimizer() -> ModuleType:
    root, _, _, _ = _load_modules()
    if not (root / "physical_optimizer_v4.py").is_file():
        raise PlanogramEngineUnavailable("Experimental Planogram market-search optimizer V4 is unavailable")
...
def _load_capacity_validator() -> ModuleType:
    root, _, _, _ = _load_modules()
    if not (root / "physical_capacity_v2.py").is_file():
...
def _capacity_summary(report: dict[str, Any]) -> dict[str, Any]:
    return {
        "contract": report.get("contract"), "available": report.get("available"),
        "valid": report.get("valid"), "violation_count": int(report.get("violation_count") or 0),
...
def _apply_capacity_v2_veto(result: dict[str, Any]) -> dict[str, Any]:
    ...
    codes = {str(row.get("code") or "unknown") for row in report.get("violations") or []}
    if not codes:
    ...
    summary["physical_capacity_v2_violation_count"] = int(report.get("violation_count") or 0)
    summary["physical_capacity_v2_warning_count"] = int(report.get("warning_count") or 0)
...
def engine_status() -> dict[str, Any]:
    ...
    return {
        "available": True, "contract": "physical-truth-gated-deterministic-v1",
        "foundation": "deterministic-best-fit-v4.2", "library_mode": True,
        "legacy_bridge_enabled": False, "production_ai_dimensions_allowed": False,
        ...
        "capacity_v2": {
            ...
            "veto_authority": True, "production_authority": False,
        },
        "optimizer": {
            "available": (root / "physical_optimizer_v3.py").is_file(),
            "contract": "physical-plan-optimizer-v3-picker-tour",
            "fallback_contract": "physical-plan-optimizer-v2", "production_authority": False,
            "route_objective": "measured-basket-picker-tour-v1", "requires_observed_baskets": True,
        },
        "market_search_benchmark": {
            "available": (root / "physical_optimizer_v4.py").is_file(),
            "contract": "physical-plan-optimizer-v4-bounded-search", "preview_only": True,
            "production_authority": False, "requires_observed_baskets": True,
            "promotion_requires": ["physical_capacity_v2", "blind_expert_benchmark", "real_store_kpi_backtest", "field_acceptance"],
        },
        "source_modules": {"engine": Path(engine.__file__ or "").name, "physical_truth": Path(physical_truth.__file__ or "").name, "physical_engine": Path(physical_engine.__file__ or "").name},
    }


def generate_preview(*, products: list[dict[str, Any]], layout: dict[str, Any], store_dna: dict[str, Any], mode: str) -> dict[str, Any]:
...
def generate_optimized_preview(*, products: list[dict[str, Any]], layout: dict[str, Any], store_dna: dict[str, Any], mode: str, orders: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    result = optimize(products, layout, store_dna, mode=mode, orders=orders)
...
def _objective_delta(canonical: dict[str, float | int], experimental: dict[str, float | int]) -> dict[str, float]:
    names = ("hard_violation_count", "weighted_unplaced_sales", "unplaced_sku_count", "tour_unsimulated_order_count", "tour_p95_m", "tour_average_m", "coverage_shortfall", "brand_fragmentation", "capacity_pressure")
    return {name: round(float(experimental.get(name) or 0.0) - float(canonical.get(name) or 0.0), 6) for name in names}


def generate_market_leadership_benchmark_preview(*, products: list[dict[str, Any]], layout: dict[str, Any], store_dna: dict[str, Any], mode: str, orders: list[dict[str, Any]], max_candidates: int = 24) -> dict[str, Any]:
    if not orders:
        raise PlanogramEngineUnavailable("Market-leadership benchmark requires observed or test SKU baskets")
    ...
    canonical = _apply_capacity_v2_veto(canonical_optimize(products, layout, store_dna, mode=mode, orders=orders))
    experimental = _apply_capacity_v2_veto(market_optimize(products, layout, store_dna, mode=mode, orders=orders, max_candidates=max_candidates))
    ...
    objectives_available = isinstance(canonical_objective, dict) and isinstance(experimental_objective, dict)
    ...
    if comparison_available:
        if canonical_valid and not experimental_valid:
            ...
            winner = "experimental_v4" if experimental_rank < canonical_rank else "canonical_v3" if canonical_rank < experimental_rank else "tie"
        else:
            winner = "unavailable"
        delta = _objective_delta(canonical_objective, experimental_objective)
    else:
        winner, delta = "unavailable", {}
    promotion_blockers = ["blind_expert_benchmark_required", "real_store_kpi_backtest_required", "field_acceptance_required"]
    ...
    return {
        ...
        "preview_only": True, "production_authority": False, "production_evidence": False,
        "comparison_available": comparison_available, "winner_on_repository_objective": winner,
        "objective_delta_experimental_minus_canonical": delta,
        "canonical_v3": {"optimizer_version": canonical_meta.get("optimizer_version"), "allowed": canonical_meta.get("allowed"), "effective": canonical_meta.get("effective"), "selected_strategy": canonical_meta.get("selected_strategy"), "candidate_count": canonical_meta.get("candidate_count"), "selected_objective": canonical_objective, "selected_tour": canonical_meta.get("selected_tour"), "physical_capacity_v2": _capacity_summary(canonical_capacity)},
        "experimental_v4": {"optimizer_version": experimental_meta.get("optimizer_version"), "allowed": experimental_meta.get("allowed"), "effective": experimental_meta.get("effective"), "selected_strategy": experimental_meta.get("selected_strategy"), "candidate_count": experimental_meta.get("candidate_count"), "search_budget": experimental_meta.get("search_budget"), "pareto_frontier_count": experimental_meta.get("pareto_frontier_count"), "selected_objective": experimental_objective, "selected_tour": experimental_meta.get("selected_tour"), "alternatives": experimental_meta.get("alternatives") or [], "physical_capacity_v2": _capacity_summary(experimental_capacity)},
        "promotion_allowed": False, "promotion_blockers": promotion_blockers,
        "evidence_boundary": "repository objective comparison is vetoed by full-depth capacity v2; neither ranking nor green capacity proves live KPI gain",
    }
