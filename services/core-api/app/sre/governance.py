...
EvidenceClass = Literal['REPOSITORY','SYNTHETIC','MANAGED_STAGING','REAL_ENVIRONMENT','REAL_MEDIA_ENVIRONMENT']
...
def external_gate_satisfied(evidence: AcceptanceEvidence) -> bool:
    return evidence.evidence_class in {'MANAGED_STAGING','REAL_ENVIRONMENT','REAL_MEDIA_ENVIRONMENT'} and evidence.measured and bool(evidence.provenance.strip()) and bool(evidence.environment.strip())


def production_shape_evidence_satisfied(profile: dict[str, object], evidence: AcceptanceEvidence) -> bool:
    if not external_gate_satisfied(evidence):
        return False
    required=str(profile.get('required_evidence',''))
    if required=='MANAGED_STAGING_LOAD':
        return evidence.evidence_class in {'MANAGED_STAGING','REAL_ENVIRONMENT','REAL_MEDIA_ENVIRONMENT'}
...
def load_sre_registry(path:Path)->dict[str,object]:
    ...
    for test in data.get('production_shape_tests',[]):
        ...
        if test.get('required_evidence') not in {'MANAGED_STAGING_LOAD','REAL_MEDIA_ENVIRONMENT_LOAD'}:
