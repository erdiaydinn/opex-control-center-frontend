...
def test_replay_retention_covers_assertion_window():
    ...
    assert required_retention <= (INTERNAL_SERVICE_REPLAY_MAX_TTL_SECONDS)
...
def test_config_fails_closed_if_replay_capacity_is_too_small(
    ...
    with pytest.raises(
        ValueError,
        match=("Internal assertion lifetime exceeds replay retention capacity"),
    ):
...
def test_mutating_internal_routes_require_fresh_assertion():
    ...
    for path in APP_ROOT.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            ...
            basic_internal = "require_internal_service" in names

            fresh_internal = "require_fresh_internal_service" in names

            if basic_internal and not fresh_internal:
    ...
    assert not violations, (
        ...
        "must use "
        "require_fresh_internal_service: " + ", ".join(violations)
    )


def test_security_module_uses_canonical_replay_skew():
    security_path = APP_ROOT / "core" / "security.py"

    source = security_path.read_text(encoding="utf-8-sig")

    assert "_INTERNAL_SERVICE_REPLAY_TTL_SKEW_SECONDS" not in source

    assert "INTERNAL_SERVICE_REPLAY_TTL_SKEW_SECONDS" in source
