...
def test_actor_budgets_cannot_exceed_tenant_budgets() -> None:
    with pytest.raises(ValidationError):
        settings(tenant_requests_per_window=5, actor_requests_per_window=6)
    with pytest.raises(ValidationError):
        settings(tenant_concurrency=2, actor_concurrency=3)
...
def test_broker_timeout_is_bound_to_lease_ttl_and_hard_cap() -> None:
    ...
    with pytest.raises(JarvisAdmissionInvalid):
        broker_lease_ttl_seconds(170, maximum_lease_ttl_seconds=180)
...
@pytest.mark.parametrize(
    "response",
    [None, [], [1], [1, "unexpected"], [0, "unknown"], [2, "admitted"]],
)
@pytest.mark.asyncio
