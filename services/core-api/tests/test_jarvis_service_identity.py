...
def verification_material(
    tmp_path: Path,
) -> tuple[ec.EllipticCurvePrivateKey, JarvisServiceSettings]:
    private_key = ec.generate_private_key(ec.SECP256R1())
    public_jwk = json.loads(
        ECAlgorithm.to_jwk(private_key.public_key())
    )
    public_jwk.update(
...
def test_disabled_identity_fails_closed() -> None:
...
def test_phase1_issuer_and_audiences_are_reserved() -> None:
    ...
    for audience in ("opex-core-api", "opex-core-preauth"):
        with pytest.raises(ValueError):
            JarvisServiceSettings(
                enabled=True,
                assertion_audience=audience,
...
def test_foreign_signing_key_is_not_trusted(tmp_path: Path) -> None:
...
def test_phase1_token_types_cannot_cross_boundary(
    tmp_path: Path,
    token_type: str,
    audience: str,
    subject: str,
...
@pytest.mark.parametrize(
    ...
    [
        ("opex-core-api", JARVIS_SERVICE_SUBJECT, JARVIS_SERVICE_PURPOSE),
        (AUDIENCE, "identity-gateway", JARVIS_SERVICE_PURPOSE),
        (AUDIENCE, JARVIS_SERVICE_SUBJECT, "preauth"),
    ],
)
...
def test_claim_smuggling_is_rejected(tmp_path: Path) -> None:
    private_key, settings = verification_material(tmp_path)
    with pytest.raises(InternalAssertionInvalid):
        verify_jarvis_service_assertion(
            token_for(
...
def test_lifetime_is_independently_bounded(tmp_path: Path) -> None:
    private_key, settings = verification_material(tmp_path)
    with pytest.raises(InternalAssertionInvalid):
        verify_jarvis_service_assertion(
            token_for(
