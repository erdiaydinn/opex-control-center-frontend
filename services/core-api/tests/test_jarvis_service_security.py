...
@pytest.mark.asyncio
async def test_disabled_boundary_is_hidden() -> None:
...
@pytest.mark.asyncio
async def test_valid_header_sets_machine_identity(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_duplicate_or_coalesced_header_is_rejected() -> None:
    duplicate = request_with_headers(
        [
            (b"x-opex-jarvis-service-assertion", b"token-one"),
            (b"x-opex-jarvis-service-assertion", b"token-two"),
        ]
    )
...
@pytest.mark.asyncio
async def test_fresh_assertion_consumes_replay_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    install_valid_verifier(monkeypatch)
...
@pytest.mark.asyncio
async def test_replay_is_401_and_clears_state(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    install_valid_verifier(monkeypatch)
    monkeypatch.setattr(
