...
TENANT_A = UUID("00000000-0000-0000-0000-000000009a01")

TENANT_B = UUID("00000000-0000-0000-0000-000000009b01")

MEMBERSHIP_A = UUID("00000000-0000-0000-0000-000000009a11")
...
def test_membership_authorization_api_has_no_external_identity_input() -> None:
    signature = inspect.signature(resolve_membership_access)
    ...
    source = inspect.getsource(resolve_membership_access)
...
async def test_membership_authorization_is_tenant_bound_and_fail_closed() -> None:
    ...
    try:
        ...
        assert result["membership_id"] == str(MEMBERSHIP_A)

        assert result["roles"] == ()
        assert result["permission_assignments"] == ()

        # Same internal membership UUID presented under another
        # tenant must never resolve.
        cross_tenant = await resolve_membership_access(
            tenant_id=str(TENANT_B),
            membership_id=str(MEMBERSHIP_A),
        )

        assert cross_tenant is not None
        assert cross_tenant["tenant_status"] == "active"

        assert cross_tenant["membership_id"] is None

        assert cross_tenant["membership_status"] is None

        assert cross_tenant["roles"] == ()

        assert cross_tenant["permission_assignments"] == ()

        # Unknown tenant also fails closed.
        missing_tenant = await resolve_membership_access(
            tenant_id=("00000000-0000-0000-0000-000000009c01"),
            membership_id=str(MEMBERSHIP_A),
        )
