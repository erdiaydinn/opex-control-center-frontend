...
def test_production_gateway_replaces_dev_ports() -> None:
    ...
    assert "${OPEX_GATEWAY_HTTP_PORT:-80}:80" in compose
    assert "${OPEX_GATEWAY_HTTPS_PORT:-443}:443" in compose
...
def test_production_gateway_replaces_dev_nginx_config() -> None:
    ...
    assert "platform.production.conf.template" in compose
...
def test_http_redirect_is_fixed_host_https() -> None:
    nginx = _read("infra/nginx/platform.production.conf.template")

    assert "return 308 https://${OPEX_PUBLIC_HOST}$request_uri;" in nginx
...
def test_hsts_exists_only_after_tls_listener() -> None:
    ...
    assert nginx.count("Strict-Transport-Security") == 1

    assert nginx.index("Strict-Transport-Security") > nginx.index("listen 443 ssl default_server;")

    hsts = next(line for line in nginx.splitlines() if "Strict-Transport-Security" in line)
...
def test_tls_gateway_overwrites_forwarding_headers() -> None:
    ...
    assert nginx.count("proxy_set_header X-Forwarded-Proto https;") == 2
...
def test_host_header_is_fail_closed() -> None:
    ...
    assert nginx.count('if ($host != "${OPEX_PUBLIC_HOST}")') == 2

    assert nginx.count("return 421;") == 2
