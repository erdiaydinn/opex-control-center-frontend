...
@pytest.mark.asyncio
async def test_postgresql_rls_blocks_cross_tenant_reads_and_writes() -> None:
    engine = create_async_engine(get_settings().database_url, pool_pre_ping=True)

    try:
        async with engine.begin() as connection:
            await seed_tenant(connection, TENANT_A, "tenant-a", "Tenant A")
            await connection.execute(
                text(
                    """
        ...
        async with engine.begin() as connection:
            await seed_tenant(connection, TENANT_B, "tenant-b", "Tenant B")
