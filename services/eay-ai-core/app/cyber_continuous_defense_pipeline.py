"""Continuous, evidence-bound cyber defense orchestration for EAY Jarvis.

The module composes existing canonical cyber authorities. Public threat feeds are
never company truth. EAY impact requires company evidence. Detection gaps require
company telemetry evidence. Recommendations are candidates only. Sandbox and
benchmark receipts never grant production writes, exploit execution or promotion.
"""
...
class LiveThreatSourceUnavailable(RuntimeError):
    """An allowlisted public source could not be observed safely."""
...
class PublicFeedObservation(BaseModel):
    ...
    def validate_observation(self) -> PublicFeedObservation:
        ...
        if self.credentials_sent or self.arbitrary_url_allowed:
            raise ValueError("cyber_live_feed_unbounded_network_forbidden")
...
class LiveThreatIngestionReceipt(BaseModel):
    ...
    nvd_threat: ThreatKnowledgeRecord
    ...
    @model_validator(mode="after")
    def validate_ingestion(self) -> LiveThreatIngestionReceipt:
        _cve(self.cve_id)
        ...
        ThreatKnowledgeRecord.model_validate(self.nvd_threat.model_dump(mode="json"))
        if self.primary_threat.known_exploited_in_wild:
            ...
            if not self.known_exploitation_authority_observed:
                raise ValueError("cyber_live_known_exploitation_requires_authority")
        if self.company_truth_granted or self.incident_confirmation_granted:
            raise ValueError("cyber_live_public_source_never_grants_company_truth")
...
class EayAssetObservation(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_asset(self) -> EayAssetObservation:
        ...
        if self.execution_authority_granted:
            raise ValueError("eay_asset_never_grants_execution_authority")
        for refs in (self.product_refs, self.cpe_refs, self.evidence_refs):
            _unique(refs, "eay_asset_refs_must_be_unique")
            for ref in refs:
                _safe_ref(ref, "eay_asset_unsafe_ref")
        _safe_ref(self.asset_ref, "eay_asset_unsafe_ref")
        _verify(self, "eay_asset_fingerprint_mismatch")
        return self
...
class EayDependencyObservation(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_dependency(self) -> EayDependencyObservation:
        ...
        for attack_id in self.attack_technique_ids:
            if not _ATTACK.fullmatch(attack_id):
...
class EayAssetInventorySnapshot(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_inventory(self) -> EayAssetInventorySnapshot:
        ...
        for dependency in self.dependencies:
            ...
            if dependency.from_asset_ref not in asset_set:
...
class EayCveImpactReceipt(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_impact(self) -> EayCveImpactReceipt:
        CompanyIdentity.model_validate(self.identity.model_dump(mode="json"))
        _cve(self.cve_id)
...
class EayAttackGraphReceipt(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_graph_receipt(self) -> EayAttackGraphReceipt:
...
class SigmaRuleMetadata(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_sigma_rule(self) -> SigmaRuleMetadata:
    ...
    def logsource_key(self) -> str:
        ...
        return "sigma-logsource:" + "/".join(part.lower() for part in parts)
...
class SigmaTelemetryObservation(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_sigma_telemetry(self) -> SigmaTelemetryObservation:
        CompanyIdentity.model_validate(self.identity.model_dump(mode="json"))
        ...
        live = self.status in {
            SigmaTelemetryStatus.AVAILABLE,
            SigmaTelemetryStatus.DEGRADED,
        }
        if live and self.telemetry_ref is None:
            raise ValueError("sigma_live_telemetry_requires_ref")
        if not live and self.telemetry_ref is not None:
        ...
        _unique(self.evidence_refs, "sigma_telemetry_evidence_refs_must_be_unique")
...
class SigmaCoverageReceipt(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_sigma_coverage(self) -> SigmaCoverageReceipt:
        CompanyIdentity.model_validate(self.identity.model_dump(mode="json"))
        _cve(self.cve_id)
...
class ControlledSandboxValidationReceipt(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_sandbox(self) -> ControlledSandboxValidationReceipt:
        CompanyIdentity.model_validate(self.identity.model_dump(mode="json"))
        external = self.environment is SandboxEvidenceClass.AUTHORIZED_EXTERNAL
        if external and not self.authorization_evidence_ref:
        ...
        expected_qualifies = external and expected_pass and bool(self.authorization_evidence_ref)
...
class ContinuousCyberBenchmarkCheckpoint(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_checkpoint(self) -> ContinuousCyberBenchmarkCheckpoint:
        if self.production_security_superiority_claim_allowed:
            raise ValueError("continuous_cyber_never_proves_production_superiority")
        ...
        if self.benchmark_superiority_claim_allowed != expected_claim:
...
class ContinuousDefenseCycleReceipt(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_cycle(self) -> ContinuousDefenseCycleReceipt:
        CompanyIdentity.model_validate(self.identity.model_dump(mode="json"))
        if (
            ...
            raise ValueError("continuous_cyber_cycle_never_grants_write_authority")
        _verify(self, "continuous_cyber_cycle_fingerprint_mismatch")
        return self


class LiveThreatFeedClient:
    """Allowlisted, credential-free reader for public cyber intelligence."""
    ...
    def fetch_kev_catalog(
        ...
        *,
        ...
        try:
            payload, http_status, digest = self._get_json(CISA_KEV_CANONICAL_URL)
            ...
            payload, http_status, digest = self._get_json(CISA_KEV_OFFICIAL_MIRROR_URL)
            observation = _feed_observation(
                source=LiveThreatFeedSource.CISA_KEV,
    ...
    def fetch_nvd_cve(
        ...
        *,
        ...
        _cve(cve_id)
        payload, http_status, digest = self._get_json(
            NVD_CVE_API_URL,
            params={"cveId": cve_id},
        )
        observation = _feed_observation(
    ...
    def _get_json(
        ...
        *,
        ...
        return payload, response.status_code, digest
...
def ingest_live_public_threat(
    *,
    ...
    """Create global threat evidence from current public observations."""

    _cve(cve_id)
    _aware(as_of, "cyber_live_ingestion_as_of_requires_timezone")
    nvd_payload, nvd_observation = client.fetch_nvd_cve(
        cve_id=cve_id,
        observed_at=as_of,
    )
    ...
    try:
        ...
        if kev_entry is not None:
            vendor_product_refs = (
                _vendor_product_ref(
                    kev_entry.get("vendorProject"),
                    kev_entry.get("product"),
                ),
            )
            kev_cwes = tuple(
                value.upper()
            ...
            kev_threat = build_threat_record(
                ...
                source_record_id=cve_id,
                published_at=_parse_date_as_datetime(kev_entry.get("dateAdded")),
                recorded_at=as_of,
                source_evidence_ref=(
    ...
    current_epss = False
    try:
        epss_payload, epss_observation = client.fetch_epss(
            cve_id=cve_id,
            observed_at=as_of,
        )
        ...
        if epss_item is not None:
            ...
            epss_age_days = (as_of.date() - score_date).days
            current_epss = 0 <= epss_age_days <= 1
    ...
    return _seal(
        LiveThreatIngestionReceipt,
        {
            ...
            "cve_id": cve_id,
            "as_of": as_of,
            "primary_threat": kev_threat or nvd_threat,
            ...
            "known_exploitation_authority_observed": known_authority,
...
def assess_eay_cve_impact(
    *,
    ...
    ingestion = LiveThreatIngestionReceipt.model_validate(
        ingestion.model_dump(mode="json")
    )
    inventory = EayAssetInventorySnapshot.model_validate(
        inventory.model_dump(mode="json")
    )
    if max_company_evidence_age_seconds <= 0:
        raise ValueError("eay_cve_company_evidence_age_must_be_positive")
    threat_cpes = set(ingestion.nvd_cpe_refs)
    threat_products = set(ingestion.vendor_product_refs)
    matches: list[CveAssetMatch] = []
    exposures: list[CompanyCyberExposure] = []
    priorities: list[DefensivePriorityReceipt] = []

    for asset in inventory.assets:
        exact_cpes = tuple(sorted(set(asset.cpe_refs) & threat_cpes))
        product_matches = tuple(sorted(set(asset.product_refs) & threat_products))
        if not exact_cpes and not product_matches:
            continue
        confirmed = bool(exact_cpes and asset.deployment_observed)
        match_status = (
            CveAssetMatchStatus.CONFIRMED
            if confirmed
            else CveAssetMatchStatus.POTENTIAL
        )
        evidence_refs = tuple(
            dict.fromkeys(
                (
                    *asset.evidence_refs,
                    f"threat:{ingestion.fingerprint[:24]}",
                )
            )
        )
        matches.append(
            CveAssetMatch(
                ...
                status=match_status,
                matched_refs=exact_cpes or product_matches,
                evidence_refs=evidence_refs,
        ...
        exposure = build_company_exposure(
            ...
            status=(
                ...
                if confirmed
                else ExposureStatus.POTENTIALLY_EXPOSED
            ),
    ...
    if any(match.status is CveAssetMatchStatus.CONFIRMED for match in matches):
        ...
        reasons = ["product_or_cpe_match_requires_deployment_confirmation"]
    elif threat_cpes or threat_products:
...
def materialize_eay_attack_graph(
    *,
    ...
    eligible_entries = tuple(
        sorted(ref for ref in set(affected_entry_refs) if ref in node_refs)
    )
    path_set: DefensiveAttackPathSet | None = None
    blast_radius: BlastRadiusAssessment | None = None
    if eligible_entries:
        ...
        blast_radius = assess_blast_radius(snapshot=snapshot, path_set=path_set)
    return _seal(
        EayAttackGraphReceipt,
        {
            "contract": CYBER_CONTINUOUS_DEFENSE_CONTRACT,
...
def build_sigma_rule_metadata(
    *,
    ...
    if not isinstance(logsource, Mapping):
        raise TypeError("sigma_logsource_mapping_required")
    tags_raw = rule.get("tags") or ()
    if not isinstance(tags_raw, (list, tuple)):
        raise TypeError("sigma_tags_sequence_required")
        ...
        sorted(
            {
                ...
                and _ATTACK.fullmatch(tag.split(".", 1)[1])
            }
        )
    )
    cve_ids: set[str] = set()
    for tag in tags:
        ...
        candidate = raw if raw.startswith("CVE-") else "CVE-" + raw.replace(".", "-")
    ...
    return _seal(
        SigmaRuleMetadata,
        {
            ...
            "level": _optional_str(rule.get("level")),
...
def assess_sigma_coverage(
    *,
    ...
    attack_ids = {attack_id.upper() for attack_id in attack_technique_ids}
    relevant = tuple(
        rule
        for rule in rules
        if ingestion.cve_id in rule.cve_ids
        or bool(set(rule.attack_technique_ids) & attack_ids)
    )
    observed_by_key: dict[str, SigmaTelemetryObservation] = {}
    for raw in telemetry:
        observation = SigmaTelemetryObservation.model_validate(
            raw.model_dump(mode="json")
        )
        if observation.identity.fingerprint != identity.fingerprint:
            raise ValueError("sigma_cross_company_telemetry_forbidden")
        if observation.logsource_key in observed_by_key:
        ...
        observed_by_key[observation.logsource_key] = observation
    ...
    for rule in relevant:
        observation = observed_by_key.get(rule.logsource_key)
        if observation is None:
            unverified.append(rule.rule_id)
            continue
        age = (as_of - observation.observed_at).total_seconds()
        if age > max_company_evidence_age_seconds:
            unverified.append(rule.rule_id)
        elif observation.status is SigmaTelemetryStatus.AVAILABLE:
            covered.append(rule.rule_id)
        elif observation.status is SigmaTelemetryStatus.DEGRADED:
    ...
    if not relevant:
        ...
        firm_gap = bool(degraded or missing)
    return _seal(
        SigmaCoverageReceipt,
        {
            "contract": CYBER_CONTINUOUS_DEFENSE_CONTRACT,
...
def triage_company_incident(
    *,
    ...
    if impact.exposures:
        signals.append(
            build_company_security_signal(
                identity=identity,
                signal_id=(
                    f"signal:vulnerability:{ingestion.cve_id}:"
                    f"{impact.fingerprint[:12]}"
                ),
                ...
                exposure_refs=tuple(
                    exposure.exposure_id for exposure in impact.exposures
                ),
    ...
    if not signals:
        return None
    seed = _fingerprint({"impact": impact.fingerprint})[:12]
    return assess_company_incident(
        identity=identity,
        incident_id=f"incident-review:{ingestion.cve_id}:{seed}",
...
def build_defensive_recommendations(
    *,
    ...
    if not impact.exposures:
        return ()
    priorities = {item.exposure_id: item for item in impact.priorities}
    candidates: list[DefensiveResponseCandidate] = []
    seen: set[tuple[str, DefensiveAction]] = set()
    for exposure in impact.exposures:
        priority = priorities[exposure.exposure_id]
        actions = list(priority.recommended_defensive_actions)
        if sigma.status in {
            SigmaCoverageStatus.UNVERIFIED,
            SigmaCoverageStatus.PARTIAL,
        }:
        ...
        for action in actions:
            key = (exposure.exposure_id, action)
            if key in seen:
                continue
            seen.add(key)
            candidates.append(
                build_defensive_response_candidate(
                    ...
                    exposure=exposure,
                    candidate_id=(
                        f"defense:{ingestion.cve_id}:{exposure.asset_ref}:"
                        f"{action.value}"
                    ),
                    action=action,
    ...
    *,
    ...
    checks = (
        ...
        SandboxCheck(
            ...
            passed=(
                ...
                or any(
                    priority.firm_company_exposure_claim_authorized
                    and priority.exposure_claim is CompanyExposureClaim.AFFECTED
                    for priority in impact.priorities
                )
            ),
        ...
        SandboxCheck(
            ...
            evidence_ref=(
                f"incident:{incident.fingerprint[:24]}"
    ...
    external = environment is SandboxEvidenceClass.AUTHORIZED_EXTERNAL
    return _seal(
        ControlledSandboxValidationReceipt,
        {
            "contract": CYBER_CONTINUOUS_DEFENSE_CONTRACT,
            ...
            "qualifies_as_authorized_sandbox_benchmark_evidence": (
                external and passed and bool(authorization_evidence_ref)
            ),
...
def build_continuous_benchmark_checkpoint(
    *,
    ...
    if baseline is not None:
        comparison = compare_cyber_benchmark_runs(
            profile=profile,
            challenger=run,
            baseline=baseline,
        )
    return _seal(
        ContinuousCyberBenchmarkCheckpoint,
        {
            ...
            "benchmark_superiority_claim_allowed": bool(
                ...
                and comparison.benchmark_superiority_claim_allowed
            ),
...
def run_continuous_defense_cycle(
    *,
    ...
    if inventory.identity.fingerprint != identity.fingerprint:
        raise ValueError("continuous_cyber_cross_company_inventory_forbidden")
...
def latest_kev_cve_id(payload: Mapping[str, Any]) -> str:
    ...
    for entry in entries:
        ...
        if not _CVE.fullmatch(cve_id) or not isinstance(date_added, str):
            continue
        try:
...
def _extract_nvd_cve(payload: Mapping[str, Any], cve_id: str) -> Mapping[str, Any]:
    ...
    for wrapper in vulnerabilities:
        ...
        if not isinstance(cve, Mapping):
            continue
        if str(cve.get("id") or "").upper() == cve_id.upper():
            return cve
...
def _find_kev_entry(
    ...
    for entry in entries:
        if not isinstance(entry, Mapping):
            continue
        if str(entry.get("cveID") or "").upper() == cve_id.upper():
            return entry
    return None


def _extract_epss_item(
    payload: Mapping[str, Any],
    cve_id: str,
) -> Mapping[str, Any] | None:
    ...
    for item in data:
        if not isinstance(item, Mapping):
            continue
        if str(item.get("cve") or "").upper() == cve_id.upper():
            return item
    return None
...
def _vendor_product_ref(vendor: Any, product: Any) -> str:
    return (
        "vendor-product:"
        f"{_normalize_product_part(vendor)}:{_normalize_product_part(product)}"
    )
...
def _seal(model_class: type[BaseModel], values: Mapping[str, Any]):
    draft = model_class.model_construct(**dict(values), fingerprint="0" * 64)
