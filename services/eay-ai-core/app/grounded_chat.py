...
from .main import ChatRequest, ChatAnswer, ollama, settings, store, _format_evidence, ANSWER_SCHEMA, SYSTEM_PROMPT, validate_citations
...
def _provenance_for_evidence(evidence_ids: list[str]) -> list[ProvenanceItem]:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        legal_chunks = {
            ...
            for row in conn.execute(
                f"SELECT * FROM legal_knowledge_chunks WHERE doc_id IN ({placeholders})",
                evidence_ids,
            ).fetchall()
    ...
    for doc in docs:
        ...
        if legal:
            result.append(
                ProvenanceItem(
                    id=doc["id"], layer="legal", source_id=legal["instrument_id"],
                    verification_id=legal["verification_id"], version=doc["version"],
                    ...
                    source_url=doc["source_url"], content_sha256=legal["content_sha256"],
        ...
        result.append(
            ProvenanceItem(
                id=doc["id"], layer=doc["layer"], version=doc["version"],
                effective_from=date.fromisoformat(doc["effective_from"]) if doc["effective_from"] else None,
                effective_to=date.fromisoformat(doc["effective_to"]) if doc["effective_to"] else None,
                source_url=doc["source_url"],
            )
...
async def grounded_chat(request: ChatRequest):
    evidence = store.search(request.message, request.as_of, request.layers, settings.top_k)
    prompt = f"""
...
""".strip()
    try:
        parsed, raw = await ollama.chat_json(system=SYSTEM_PROMPT, user=prompt, schema=ANSWER_SCHEMA)
    except Exception as exc:
        raise HTTPException(status_code=503, detail=f"Local model unavailable or invalid response: {exc}") from exc
    ...
    has_legal = any(item.layer == "legal" and item.authority_level == "binding" for item in evidence)
    answer = validate_citations(answer, valid_ids, has_legal)
    store.save_interaction(
        interaction_id=interaction_id, request=request, model=settings.model,
        model_answer=answer.model_dump_json(), evidence=evidence, confidence=answer.confidence,
    )
    if answer.confidence < settings.low_confidence_threshold:
        store.create_low_confidence_candidate(interaction_id)

    conflicts = [item.model_dump() for item in legal_engine.compare_company_to_law(request.as_of)]
    provenance = _provenance_for_evidence([item.id for item in evidence])
    return GroundedChatAnswer(response=answer, provenance=provenance, conflicts=conflicts)
