...
3. invoke that exact local engine with no paid-token ledger activity;
4. only when no qualified local specialist exists, enter the existing
...
benchmark quality and never grants execution authority. Paid escalation remains subject
to the exact user/tenant/provider/model grant and billing controls already implemented
by ProductionEngineRuntime/AdminGovernedEngineGateway.
"""
...
class LocalFirstProductionRuntime:
    ...
    async def invoke_primary(
        ...
        *,
        ...
        selection = self._select_local(local_task=local_task, context=context)
