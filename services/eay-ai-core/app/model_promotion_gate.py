...
def offline_eval_fingerprint(model: sqlite3.Row) -> str:
    """Deterministically bind offline eval evidence to the exact model/artifact lineage."""
...
class ModelPromotionGate:
    ...
    def promote(self, payload: PromotionRequest) -> PromotionRecord:
        ...
        artifact_provenance_fingerprint = model["artifact_provenance_fingerprint"]
        if artifact_provenance_fingerprint != primary_artifact.provenance_fingerprint:
        ...
        with sqlite3.connect(self.db_path) as conn:
            try:
                ...
                current = conn.execute(
                    "SELECT * FROM model_registry WHERE id=?", (payload.model_record_id,)
                ).fetchone()
                if current is None:
                    raise KeyError("model_not_found")
                if current["status"] != "canary":
                    raise ValueError("production_promotion_stale_model_state")
                if current["artifact_sha256"] != model["artifact_sha256"]:
                    raise ValueError("production_promotion_stale_model_state")
