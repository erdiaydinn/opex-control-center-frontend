...
class ReleasePolicy:
    ...
    # Legal temporal correctness is a hard promotion gate. The corpus can start
    # relatively small, but every curated historical case must pass before rollout.
...
def evaluate_release(
    ...
    *,
    ...
    if historical_legal is None:
        ...
        if (
            historical_legal.source_match_rate
            < ReleasePolicy.min_historical_source_match_rate
        ):
            violations.append("historical_legal_source_match_failed")
        if (
            historical_legal.fingerprint_validity_rate
            < ReleasePolicy.min_historical_fingerprint_validity_rate
        ):
            violations.append("historical_legal_fingerprint_invalid")
        if (
            historical_legal.inactive_legal_leak_rate
            > ReleasePolicy.max_inactive_legal_leak_rate
        ):
            violations.append("inactive_legal_source_leak_detected")
        if (
            historical_legal.temporal_block_bypass_rate
            > ReleasePolicy.max_temporal_block_bypass_rate
        ):
