...
class VoiceAsyncExecutionCoordinator:
    ...
    def __init__(self, *, controller: VoiceRealtimeSessionController, ledger: VoiceSessionLedger | None = None) -> None:
    ...
    def cancel_for_barge_in(self) -> tuple[str, ...]:
        ...
        if self.ledger is not None:
            self.ledger.append(session_id=self.controller.streaming.session_id, event_type="interrupted", language=self.controller.streaming.language, metadata={"cancelled_task_ids_sha256": _sha256(cancelled_ids), "new_turn_epoch": self._turn_epoch})
    ...
    def accept_result(self, *, lease: TaskLease, result_sha256: str, governed_provenance_fingerprint: str | None = None) -> AcceptedAsyncResult:
        ...
        payload = {"task_id": lease.task_id, "kind": lease.kind, "turn_epoch": lease.turn_epoch, "result_sha256": result_sha256, "request_fingerprint": lease.request_fingerprint, "governed_provenance_fingerprint": governed_provenance_fingerprint}
    ...
    def seal_response_generation(self, *, user_input_sha256: str, deployment_manifest_fingerprint: str, model_execution_identity: VoiceModelExecutionIdentity, tool_task_ids: tuple[str, ...] = (), legal_context_fingerprint: str | None = None, kpi_context_fingerprint: str | None = None) -> VoiceResponseGenerationProof:
        tool_results = tuple(self.accepted_result_for(task_id) for task_id in tool_task_ids)
        proof = seal_response_generation_proof(session_id=self.controller.streaming.session_id, turn_epoch=self._turn_epoch, user_input_sha256=user_input_sha256, deployment_manifest_fingerprint=deployment_manifest_fingerprint, model_execution_identity=model_execution_identity, accepted_tool_results=tool_results, legal_context_fingerprint=legal_context_fingerprint, kpi_context_fingerprint=kpi_context_fingerprint)
        if self.ledger is not None:
            self.ledger.append(session_id=self.controller.streaming.session_id, event_type="response_proof", language=self.controller.streaming.language, metadata={"turn_epoch": self._turn_epoch, "response_proof_fingerprint": proof.fingerprint, "deployment_manifest_fingerprint": deployment_manifest_fingerprint, "tool_result_count": len(tool_results), "model_execution_identity_fingerprint": model_execution_identity.fingerprint, "model_artifact_sha256": model_execution_identity.artifact_sha256})
        return proof
