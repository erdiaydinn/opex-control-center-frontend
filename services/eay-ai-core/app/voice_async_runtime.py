...
class VoiceAsyncExecutionCoordinator:
    ...
    def _assert_deployment_fresh(self) -> None:
        ...
        if pinned is None:
            return
        if self.deployment_freshness_check is None:
            return
    ...
    def start_task(self, *, task_id: str, kind: str, request_fingerprint: str, cancellable: bool = True) -> tuple[TaskLease, CancellationToken]:
        self._assert_deployment_fresh()
        ...
        lease = TaskLease(task_id=task_id, kind=kind, turn_epoch=self._turn_epoch, request_fingerprint=request_fingerprint)
    ...
    def cancel_for_barge_in(self) -> tuple[str, ...]:
        ...
        if self.ledger is not None:
            self.ledger.append(session_id=self.controller.streaming.session_id, event_type="interrupted", language=self.controller.streaming.language, metadata={"cancelled_task_ids_sha256": _sha256(cancelled_ids), "new_turn_epoch": self._turn_epoch, "deployment_manifest_fingerprint": self.deployment_manifest_fingerprint})
        return cancelled_ids

    def accept_result(self, *, lease: TaskLease, result_sha256: str, governed_provenance_fingerprint: str | None = None) -> AcceptedAsyncResult:
        self._assert_deployment_fresh()
        ...
        payload = {"task_id": lease.task_id, "kind": lease.kind, "turn_epoch": lease.turn_epoch, "result_sha256": result_sha256, "request_fingerprint": lease.request_fingerprint, "governed_provenance_fingerprint": governed_provenance_fingerprint, "deployment_manifest_fingerprint": self.deployment_manifest_fingerprint}
        accepted = AcceptedAsyncResult(**{key: value for key, value in payload.items() if key != "deployment_manifest_fingerprint"}, fingerprint=_sha256(payload))
    ...
    def seal_response_generation(self, *, user_input_sha256: str, input_lineage_fingerprint: str, deployment_manifest_fingerprint: str, model_execution_identity: VoiceModelExecutionIdentity, tool_task_ids: tuple[str, ...] = (), legal_context_fingerprint: str | None = None, kpi_context_fingerprint: str | None = None) -> VoiceResponseGenerationProof:
        self._assert_deployment_fresh()
        ...
        proof = seal_response_generation_proof(session_id=self.controller.streaming.session_id, turn_epoch=self._turn_epoch, user_input_sha256=user_input_sha256, input_lineage_fingerprint=input_lineage_fingerprint, deployment_manifest_fingerprint=deployment_manifest_fingerprint, model_execution_identity=model_execution_identity, accepted_tool_results=tool_results, legal_context_fingerprint=legal_context_fingerprint, kpi_context_fingerprint=kpi_context_fingerprint)
        if self.ledger is not None:
            self.ledger.append(session_id=self.controller.streaming.session_id, event_type="response_proof", language=self.controller.streaming.language, metadata={"turn_epoch": self._turn_epoch, "response_proof_fingerprint": proof.fingerprint, "input_lineage_fingerprint": input_lineage_fingerprint, "deployment_manifest_fingerprint": deployment_manifest_fingerprint, "tool_result_count": len(tool_results), "model_execution_identity_fingerprint": model_execution_identity.fingerprint, "model_artifact_sha256": model_execution_identity.artifact_sha256})
        return proof

    def seal_tts_generation(self, *, response_proof: VoiceResponseGenerationProof, deployment_manifest_fingerprint: str, response_text_sha256: str, voice_profile_fingerprint: str, tts_execution_identity: VoiceTtsExecutionIdentity) -> VoiceTtsGenerationProof:
        self._assert_deployment_fresh()
        if self.deployment_manifest_fingerprint is not None and deployment_manifest_fingerprint != self.deployment_manifest_fingerprint:
            raise ValueError("voice_async_tts_deployment_manifest_mismatch")
        proof = seal_tts_generation_proof(response_proof=response_proof, current_turn_epoch=self._turn_epoch, deployment_manifest_fingerprint=deployment_manifest_fingerprint, response_text_sha256=response_text_sha256, voice_profile_fingerprint=voice_profile_fingerprint, tts_execution_identity=tts_execution_identity)
        if self.ledger is not None:
            self.ledger.append(session_id=self.controller.streaming.session_id, event_type="tts_proof", language=self.controller.streaming.language, metadata={"turn_epoch": self._turn_epoch, "response_proof_fingerprint": response_proof.fingerprint, "tts_proof_fingerprint": proof.fingerprint, "deployment_manifest_fingerprint": deployment_manifest_fingerprint, "tts_execution_identity_fingerprint": tts_execution_identity.fingerprint, "tts_adapter_artifact_sha256": tts_execution_identity.artifact_sha256, "tts_adapter_promotion_fingerprint": tts_execution_identity.promotion_fingerprint})
        return proof
    ...
    def authorize_tool_execution(self, *, intent: VoiceToolIntent, approval_token: str | None = None) -> None:
        ...
        self.controller.approvals.consume(token=approval_token, session_id=intent.session_id, tool_call_id=intent.tool_call_id, risk=intent.risk, intent_fingerprint=intent.fingerprint)

    async def execute_tool(self, *, intent: VoiceToolIntent, approval_token: str | None, adapter: AsyncToolAdapter, lease: TaskLease, cancellation: CancellationToken) -> AcceptedAsyncResult:
        self._assert_deployment_fresh()
        ...
        accepted = self.accept_result(lease=lease, result_sha256=result_sha, governed_provenance_fingerprint=governed_result.execution_proof.fingerprint)
        if self.ledger is not None:
            self.ledger.append(session_id=intent.session_id, event_type="tool_result", language=intent.language, action_risk=intent.risk, tool_call_id=intent.tool_call_id, approval_reference=intent.approval_reference, metadata={"intent_fingerprint": intent.fingerprint, "result_sha256": accepted.result_sha256, "governed_execution_provenance_fingerprint": accepted.governed_provenance_fingerprint, "accepted_result_fingerprint": accepted.fingerprint, "turn_epoch": accepted.turn_epoch, "deployment_manifest_fingerprint": self.deployment_manifest_fingerprint})
        return accepted
