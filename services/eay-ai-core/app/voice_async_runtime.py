...
class VoiceAsyncExecutionCoordinator:
    ...
    def seal_response_generation(self, *, user_input_sha256: str, model_execution_identity: VoiceModelExecutionIdentity, tool_task_ids: tuple[str, ...] = (), legal_context_fingerprint: str | None = None, kpi_context_fingerprint: str | None = None) -> VoiceResponseGenerationProof:
        tool_results = tuple(self.accepted_result_for(task_id) for task_id in tool_task_ids)
        proof = seal_response_generation_proof(session_id=self.controller.streaming.session_id, turn_epoch=self._turn_epoch, user_input_sha256=user_input_sha256, model_execution_identity=model_execution_identity, accepted_tool_results=tool_results, legal_context_fingerprint=legal_context_fingerprint, kpi_context_fingerprint=kpi_context_fingerprint)
        if self.ledger is not None:
            self.ledger.append(session_id=self.controller.streaming.session_id, event_type="response_proof", language=self.controller.streaming.language, metadata={"turn_epoch": self._turn_epoch, "response_proof_fingerprint": proof.fingerprint, "tool_result_count": len(tool_results), "model_execution_identity_fingerprint": model_execution_identity.fingerprint, "model_artifact_sha256": model_execution_identity.artifact_sha256})
        return proof

    def seal_tts_generation(self, *, response_proof: VoiceResponseGenerationProof, response_text_sha256: str, voice_profile_fingerprint: str, tts_execution_identity: VoiceTtsExecutionIdentity) -> VoiceTtsGenerationProof:
        proof = seal_tts_generation_proof(response_proof=response_proof, current_turn_epoch=self._turn_epoch, response_text_sha256=response_text_sha256, voice_profile_fingerprint=voice_profile_fingerprint, tts_execution_identity=tts_execution_identity)
        if self.ledger is not None:
            self.ledger.append(session_id=self.controller.streaming.session_id, event_type="tts_proof", language=self.controller.streaming.language, metadata={"turn_epoch": self._turn_epoch, "response_proof_fingerprint": response_proof.fingerprint, "tts_proof_fingerprint": proof.fingerprint, "tts_execution_identity_fingerprint": tts_execution_identity.fingerprint, "tts_adapter_artifact_sha256": tts_execution_identity.artifact_sha256, "tts_adapter_promotion_fingerprint": tts_execution_identity.promotion_fingerprint})
        return proof
