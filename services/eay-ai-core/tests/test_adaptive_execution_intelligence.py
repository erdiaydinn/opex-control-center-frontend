...
def oidc() -> OidcMetadataSnapshot:
    return OidcMetadataSnapshot(
        ...
        redirect_uris=frozenset(
            {"https://jarvis.acme.example/callback"}
        ),
        policy_fingerprint="policy-v1",
    )


def test_trusted_oidc_endpoint_drift_becomes_sandbox_candidate(
    scope,
    oidc,
):
    ...
    current = replace(
        oidc,
        authorization_endpoint=(
            "https://acme.okta.com/oauth2/default/v2/authorize"
        ),
...
def test_jwks_rotation_is_dynamic_revalidation_candidate(scope, oidc):
    ...
    current = replace(
        oidc,
        jwks_uri="https://acme.okta.com/oauth2/default/v2/keys",
    )
    proposal = planner.assess_oidc_drift(oidc, current)
    ...
    assert any(
        "signature" in step
        for step in proposal.verification_requirements
    )
...
def test_issuer_change_holds(scope, oidc):
    current = replace(
        oidc,
        issuer="https://evil.example/oauth2/default",
    )
    proposal = AdaptiveExecutionRepairPlanner(scope).assess_oidc_drift(
        oidc,
        current,
...
def test_http_downgrade_holds(scope, oidc):
    current = replace(
        oidc,
        token_endpoint="http://acme.okta.com/oauth2/default/v1/token",
    )
    proposal = AdaptiveExecutionRepairPlanner(scope).assess_oidc_drift(
        oidc,
        current,
...
def test_malformed_endpoint_holds_instead_of_raising(scope, oidc):
    current = replace(
        oidc,
        token_endpoint="https://acme.okta.com:bad/token",
    )
    proposal = AdaptiveExecutionRepairPlanner(scope).assess_oidc_drift(
        oidc,
        current,
...
def test_scope_expansion_holds(scope, oidc):
    current = replace(
        oidc,
        scopes=oidc.scopes | {"reports.write"},
    )
    proposal = AdaptiveExecutionRepairPlanner(scope).assess_oidc_drift(
        oidc,
        current,
...
def test_new_mfa_or_consent_holds_instead_of_bypass(scope, oidc):
    planner = AdaptiveExecutionRepairPlanner(scope)
    mfa = planner.assess_oidc_drift(
        oidc,
        replace(oidc, mfa_required=True),
    )
    consent = planner.assess_oidc_drift(
        oidc,
        replace(oidc, consent_required=True),
    )
...
def test_unknown_redirect_origin_holds(scope, oidc):
    current = replace(
        oidc,
        redirect_uris=frozenset({"https://unknown.example/callback"}),
    )
    proposal = AdaptiveExecutionRepairPlanner(scope).assess_oidc_drift(
        oidc,
        current,
...
def test_new_required_api_input_holds(scope):
    previous = api_snapshot()
    current = api_snapshot(
        request_required_fields=frozenset({"date", "region"})
    )
    proposal = AdaptiveExecutionRepairPlanner(scope).assess_api_drift(
        previous,
        current,
...
def test_missing_required_api_output_holds(scope):
    previous = api_snapshot()
    current = api_snapshot(
        response_required_fields=frozenset({"report_id"})
    )
    proposal = AdaptiveExecutionRepairPlanner(scope).assess_api_drift(
        previous,
        current,
...
def test_api_privilege_expansion_holds(scope):
    previous = api_snapshot()
    current = api_snapshot(
        required_scopes=frozenset({"reports.read", "reports.write"})
    )
    proposal = AdaptiveExecutionRepairPlanner(scope).assess_api_drift(
        previous,
        current,
...
def test_semantic_ui_relocation_can_be_repaired_with_outcome_verification(
    scope,
):
    ...
    proposal = AdaptiveExecutionRepairPlanner(scope).assess_ui_drift(
        previous,
        current,
    )
...
def test_ui_context_change_requires_review(scope):
    ...
    current = replace(
        previous,
        context_fingerprint="admin-delete-page",
    )
    proposal = AdaptiveExecutionRepairPlanner(scope).assess_ui_drift(
        previous,
        current,
...
def test_cross_tenant_repair_is_forbidden(scope):
    ...
    with pytest.raises(ValueError, match="cross-tenant/company"):
        AdaptiveExecutionRepairPlanner(scope).assess_api_drift(
            previous,
            current,
        )
