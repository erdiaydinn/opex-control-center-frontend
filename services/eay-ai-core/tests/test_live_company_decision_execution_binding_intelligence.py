...
from app.live_company_readiness import DecisionTruthReceipt, DecisionTruthStatus
...
def test_proceed_truth_receipt_is_bound_into_reasoning_evidence():
    ...
    summary = asyncio.run(
        execute_mission_until_blocked(
            ...
            capability_handlers={},
            decision_truth_receipts={
                "inventory-live": _truth(DecisionTruthStatus.PROCEED, firm=True)
            },
        )
    )
    ...
    assert "live-truth-readiness://YS_TR/inventory-live/proceed" in evidence_refs
