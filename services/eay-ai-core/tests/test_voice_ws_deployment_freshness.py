...
def test_websocket_rejects_task_result_if_deployment_changes_mid_task():
    ...
    try:
        with client.websocket_connect("/v1/voice/ws/session-drift?language=tr") as ws:
            ws.receive_json()
            ws.send_json({"event": "wake", "sequence": 0, "message_id": "m0", "payload": {}})
            ws.receive_json()
            ws.send_json({"event": "stt_final", "sequence": 1, "message_id": "m1", "payload": {"text_sha256": _hash("a")}})
            ws.receive_json()
            ws.send_json({"event": "task_start", "sequence": 2, "message_id": "m2", "payload": {"task_id": "tool-1", "kind": "tool", "request_fingerprint": _hash("b")}})
            assert ws.receive_json()["event"] == "task_started"

            configure_voice_deployment_bindings(_bindings(_hash("9")))
            ws.send_json({"event": "task_result", "sequence": 3, "message_id": "m3", "payload": {"task_id": "tool-1", "result_sha256": _hash("c"), "governed_provenance_fingerprint": _hash("d")}})
...
def test_websocket_response_proof_carries_exact_microphone_to_stt_lineage():
    ...
    try:
        with client.websocket_connect("/v1/voice/ws/session-input-lineage?language=tr") as ws:
            ...
            ws.send_json({"event": "audio_frame", "sequence": 1, "message_id": "m1", "payload": {"frame_sequence": 0, "pcm_sha256": _hash("e"), "duration_ms": 20, "sample_rate_hz": 16000}})
            frame = ws.receive_json()
            assert len(frame["audio_frame_proof_fingerprint"]) == 64
            ws.send_json({"event": "stt_final", "sequence": 2, "message_id": "m2", "payload": {"text_sha256": _hash("a")}})
            stt = ws.receive_json()
            ...
            ws.send_json({"event": "response_start", "sequence": 3, "message_id": "m3", "payload": {"task_id": "model-1", "user_input_sha256": _hash("a")}})
