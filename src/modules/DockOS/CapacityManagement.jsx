...
export default function CapacityManagement() {
  ...
  function addGeneratedSlots() {
    ...
    const duration = Number(newSlotDuration);
    const count = Number(newSlotCount);
    if (start === null || duration < 15 || duration > 720 || count < 1 || count > 24 || duration * count > 1440) {
    ...
    setSelectedHours(sortSlots(generated));
      ...
      <section style={styles.panel}>
        ...
        {capacityMode && <div style={styles.slotBuilder}>
          ...
          <div style={styles.customSlotGrid}>
            ...
            <FieldInput label={t("slotDuration")} type="number" min="15" max="720" step="5" value={newSlotDuration} onChange={(event) => setNewSlotDuration(event.target.value)} />
          ...
          {customSlotPreview.length > 0 && <div style={styles.customPreview}><strong>{t("customBlockPreview")}</strong><span>{customSlotPreview.join(" · ")}</span><small>{Number(newSlotDuration)} {t("minutesEachBlock")}</small></div>}
...
const styles = {
  ...
  hourPickerHead: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }, hourGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(118px,1fr))", gap: 8 }, hourButton: { minHeight: 38, padding: "8px 10px", border: "1px solid var(--dockos-border)", borderRadius: 10, color: "var(--dockos-text)", background: "var(--dockos-surface)", fontWeight: 800, cursor: "pointer" }, hourButtonActive: { minHeight: 38, padding: "8px 10px", border: "2px solid #e5005a", borderRadius: 10, color: "var(--dockos-info-text)", background: "var(--dockos-accent-soft-bg)", fontWeight: 900, cursor: "pointer" }, customPreview: { display: "grid", gridTemplateColumns: "auto 1fr auto", gap: 10, alignItems: "center", padding: "11px 13px", border: "1px solid #84adff", borderRadius: 11, color: "var(--dockos-info-text)", background: "var(--dockos-info-bg)" },
