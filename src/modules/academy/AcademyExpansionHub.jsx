...
import { ArrowLeft, Award, GitBranch, Languages, LoaderCircle, RefreshCw } from "lucide-react";
...
export default function AcademyExpansionHub() {
  ...
  const [tab, setTab] = useState(canStudio ? "scenario" : "achievements");
  ...
  const tabs = useMemo(() => {
    const items = [["achievements", tx("achievements"), Award]];
    if (canStudio) {
  ...
  return (
    <main className="eay-academy-page">
      ...
      <section className="eay-academy-main" aria-busy={loading ? "true" : "false"}>
        <header className="eay-academy-header"><div><span>EAY · {t("academy")}</span><h1>{tabs.find(([key]) => key === tab)?.[1] || t("academy")}</h1><p>{tab === "scenario" ? tx("scenarioHint") : tab === "localization" ? tx("localizationHint") : tx("achievementsHint")}</p></div>{canStudio ? <button type="button" className="eay-academy-refresh" onClick={load} disabled={loading}><RefreshCw className={loading ? "spin" : ""} size={17} aria-hidden="true" /><span>{tx("refresh")}</span></button> : null}</header>
        ...
        {!loading && !error ? <div className="eay-academy-view" data-eay-product-state="ready">{tab === "scenario" && canStudio ? <AcademyScenarioStudio workspace={workspace} locale={locale} t={t} refresh={load} /> : null}{tab === "localization" && canStudio ? <AcademyLocalizationGovernance workspace={workspace} locale={locale} t={t} /> : null}{tab === "achievements" ? <AcademyAchievements locale={locale} t={t} formatDate={formatDate} /> : null}</div> : null}
      </section>
    </main>
  );
}
