...
function numericValue(value, fallback) { const next = Number(value); return Number.isFinite(next) ? next : fallback; }

function architectureSignature(candidate) {
  const architecture = candidate?.store_dna?.architecture;
  if (!architecture) return "none";
  const rows = (architecture.elements || []).map((row) => [row.element_id, row.element_type, row.center_x_m, row.center_y_m, row.x_m, row.y_m, row.width_m, row.depth_m, row.rotation_deg, row.clearance_m, Boolean(row.locked), row.scan_source_element_id]);
  return JSON.stringify([candidate?.store_code || candidate?.store_dna?.store_code || null, architecture.source_ref || null, architecture.source_review_fingerprint || null, architecture.floor_width_m, architecture.floor_depth_m, rows]);
}

function elementFromProjection(node, baseElement = null) {
  return { ...(baseElement || {}), element_id: node.nodeId, element_type: node.nodeType, center_x_m: node.geometry.centerXM, center_y_m: node.geometry.centerYM, width_m: node.geometry.widthM, depth_m: node.geometry.depthM, height_m: node.geometry.heightM, rotation_deg: node.geometry.rotationDeg, clearance_m: node.metadata?.clearanceM || 0, locked: Boolean(node.locked), provenance: node.provenance || {} };
}
...
export default function PlanogramArchitecturalAuthoring({ candidate, locale, canEdit, onCandidateChange }) {
  ...
  const formatMetric = useMemo(() => { const formatter = new Intl.NumberFormat(locale || "en", { maximumFractionDigits: 2 }); return (value) => formatter.format(Number(value || 0)); }, [locale]);
  ...
  const applySession = useCallback((nextSession) => {
    ...
    setSession(nextSession); setCommandError(""); setAppliedPulse(true);
    window.clearTimeout(pulseTimerRef.current); pulseTimerRef.current = window.setTimeout(() => setAppliedPulse(false), 1100);
    setSelectedIds((current) => current.filter((id) => nextSession.scene.nodes.some((node) => node.nodeId === id)));
    if (typeof onCandidateChange === "function" && nextSession.candidate) { emittedSignatureRef.current = architectureSignature(nextSession.candidate); onCandidateChange(nextSession.candidate); }
  }, [onCandidateChange]);
  ...
  const executeCommand = useCallback((rawCommand) => {
    ...
    try { const next = executePlanogramCadSessionCommand(session, command); if (next !== session) applySession(next); return next; }
    catch { setCommandError(t("commandRejected")); return null; }
  ...
  const handleFloorPointerDown = useCallback((event) => {
    ...
    const point = modelPoint(event, svgRef.current, projection, document); if (!point) return;
    const created = createPlanogramAuthoringElement({ type: tool, centerXM: point[0], centerYM: point[1], floor: document.floor, sequence: sequenceRef.current, gridM: document.gridM }); sequenceRef.current += 1;
    ...
    const next = executeCommand({ type: "CREATE_NODE", node }); if (next) { setSelectedIds([node.nodeId]); setTool("select"); }
  ...
  const beginDrag = useCallback((event, element) => {
    ...
    if (modifier) { setSelectedIds((current) => current.includes(element.element_id) ? current.filter((id) => id !== element.element_id) : [...current, element.element_id]); return; }
  ...
  const moveDrag = useCallback((event) => {
    const drag = dragRef.current; if (!drag || !document || !projection || !canEdit) return;
    const point = modelPoint(event, svgRef.current, projection, document); if (!point) return;
    const deltaX = snapPlanogramAuthoringValue(point[0] - drag.startPoint[0], document.gridM);
    const deltaY = snapPlanogramAuthoringValue(point[1] - drag.startPoint[1], document.gridM);
    drag.deltaX = deltaX; drag.deltaY = deltaY;
    const preview = {}; for (const row of drag.rows) preview[row.id] = { centerXM: row.centerXM + deltaX, centerYM: row.centerYM + deltaY }; setDragPreview(preview);
  }, [canEdit, document, projection]);

  const endDrag = useCallback((event) => {
    const drag = dragRef.current; dragRef.current = null; setDragPreview({});
  ...
  const deletePrimary = useCallback(() => { if (primarySelected && !primarySelected.locked && canEdit && executeCommand({ type: "DELETE_NODE", nodeId: primarySelected.element_id })) setSelectedIds((current) => current.filter((id) => id !== primarySelected.element_id)); }, [canEdit, executeCommand, primarySelected]);
  ...
  const ensureGroupEditable = useCallback(() => { const rows = selectionRows(); if (rows.some((row) => row.locked)) { setCommandError(t("groupLocked")); return null; } return rows; }, [selectionRows, t]);
  const alignSelection = useCallback((axis) => { const rows = ensureGroupEditable(); if (!rows || rows.length < 2 || !primarySelected) return; const field = axis === "x" ? "centerXM" : "centerYM"; const target = axis === "x" ? primarySelected.center_x_m : primarySelected.center_y_m; executeCommand({ type: "UPDATE_NODES", updates: rows.map((row) => ({ nodeId: row.element_id, patch: { geometry: { [field]: target } } })) }); }, [ensureGroupEditable, executeCommand, primarySelected]);
  const nudgeSelection = useCallback((deltaX, deltaY) => { const rows = ensureGroupEditable(); if (!rows?.length) return; executeCommand({ type: "UPDATE_NODES", updates: rows.map((row) => ({ nodeId: row.element_id, patch: { geometry: { centerXM: row.center_x_m + deltaX, centerYM: row.center_y_m + deltaY } } })) }); }, [ensureGroupEditable, executeCommand]);
  ...
  const handleKeyboardShortcut = useCallback((event) => {
    ...
    const tagName = String(event.target?.tagName || "").toUpperCase(); if (["INPUT", "TEXTAREA", "SELECT"].includes(tagName)) return;
  ...
  const diagnostics = session.diagnostics; const hasDiagnostics = diagnostics.collisionCount + diagnostics.aisleViolationCount + diagnostics.boundaryViolationCount > 0; const gridStepPx = Math.max(8, document.gridM * projection.scale * 10);
  return (
    <section className="eay-authoring" data-preview-only={session.previewOnly ? "true" : "false"} data-cad-session-contract={session.contract} data-store-scene-revision={session.scene.revision} data-selection-count={selectedIds.length} onKeyDown={handleKeyboardShortcut}>
      ...
      <div className="eay-authoring-status"><span><Grid3X3 size={15} aria-hidden="true" />{t("grid")}: {document.gridM} m</span><span><BoxSelect size={15} aria-hidden="true" />{t("elements")}: {renderedElements.length}</span><span>{t("revision")}: {session.scene.revision}</span><span>{t("history")}: {session.historyDepth} / {session.redoDepth}</span><span>{t("selectedCount")}: {selectedIds.length}</span><span>{t("source")}: {document.architecture.source || document.sourceContract}</span>{appliedPulse ? <strong role="status">{t("applied")}</strong> : null}</div>
      ...
      {selectedIds.length > 1 ? <div className="eay-authoring-selection-controls" aria-label={t("selection")}><strong>{t("selection")}: {selectedIds.length}</strong><button type="button" disabled={!canEdit} onClick={() => alignSelection("x")}><AlignCenterVertical size={16} aria-hidden="true" />{t("alignX")}</button><button type="button" disabled={!canEdit} onClick={() => alignSelection("y")}><AlignCenterHorizontal size={16} aria-hidden="true" />{t("alignY")}</button><button type="button" onClick={() => setSelectedIds([])}><X size={16} aria-hidden="true" />{t("clearSelection")}</button></div> : null}
      ...
      <div className="eay-authoring-workspace">
        ...
        <aside className="eay-authoring-inspector"><h3>{t("architectureInspector")}</h3><fieldset><legend>{t("floor")}</legend><label><span>{t("width")}</span><input type="number" min="1" step="0.05" value={document.floor.widthM} disabled={!canEdit} onChange={(event) => resizeFloor(numericValue(event.target.value, document.floor.widthM), document.floor.depthM)} /></label><label><span>{t("depth")}</span><input type="number" min="1" step="0.05" value={document.floor.depthM} disabled={!canEdit} onChange={(event) => resizeFloor(document.floor.widthM, numericValue(event.target.value, document.floor.depthM))} /></label></fieldset>
          {primarySelected ? <fieldset><legend>{t("exactDimensions")}</legend><code>{primarySelected.element_id}</code><div className="eay-authoring-lock-state" data-locked={primarySelected.locked ? "true" : "false"}>{primarySelected.locked ? <LockKeyhole size={15} aria-hidden="true" /> : <UnlockKeyhole size={15} aria-hidden="true" />}<strong>{primarySelected.locked ? t("locked") : t("unlocked")}</strong></div><label><span>{t("x")}</span><input type="number" step="0.05" value={primarySelected.center_x_m} disabled={!canEdit || primarySelected.locked} onChange={(event) => updateSelectedField("centerXM", numericValue(event.target.value, primarySelected.center_x_m))} /></label><label><span>{t("y")}</span><input type="number" step="0.05" value={primarySelected.center_y_m} disabled={!canEdit || primarySelected.locked} onChange={(event) => updateSelectedField("centerYM", numericValue(event.target.value, primarySelected.center_y_m))} /></label><label><span>{t("width")}</span><input type="number" min="0.05" step="0.05" value={primarySelected.width_m} disabled={!canEdit || primarySelected.locked} onChange={(event) => updateSelectedField("widthM", numericValue(event.target.value, primarySelected.width_m))} /></label><label><span>{t("depth")}</span><input type="number" min="0.05" step="0.05" value={primarySelected.depth_m} disabled={!canEdit || primarySelected.locked} onChange={(event) => updateSelectedField("depthM", numericValue(event.target.value, primarySelected.depth_m))} /></label><label><span>{t("rotation")}</span><input type="number" step="1" value={primarySelected.rotation_deg} disabled={!canEdit || primarySelected.locked} onChange={(event) => updateSelectedField("rotationDeg", numericValue(event.target.value, primarySelected.rotation_deg))} /></label><label><span>{t("clearance")}</span><input type="number" min="0" step="0.05" value={primarySelected.clearance_m || 0} disabled={!canEdit || primarySelected.locked} onChange={(event) => updateSelectedField("clearanceM", numericValue(event.target.value, primarySelected.clearance_m || 0))} /></label><button type="button" className="eay-authoring-lock" disabled={!canEdit} onClick={togglePrimaryLock}>{primarySelected.locked ? <UnlockKeyhole size={16} aria-hidden="true" /> : <LockKeyhole size={16} aria-hidden="true" />}{primarySelected.locked ? t("unlockElement") : t("lockElement")}</button><button type="button" className="eay-authoring-delete" disabled={!canEdit || primarySelected.locked} onClick={deletePrimary}><Trash2 size={16} aria-hidden="true" />{t("delete")}</button></fieldset> : <p>{t("selectHint")}</p>}
