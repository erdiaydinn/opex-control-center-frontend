...
function addFallbackFixture(THREE, scene, module, disposables) {
  ...
  const colors = { regular: 0x717b8d, chilled: 0x0891b2, frozen: 0x2563eb, pallet: 0x92400e };
  const geometry = new THREE.BoxGeometry(module.widthM, height, module.depthM);
  const material = new THREE.MeshStandardMaterial({
    color: colors[kind],
    roughness: kind === "pallet" ? 0.88 : 0.5,
    metalness: kind === "pallet" ? 0.02 : 0.16,
    transparent: true,
    opacity: 0.34,
  });
...
function addProducts(THREE, scene, model, productAssets, disposables) {
  const textureLoader = new THREE.TextureLoader();
  const textureMaterials = new Map();
  const planeGeometry = new THREE.PlaneGeometry(1, 1);
  const boxGeometry = new THREE.BoxGeometry(1, 1, 1);
  const boxMaterial = new THREE.MeshStandardMaterial({ color: 0xd6d9e0, roughness: 0.5, metalness: 0.01 });
  disposables.push(planeGeometry, boxGeometry, boxMaterial);
  let productBoxes = 0;
  let texturedFacings = 0;
  ...
  const materialFor = (asset) => {
    ...
    textureMaterials.set(asset.front_image_path, material);
    disposables.push(material);
    textureLoader.load(
      asset.front_image_path,
      (texture) => {
  ...
  outer: for (const module of model.modules) {
    ...
    const left = -module.widthM / 2 + 0.04;
    const right = module.widthM / 2 - 0.04;
      ...
      for (const product of shelf?.products || []) {
        ...
        for (let facing = 0; facing < countFacings(product); facing += 1) {
          ...
          if (material && texturedFacings < MAX_TEXTURED_FACINGS) {
            ...
            plane.position.set(
              module.centerXM + localX * cos - planeLocalZ * sin,
              worldY,
              module.centerYM + localX * sin + planeLocalZ * cos
            );
            plane.rotation.y = rotation + (frontSign < 0 ? Math.PI : 0);
            plane.scale.set(width * 0.92, height * 0.92, 1);
            plane.renderOrder = 3;
            scene.add(plane);
            texturedFacings += 1;
          }
  ...
  useEffect(() => {
    ...
    async function mount() {
      ...
      try {
        ...
        scene.fog = new THREE.Fog(0x080a0f, 18, 55);
        const camera = new THREE.PerspectiveCamera(58, 1, 0.04, 180);
        ...
        scene.add(new THREE.HemisphereLight(0xffffff, 0x111827, 1.7));
        const key = new THREE.DirectionalLight(0xffffff, 2.4);
        key.position.set(model.floor.widthM * 0.4, 9, model.floor.depthM * 0.2);
        key.castShadow = true;
        scene.add(key);

        const floorGeometry = new THREE.PlaneGeometry(model.floor.widthM, model.floor.depthM);
        const floorMaterial = new THREE.MeshStandardMaterial({ color: 0x171a22, roughness: 0.9 });
        ...
        for (const element of model.elements) {
          if (!new Set(["wall", "column", "chiller", "freezer"]).has(element.type)) continue;
          ...
          const material = new THREE.MeshStandardMaterial({ color: element.type === "wall" ? 0x4b5563 : 0x334155, roughness: 0.78 });
          disposables.push(geometry, material);
          const mesh = new THREE.Mesh(geometry, material);
        ...
        for (const module of model.modules) {
          ...
          if (source) {
            const group = addFixtureAsset(THREE, scene, module, source);
            if (group) addedAssets.push(group);
            else addFallbackFixture(THREE, scene, module, disposables);
          } else {
            addFallbackFixture(THREE, scene, module, disposables);
          }
        }

        addProducts(THREE, scene, model, buildProductAssetIndex(assetManifest), disposables);
        ...
        camera.position.set(entry[0], EYE_HEIGHT_M, entry[1]);
        camera.lookAt(next[0], 1.35, next[1]);
        controls = new OrbitControls(camera, renderer.domElement);
        controls.enableDamping = true;
        controls.dampingFactor = 0.08;
        controls.enablePan = false;
        controls.minDistance = 0.25;
        controls.maxDistance = Math.max(5, Math.min(14, Math.max(model.floor.widthM, model.floor.depthM)));
        controls.target.set(next[0], 1.35, next[1]);
        controls.update();

        const resize = () => {
          const width = Math.max(1, mountRef.current?.clientWidth || 1);
          const height = Math.max(360, mountRef.current?.clientHeight || 540);
    ...
    return () => {
      ...
      for (const asset of addedAssets) disposeObject(asset);
      if (mountRef.current) mountRef.current.replaceChildren();
    };
  }, [assetManifest, model, t]);

  return (
    <div className="eay-picker-eye-scene">
...
export default function PlanogramPickerEyePreview({ engineResult, candidate, locale, formatNumber }) {
  ...
  const productCount = manifest?.product_assets?.length || 0;
  const fixtureCount = manifest?.fixture_assets?.length || 0;
  const placedProducts = Math.max(1, model.stats?.placedProductCount || 0);
  const productCoverage = Math.min(100, (productCount / placedProducts) * 100);

  return (
    <section className="eay-picker-eye">
      <header>
        <div><Eye size={21} aria-hidden="true" /><div><h3>{t("title")}</h3><p>{t("subtitle")}</p></div></div>
