    ...
    <div className="eay-store-scan-geometry">
      ...
      >
        ...
        {architecture.elements.map((element) => (
          ...
          >
            <title>{`${element.element_type} · ${element.element_id} · ${Math.round(
              element.scan_confidence * 100
            )}%`}</title>
          </polygon>
        ))}
        {fixtureRows.map((fixture) => (
          ...
          >
            <title>{`${fixture.label || fixture.element_id} · ${Math.round(
    ...
    <section className="eay-store-scan" aria-busy={running ? "true" : "false"}>
      ...
      <div className="eay-store-scan-controls">
        ...
        <button
          type="button"
          onClick={runReview}
          disabled={!bundle || !canCreate || running}
        >
          {running ? t("running") : t("run")}
        </button>
      </div>
      {!canCreate ? (
        <p className="eay-store-scan-note">{t("permissionRequired")}</p>
      ) : null}
      {error ? (
        <p className="eay-store-scan-error" role="alert">
          {error}
        </p>
      ) : null}
      ...
      {scan ? (
        <div className="eay-store-scan-result">
          <div className="eay-store-scan-boundary">
            ...
            <div>
              <strong>{t("rawMedia")}</strong>
              <span>{t("notStoreDna")}</span>
            </div>
          </div>
          <div className="eay-store-scan-metrics" aria-label={t("quality")}>
            <div>
              <span>{t("provider")}</span>
              <strong>{scan.provider}</strong>
            </div>
            <div>
              <span>{t("elements")}</span>
              <strong>{formatNumber(scan.scan_element_count || 0)}</strong>
            </div>
            <div>
              <span>{t("fixtures")}</span>
              <strong>{formatNumber(scan.recognized_fixture_count || 0)}</strong>
            </div>
            <div>
              <span>{t("lowConfidence")}</span>
              <strong>{formatNumber(scan.low_confidence_count || 0)}</strong>
            </div>
            <div>
              <span>{t("preservedV2")}</span>
              <strong>{formatNumber(scan.v2_preserved_element_count || 0)}</strong>
            </div>
          </div>
          <div className="eay-store-scan-fingerprint">
            <span>{t("fingerprint")}</span>
            <code>{scan.scan_fingerprint}</code>
          </div>
