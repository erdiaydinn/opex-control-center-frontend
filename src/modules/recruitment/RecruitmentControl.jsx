...
export default function RecruitmentControl() {
  ...
  async function refresh() {
    ...
    try { setData(await loadRecruitment()); }
  ...
  async function decide(decision) {
    ...
    try { await decideRecruitmentRequest(selected.id, decision, decisionNote.trim()); setSelected(null); setDecisionNote(""); flash(decision === "APPROVED" ? "Talep onaylandı ve e-posta kuyruğu oluşturuldu." : "Talep gerekçesiyle reddedildi."); await refresh(); }
    catch (err) { setError(err.message); }
  }
    ...
    <main className={`rec-page ${dark ? "is-dark" : ""}`}>
      ...
      <section className="rec-shell">
        ...
        <section className="rec-hero">
          <div><span className="rec-eyebrow"><BadgeCheck size={15} /> Norm kontrollü işe alım akışı</span><h1>Doğru depoya,<br /><em>doğru zamanda</em> doğru ekip.</h1><p>Aktif çalışan, açık talep, planlı ayrılış ve müdür kapasitesini tek kararda birleştirir.</p></div>
          <aside><span>Karar motoru</span><strong>Norm + Aktif Çalışan</strong><div><Check size={17} /> Müdürler norm dışında</div><div><Check size={17} /> Fulya müdür kapasitesi: 2</div><div><Check size={17} /> İstifada belge zorunlu</div></aside>
        </section>
        ...
        <nav className="rec-tabs">
          {[['overview','Genel Bakış'],['requests','Talepler'],['new','Yeni Talep'],['settings','Ayarlar ve Norm']].map(([key,label]) => <button key={key} className={tab === key ? "active" : ""} onClick={() => setTab(key)}>{label}</button>)}
        </nav>
      ...
      {selected && <div className="rec-modal-backdrop" onMouseDown={() => setSelected(null)}><section className="rec-modal" onMouseDown={(event) => event.stopPropagation()}><button className="rec-modal-close" onClick={() => setSelected(null)}><X /></button><span className="rec-kicker">TALEP KARARI</span><h2>{selected.warehouseName}</h2><p className="rec-modal-lead">{selected.positionLabel} · {selected.quantity} kişi · {selected.id}</p><EvaluationCard evaluation={selected} />{selected.evidence && <button className="rec-secondary wide" onClick={() => downloadRecruitmentEvidence(selected.id, selected.evidence.originalName)}><FileCheck2 size={17} /> İstifa belgesini indir</button>}<label>Karar açıklaması <small>Karar audit kaydına gerekçesiyle yazılır</small><textarea value={decisionNote} onChange={(e) => setDecisionNote(e.target.value)} placeholder="Norm, aktif çalışan ve operasyon ihtiyacına göre karar gerekçesi…" /></label>{canApprove ? <div className="rec-decision-actions"><button className="reject" onClick={() => decide("REJECTED")}><X size={17} /> Reddet</button><button className="approve" onClick={() => decide("APPROVED")}><Check size={17} /> Onayla ve bildir</button></div> : <div className="rec-alert">Bu kullanıcı yalnızca talebi görüntüleyebilir.</div>}</section></div>}
    </main>
  );
}
